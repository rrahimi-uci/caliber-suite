...
| # | Step | File | Do this |
| --- | --- | --- | --- |
| 1 | Register `catalog-github` | — (catalog tile) | Add `api.githubcopilot.com` to the server's `CALIBER_MCP_REMOTE_HOST_ALLOWLIST`. Then open `Library → MCP Servers`, click `data-testid="catalog-github"`. It seeds the official endpoint `https://api.githubcopilot.com/mcp/`, transport `streamable-http`, and token env var `GITHUB_PERSONAL_ACCESS_TOKEN`. Name it, supply the token, **Register**. API: `POST /mcp-servers {name, transport, uri, auth_type, auth_config, env}` |
...
| 4 | Set policy on the write tool | [`policy/github-issue-write.block.policy.json`](policy/github-issue-write.block.policy.json) (block — use this for the read-only explorer) **or** [`policy/github-issue-write.approval.policy.json`](policy/github-issue-write.approval.policy.json) (approval — workflow-time gate only) | Server detail → row `issue_write` → set the policy. API: `PATCH /mcp-servers/{id}/tools/issue_write/policy` with the JSON body (drop the `_comment` key — it is documentation only) |
| 5 | Re-invoke the governed tool | [`dataset/policy-enforcement-checks.jsonl`](dataset/policy-enforcement-checks.jsonl) | Playground → `issue_write` → invoke → **expect a refusal**, not an execution: `success:false`, `result:null`, `duration_ms:0`, `error:"Tool 'issue_write' is blocked by policy"`. This is your enforcement record, and it only holds for the **block** (`allowed:false`). API: same `invoke-tool` endpoint |
