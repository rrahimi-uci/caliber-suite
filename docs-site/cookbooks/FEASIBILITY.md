...
| | Playground | ⚠️ Render-only | `POST /prompts/{name}/test-render` renders the template with variables; it does **not** call a model. For real scored runs use **Evaluations**. |
...
| **Aria / Aria Plans** | Plan decompose → approve → execute | ✅ Works | Real LLM-backed (OpenAI/Anthropic) + `fake` test double. `POST /aria/plans`, `/approve`, `/execute`, `/poll`. |
