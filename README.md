...
| 📐 [`docs/`](./docs/) | Source-of-truth implementation and per-area design specs beneath the root layered map — one `architecture.md` per numbered area (platform, registries, workflows, data, observability, QA, evaluation, calibration, assistant) plus the [workflow components reference](./docs/06-workflows/components.md). Each opens with an **At a glance** summary + a typed-color diagram, then a banded **Reference** tier. Conventions in [`docs/STYLE.md`](./docs/STYLE.md); machine index for agents at `docs-site/llms.txt`. Dated point-in-time audit records — frozen by design, appended to rather than rewritten — live in [`docs/reports/`](./docs/reports/) and are not published to the site. |
| 📄 [`paper/`](./paper/) | LaTeX source for the technical paper *CALIBER: A Layered Control Plane for Per-Family Governance of AI-Agent Resources* — the architectural argument behind this repository, its per-family guarantee surface, and an explicit account of what it does **not** establish. Also holds the generated [seminar deck](./paper/slides/). See [`paper/README.md`](./paper/README.md). |
