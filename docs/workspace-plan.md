...
| `P4-B` | Import/backend | **Partially delivered (source materialization, import-lease, and bounded-retry slices).** Added provider-free ZIP source validation with the ratified 100 MB compressed-bundle, 10,000-entry, 25 MB-file, 1 GiB-total, and 50:1 decompression-ratio limits; normalized repository-relative paths, duplicate-path and symlink rejection; deterministic media-type/executable-aware source-tree digests; and fixed tar snapshots suitable for content-addressed storage. Added an idempotent source/commit observation check that raises `source_commit_digest_conflict` on equivocation while ignoring ZIP ordering/compression/timestamp metadata. Added provider-neutral durable job arbitration: oldest queued imports are atomically claimed, live leases can be renewed only by their owner, terminal writes are lease-fenced, and expired workers move to `reconcile_required` without blind retry. Added durable total-attempt budgets: claims increment `attempt_count`, only jobs below `max_attempts` enter the queue, known `failed` outcomes can be deliberately requeued while budget remains, and exhausted or `reconcile_required` jobs refuse retry. Still open: mutable-resource adapter snapshots, model dependency policy, provider adapters, and public import APIs | `P4-A` | Canonical tree and retained snapshot bytes are stable across ZIP metadata changes; path traversal, symlink, duplicate-path, archive-bomb, size, and digest-equivocation inputs fail closed; claim races allow one winner, stale workers cannot publish late results, bounded retry never exceeds its durable budget, and expired imports require explicit observation/reconciliation before retry |
