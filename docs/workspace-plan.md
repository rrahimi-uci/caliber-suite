...
3. Add missing indexes and FKs where migration evidence permits.
   **Delivered (partial, `P2-F`).** The indexes half: a pass over
   ...
   `caliber_workflow_benchmark_reports`) are left untouched. **Not yet
   delivered:** the FKs half. No `ForeignKey` was added on any `project_id`
   column -- an FK on a column that may already carry stray/legacy values
   (never enforced before now) would either fail to apply outright or
   silently corrupt the affected rows. This stays blocked on a real
   orphan-reference audit (`db/legacy_data_report.py`'s planned
   `orphan_project_id_report`, alongside the already-shipped
   `legacy_null_report()`/`duplicate_name_report()`), not yet written.
