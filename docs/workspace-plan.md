...
6. Freeze the SDK-facing contracts: list pagination shape, error envelope and
   ...
   **The error-envelope slice is delivered** (`P0-B`): the transport-level
   ...
   codes remain a real, open gap**, not yet built as a transport field across
   the API: most route call sites still attach only a human-`detail` string,
   while the P4-C source routes use stable detail values but do not yet expose
   a separate `reason_code` field. The SDK's exception hierarchy
   (`sdk/caliber-sdk/src/caliber_sdk/errors.py`) still has no `reason_code`
   attribute and no `CaliberPreconditionError` class. Section 13.6 already describes the target shape in
   full; turning it into a closed reason-code registry attached to every
   call site is a materially larger, separate undertaking than this
