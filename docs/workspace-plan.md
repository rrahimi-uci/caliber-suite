...
| `P6-B` | SDK + API | **Partially delivered (source/import/revision/Change-Request slices).** Added `CursorPage[T]` (the opaque-token pagination primitive Workspace's cursor-based list endpoints need, distinct from the rest of the SDK's `limit`/`offset` `Page`); `WorkspaceImportJob`/`WorkspaceRevision`/`WorkspaceRevisionResource`/`WorkspaceRevisionDiff`/`WorkspaceImportReconciliation`/`WorkspaceSource`/`WorkspaceSourceState`/`WorkspaceSourceCapabilities`/`WorkspaceChangeRequest`/`WorkspaceChangeRequestHead`/`WorkspaceChangeRequestReviewer`/`WorkspaceChangeRequestComment`/`WorkspaceChangeRequestCheck`/`WorkspaceChangeRequestReview`/`WorkspaceExternalReviewAttestation`/`WorkspaceVersionTag` models; and `ProjectSourceAPI` (`get`/`configure`/`enable`/`disable`/`reconcile`/`capabilities`, every mutation `If-Match`-checked)/`ProjectImportsAPI` (`list`/`get`/`create`/`reconcile`/`wait`, multipart bundle upload, digest-safe-replay `Idempotency-Key`)/`ProjectRevisionsAPI` (`list`/`get`/`diff`)/`ProjectChangeRequestsAPI` (`list`/`get`/`create`/`submit`/`update_head`/`rebase`/`close`, comments, reviewers, reviews, external-review attestations/refresh, checks -- lock-based mutations take `expected_lock_version`, the server's own optimistic-concurrency field for this resource, distinct from `ProjectSourceAPI`'s `If-Match` etag)/`ProjectVersionTagsAPI` (`list`/`get`), exposed as `client.workspaces.source`/`.imports`/`.revisions`/`.change_requests`/`.version_tags`. Closes 32 of `coverage_allowlist.toml`'s 38 tracked `P6-B` gaps (the number of gaps actually present when this row was last verified against the file, corrected from an earlier imprecise "40"). Deliberately sync-only, matching the table's own phase split (`P6-C` owns async parity) rather than the fuller Phase 6 narrative's "sync and async together" phrasing for this step -- revisiting both at once would have coupled two independently reviewable changes. **Still open:** members grouped API/ownership-transfer conveniences (largely already covered by pre-existing `ProjectsAPI` methods -- audited, not re-verified here), environment `update`/`rollback` with ETags (list/get/enable/disable already existed pre-`P6-B`), release/release-operation methods and their waiters, and the remaining 6 allowlist gaps (all release-operation routes) | `P4-D`, `P5-D`, `P6-A` | OpenAPI inventory has no untracked Workspace route; examples drive package, review and development release without raw HTTP |
