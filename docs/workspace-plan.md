...
| `P2-B` | Runtime | **Partially delivered (slice 1).** `workflows/deployment_bundle.py::build_deployment_bundle`'s three remaining bare-name/id lookups (eval dataset, skill, knowledge base) and `workflows/promoter.py`'s two siblings in the same compile/promote pipeline (`_skill_contents_for`'s skill-content resolution, `evaluate_deploy_gates`'s dataset resolution) now resolve through `apply_visibility_filter`/`get_visible`, keyed off a new `db/scoping.py::synthetic_identity(owner, project_id)` -- a non-request identity built from the workflow's own recorded `owner`/`project_id` (there is no live requester identity at compile/promote time). Two projects using the same skill/dataset/KB name can no longer resolve to, embed, or gate a release on each other's row. **Not yet delivered:** run queue, workers, callbacks, and plan-executor/Aria delegation project-awareness (this row's remaining named scope), plus the `orchestrator/*.py` and `routes/workflow_versions.py` bare-name sites flagged during this slice but not yet touched | `P2-A` | Two new tests prove a same-named knowledge base in a different project is hidden from bundle resolution (`status": "unresolved"`, no `snapshot` key) while the owner's own is resolved; a workflow with no matching owner/project row falls back to an empty synthetic identity rather than an unscoped global lookup; persisted context surviving restart and worker actor-authority containment remain open for the run-queue/worker slice |
...
5. Make workflow compiler and runtime resolvers project- and revision-aware for
   ...
   **Delivered (partial, `P2-B`).** The compile/promote pipeline's remaining
   bare-name/id lookups: `workflows/deployment_bundle.py::build_deployment_bundle`'s
   ...
   identity at compile/promote time. MCP bindings remain unscoped
   (`CaliberMcpServer` has no `project_id`/`visibility` at all -- its own
   schema migration, item 4's job). Run queue/workers/callbacks/plan-executor
   project-awareness and the `orchestrator/*.py`/`routes/workflow_versions.py`
   bare-name sites flagged during this slice remain open.
