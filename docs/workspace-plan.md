...
5. Make workflow compiler and runtime resolvers project- and revision-aware for
   ...
   **Delivered (partial, `P2-B`).** Slice 1: the compile/promote pipeline's
   ...
   job). Still open, explicitly sized as a separate future slice: the Aria
   delegation *tool surface* (`assistant/agent_tools.py`, `assistant/tools.py`,
   `assistant/service.py`, `assistant/skill_runtime.py`).
