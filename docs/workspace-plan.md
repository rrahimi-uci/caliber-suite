...
| `P2-C` | Integrations/storage | **Partially delivered (assistant publisher context slice).** Route-backed assistant publishing now threads the authenticated `CaliberIdentity` into tool, skill, workflow, and MCP-server creation, persisting the active project and the same no-project visibility defaults as their REST routes; prompt publishing preflights the CALIBER-side hidden prompt target before the MLflow write and provisions a new target with the caller's project context. Direct publisher callers retain compatibility behavior. **Not yet delivered:** storage/file isolation and the provider-side prompt namespace decision, plus immutable pin/reconstructability semantics | `P2-A` | A project-scoped assistant publish creates visible, project-bound registry resources; a prompt name owned by another project is refused before the provider write; remaining storage/provider/pin work must still prove that guessed refs cannot cross workspaces |
