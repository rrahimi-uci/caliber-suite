...
| `P2-D` | SDK/CLI | **Delivered (partial).** `resources/projects.py`'s 18 `/projects/{project_id}/...` call sites now pin `X-CALIBER-Project` to the path's own id via a new tri-state `project` parameter on `Transport.request()`/`download()` (sync and async); a mismatched explicit override vs. a manual header raises `CaliberConfigError` before I/O. `caliberctl`'s 4 project-scoped writes with no other way to name a project (`workflow run`, `cookbook install`, `service publish`, `gate-verdict record`) now refuse without `--project`/`$CALIBER_PROJECT` instead of silently writing an unscoped row. **Not yet delivered:** the `ContextVar` ambient-scope migration, the server-side `400 workspace_context_mismatch`, and an async `ProjectsAPI` | -- | A `PRJ-A`-scoped client asking about `PRJ-B` by id sends `PRJ-B`; the pin does not leak into the next call; a workspace-bound CLI write with no resolved project is refused with `exits.USAGE`, not silently unscoped |
...
8. Make SDK and CLI automation send an explicit project scope for every
   ...
   **Delivered (partial).** A project-scope mechanism already existed
   ...
   `gate-verdict record`) now refuse with a usage error instead. **Not yet
   delivered:** the `ContextVar`-based ambient-scope migration (today's
   `project_scope()` mutates shared transport state, documented as
   thread-unsafe in its own docstring) and the server-side `400
   workspace_context_mismatch` for a header/path disagreement -- both
   independently useful, differently-shaped follow-up work; the async
   client also still has no `ProjectsAPI` equivalent at all (a separate,
   larger resource-tree gap, not specific to project scoping).
