...
| `P2-B` | Runtime | **Partially delivered (slice 3).** Slice 1: `workflows/deployment_bundle.py::build_deployment_bundle`'s three remaining bare-name/id lookups (eval dataset, skill, knowledge base) and `workflows/promoter.py`'s two siblings in the same compile/promote pipeline now resolve through `apply_visibility_filter`/`get_visible`, keyed off a new `db/scoping.py::synthetic_identity(owner, project_id)`. Slice 2: `workflows/promoter.py::resolver_from_session` -- the tool-registry-to-resolver builder used everywhere a workflow references a tool -- queried every tool globally with no visibility filter; now scoped at all 12 call sites, and `routes/workflows.py`'s independently-reinvented (and subtly incomplete) `_scoped_tool_resolver` is retired in favor of it. Slice 3 ("run queue + plan executor", the highest-severity remainder): (a) the *subworkflow* branch of both `build_deployment_bundle` and `promoter.py::build_plan`'s `_resolve_version` resolved a child `workflow_id` with no visibility check at all -- a manifest naming another project's workflow could get pinned at compile time and **executed at run time**, both now closed via `get_visible`; (b) `build_plan`'s `_enqueue_review`/`routes/review_queues.py::add_review_items_records` let the `review_queue_enqueue` runtime node write into another project's queue by a guessed id -- the visibility check now lives inside `add_review_items_records` itself so route, Aria, and runtime callers all get it for free; (c) `orchestrator/workflow_stages.py::_dataset_inputs` (refinement replay-input dataset) and `workflows/calibration.py::_resolve_active_dataset` (calibration dataset) had the same unscoped dataset-by-name lookup `evaluate_deploy_gates` already had fixed, now closed the same way; (d) the Aria plan worker (`assistant/executor.py::poll_waiting_plans`) resumed every parked plan as `actor="@system"`, `project_id=None` regardless of the plan's own recorded owner/project -- every capability a background-resumed step touched silently lost its project tier; it now reads and resumes as the plan's own identity. **Not yet delivered:** the refinement-worker pipeline's remaining enumeration/name-resolution sites (`orchestrator/candidate.py`, `evidence.py`, `triage.py`, `artifact_store.py`) and the Aria delegation *tool surface* (`assistant/agent_tools.py`, `assistant/tools.py`, `assistant/service.py`, `assistant/skill_runtime.py`) -- both scoped and sized as separate future slices, not attempted here; MCP bindings remain structurally blocked on a schema migration | `P2-A` | Two new tests (slice 1) prove a same-named knowledge base in a different project is hidden from bundle resolution; new tests (slice 3) prove: a same-named child workflow in a different project is hidden from both compile-time bundling and an actual run-worker execution (`"subworkflow ... is not available"`, not a silent cross-project run); `add_review_items_records` refuses a queue in a different project even when called directly (the runtime's own path, not just the route); and a plan resumed by `poll_waiting_plans` runs its next step as the plan's own owner/project, not `@system`/no-project; full backend suite (6727 tests) green after all three slices |
...
5. Make workflow compiler and runtime resolvers project- and revision-aware for
   ...
   **Delivered (partial, `P2-B`).** Slice 1: the compile/promote pipeline's
   ...
   identity. MCP bindings remain unscoped (`CaliberMcpServer` has no
   `project_id`/`visibility` at all -- its own schema migration, item 4's
   job). Still open, explicitly sized as separate future slices per the
   research behind this row: the refinement-worker pipeline's remaining
   enumeration/name-resolution sites (`orchestrator/candidate.py`,
   `evidence.py`, `triage.py`, `artifact_store.py`) and the Aria delegation
   *tool surface* (`assistant/agent_tools.py`, `assistant/tools.py`,
