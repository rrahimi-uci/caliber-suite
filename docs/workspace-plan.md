...
8. Make SDK and CLI automation send an explicit project scope for every
   ...
   **Delivered (follow-up slice, `P2-D`).** Ambient project selection now uses
   ...
   another caller sharing the client. **Not yet delivered:** the async client
   still has no `ProjectsAPI` equivalent (a separate, larger resource-tree
   gap, not specific to project scoping).
