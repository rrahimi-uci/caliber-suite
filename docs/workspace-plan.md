...
`P7-A` dependency check (as of the encrypted-connection-storage/real-provider-
registration slice of `P4-E` above): `P6-C` is delivered. `P4-E` is **not**
yet fully closed -- production webhook ingress and durable provider-payload
ingestion, actor-link revalidation against provider identity,
pagination/reconciliation, first-package baseline procedure, and production
status publication wiring remain open per its own table row. `P7-A` therefore
remains blocked; Phase 7 should not start until the rest of `P4-E` lands.
