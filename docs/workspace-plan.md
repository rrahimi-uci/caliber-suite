...
| `P2-B` | Runtime | **Partially delivered (slice 2).** Slice 1: `workflows/deployment_bundle.py::build_deployment_bundle`'s three remaining bare-name/id lookups (eval dataset, skill, knowledge base) and `workflows/promoter.py`'s two siblings in the same compile/promote pipeline (`_skill_contents_for`'s skill-content resolution, `evaluate_deploy_gates`'s dataset resolution) now resolve through `apply_visibility_filter`/`get_visible`, keyed off a new `db/scoping.py::synthetic_identity(owner, project_id)` -- a non-request identity built from the workflow's own recorded `owner`/`project_id` (there is no live requester identity at compile/promote time). Slice 2: `workflows/promoter.py::resolver_from_session` -- the tool-registry-to-resolver builder used by the compiler and runtime everywhere a workflow references a tool -- queried every tool globally with no visibility filter at all; now scoped at all 12 call sites (`promoter.py`'s own compile/build-plan/promote paths reuse its pre-existing `build_workflow_identity` rather than the slice-1 helper; `orchestrator/workflow_stages.py`'s 3 refinement-stage call sites do the same; `routes/workflow_versions.py`'s 5 route call sites pass the real request identity). `routes/workflows.py`'s independently-reinvented `_scoped_tool_resolver` -- which applied the visibility filter but silently dropped the non-archived-status filter and successor-tool resolution the canonical builder already had -- is retired in favor of `resolver_from_session`, closing the drift between the two. Two projects using the same skill/dataset/KB/tool name can no longer resolve to, embed, gate a release on, or compile against each other's row. **Not yet delivered:** run queue, workers, callbacks, and plan-executor/Aria delegation project-awareness (this row's remaining named scope) | `P2-A` | Two new tests (slice 1) prove a same-named knowledge base in a different project is hidden from bundle resolution while the owner's own is resolved; a workflow with no matching owner/project row falls back to an empty synthetic identity rather than an unscoped global lookup; full backend suite (6722 tests) green after both slices, including the retired `_scoped_tool_resolver`'s former callers; persisted context surviving restart and worker actor-authority containment remain open for the run-queue/worker slice |
...
5. Make workflow compiler and runtime resolvers project- and revision-aware for
   ...
   **Delivered (partial, `P2-B`).** Slice 1: the compile/promote pipeline's
   ...
   canonical builder, closing the drift between them. MCP bindings remain
   unscoped (`CaliberMcpServer` has no `project_id`/`visibility` at all --
   its own schema migration, item 4's job). Run queue/workers/callbacks/
   plan-executor project-awareness remain open.
