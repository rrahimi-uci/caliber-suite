...
| `P2-G` | Backend | **Partially delivered.** Item 4: no `CaliberPrompt` model exists (a prompt is purely an MLflow Prompt Registry entity, flat and globally named) -- the only CALIBER-side data is a hidden runtime target (`CaliberAgentConfig` row keyed by `agent_id == prompt_name`) that already carries `project_id`/`visibility` but was never checked. `prompt_targets.py::ensure_prompt_target` now takes an optional `identity` and refuses (404) to reuse/mutate an existing target invisible to the caller; `routes/prompts.py::create_prompt` checks before its MLflow write (never leaves an orphaned registered version on refusal); `get_prompt`/`list_prompts` both hide a name whose target belongs to another project while a bare provider-only/legacy name (no target row) stays visible to everyone. **Not yet delivered:** the actual MLflow-side namespace scheme -- two projects registering the same name still land in one interleaved MLflow entity; this needs a real product decision (silently rewrite the MLflow-side name, refuse the second registration, or something else) that trades off against this route's own stated promise that a prompt is visible identically from CALIBER and MLflow's native UI | `P2-A` | A stranger cannot take over, read, or list another project's prompt via its hidden target; a bare MLflow-only prompt with no CALIBER row stays visible to everyone; `create_prompt`'s MLflow `register_prompt` is never called when the pre-check refuses |
