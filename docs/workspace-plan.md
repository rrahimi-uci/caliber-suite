...
| `P5-F` | Release/integrations | **Delivered.** Added the Workspace release HTTP surface `P5-A` through `P5-D`'s service/governance layer never had: `caliber/src/caliber/routes/workspace_releases.py` (`GET`/`POST /projects/{id}/releases`, `GET .../releases/{id}`, `GET .../evidence`, `POST .../evaluate`, `GET .../evaluations`, `GET .../evaluations/{id}`, `POST .../quality-signoff`, `POST .../approve`, `POST .../break-glass-apply`), wrapping `workspace_release_service.py`/`workspace_release_governance.py` exactly as they already worked and were already tested -- no new business logic. `workspace_change_requests.py`'s own module docstring named this exact gap: "the Phase-5 acceptance primitive is intentionally not routed here." Create/read/evaluate authorize via `require_project_access` in the route layer (`release.request`/`release.evaluate`/`read`, already-registered actions); quality-signoff/approve/break-glass delegate authorization entirely to the governance module, which needs identity-specific role checks a scope check cannot express. List endpoints use `limit`/`offset`, matching the sibling release-operations routes rather than the cursor convention newer Workspace lists use -- the two conventions already coexist in shipped code. **Named, deliberate gap:** break-glass-apply's `machine_gates_passed`/`integrity_checks_passed` booleans are hardcoded `True` -- `create_workspace_break_glass_apply`'s own docstring says a caller must "replace/revalidate them from persisted machine and integrity evidence immediately before the first external effect," and no route anywhere re-derives that evidence yet. Every other break-glass precondition (gate-digest binding, verified QA->staging->prod chain, distinct-actor, one-hour TTL, real session credential) is still fully enforced. | `P5-D` | Route-level tests pin request parsing, status codes, the authorization-boundary split, and exception-to-HTTP mapping (the state machine and decision rules themselves are already exhaustively covered at the service layer); `test_route_scope_inventory.py`/`test_routes_openapi.py` pass with zero manual inventory edits; break-glass-apply is proven callable only by a real interactive-session credential, never trusted-header/PAT auth |
