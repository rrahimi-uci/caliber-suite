...
| `P2-E` | Backend | **Partially delivered.** Item 7 (route Aria capabilities and durable plan execution through the central authorization contract): `routes/aria_plans.py::execute_plan`/`poll_plan` checked resource *visibility* only -- any active project member (including a plain `viewer`) could execute or poll a teammate's project-scoped plan. Both routes now also call `require_project_access_if_scoped` against the plan's own `project_id` with the `resource.execute` action, the same role floor (owner/editor/reviewer) `create_workflow_run` already enforces; a personal (`project_id is None`) plan is unaffected. Separately, `assistant/agent_tools.py::_dispatch_capability` (the synchronous per-turn tool-call path) never consulted a capability's declared `required_scopes` at all -- only the async plan executor did; now it consults the same `scopes_for_user` primitive. **Not yet delivered:** the rest of item 7 (a first-class scope/role decision recorded on the plan/step rows rather than re-derived per call) | `P1-D` | A project member with only the `viewer` role is refused (403) on `execute`/`poll` for a project-scoped plan they can see but not act on; a member with `editor`/`owner`/`reviewer` succeeds; a capability requiring a scope the caller lacks is refused by `_dispatch_capability`, not silently run |
...
7. Route Aria capabilities and durable plan execution through the central
   ...
   **Delivered (partial, `P2-E`).** Two independent gaps closed. First,
   ...
   *visibility* (`PlanService.get_plan`) before running a project-scoped
   ...
   routes now call `require_project_access_if_scoped` with the plan's own
   `project_id` right after the visibility check, closing the gap for a
   personal (`project_id is None`) plan is unaffected. Second,
   ...
   already used. **Not yet delivered:** the durable-plan-execution half of
   this item beyond these two gaps -- e.g. a first-class scope/role
   decision recorded on the plan/step rows themselves rather than
   re-derived at each call -- remains open.
