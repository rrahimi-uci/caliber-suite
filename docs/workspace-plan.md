...
version_applicability: reviewed against origin/main at c220a946e86; the Workspace, revision, environment, release, rework, and CALIBER SDK models described as target state remain proposed except for the explicitly marked delivered slices, including P4-E's provider-neutral source-control contract and offline fake adapter on this branch
tags:
  - workspace
  - lifecycle
  - cicd
...
| `P4-E` | Integration/security | **Partially delivered (provider-neutral SourceControlProvider contract and deterministic fake-adapter slice).** Added typed capabilities, commit/tree and reachability comparisons, change-request/review/check/ruleset normalization, exact-head and complete-range verification, signed webhook normalization, optional status publication, and a bridge into P4-D's external-attestation input shape. The fake adapter performs no network I/O and refuses unavailable capabilities explicitly. Still open: least-privilege GitHub App, durable signed-webhook inbox, actor-link persistence/revalidation, provider reconciliation, and production status publication | `P4-D` | Offline contract tests prove exact-head and source-range binding, unknown/bot/untrusted evidence does not count, signature and repository checks fail closed, and missing capabilities are typed refusals; the slice remains partial until durable event and real-provider integration land |
