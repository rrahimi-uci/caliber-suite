...
8. Make SDK and CLI automation send an explicit project scope for every
   ...
   **Not yet delivered:**
   the `ContextVar`-based ambient-scope migration (today's `project_scope()`
   mutates shared transport state, documented as thread-unsafe in its own
   docstring); the async client also still has no `ProjectsAPI` equivalent at
   all (a separate, larger resource-tree gap, not specific to project
   scoping).
