...
| `P4-B` | Import/backend | **Partially delivered (source materialization slice).** Added provider-free ZIP source validation with the ratified 100 MB compressed-bundle, 10,000-entry, 25 MB-file, 1 GiB-total, and 50:1 decompression-ratio limits; normalized repository-relative paths, duplicate-path and symlink rejection; deterministic media-type/executable-aware source-tree digests; and fixed tar snapshots suitable for content-addressed storage. Added an idempotent source/commit observation check that raises `source_commit_digest_conflict` on equivocation while ignoring ZIP ordering/compression/timestamp metadata. Still open: mutable-resource adapter snapshots, model dependency policy, durable claim/lease/retry/reconcile worker behavior, provider adapters, and public import APIs | `P4-A` | Canonical tree and retained snapshot bytes are stable across ZIP metadata changes; path traversal, symlink, duplicate-path, archive-bomb, size, and digest-equivocation inputs fail closed; provider/import execution and worker leases remain dormant until later slices |
