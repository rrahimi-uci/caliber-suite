...
| `P6-B` | SDK + API | **Partially delivered (import/revision slice).** Added `CursorPage[T]` (the opaque-token pagination primitive Workspace's cursor-based list endpoints need, distinct from the rest of the SDK's `limit`/`offset` `Page`), `WorkspaceImportJob`/`WorkspaceRevision`/`WorkspaceRevisionResource`/`WorkspaceRevisionDiff`/`WorkspaceImportReconciliation` models, and `ProjectImportsAPI` (`list`/`get`/`create`/`reconcile`/`wait`, multipart bundle upload, digest-safe-replay `Idempotency-Key`)/`ProjectRevisionsAPI` (`list`/`get`/`diff`), exposed as `client.workspaces.imports`/`.revisions`. Closes 7 of `coverage_allowlist.toml`'s 40 tracked `P6-B` gaps. Deliberately sync-only, matching the table's own phase split (`P6-C` owns async parity) rather than the fuller Phase 6 narrative's "sync and async together" phrasing for this step -- revisiting both at once would have coupled two independently reviewable changes. **Still open:** members grouped API/ownership-transfer conveniences (largely already covered by pre-existing `ProjectsAPI` methods -- audited, not re-verified here), source get/configure/enable/disable/capabilities/reconcile with ETags, Change Request lifecycle/native-review/external-attestation/history, version-tag history, environment `update`/`rollback` with ETags (list/get/enable/disable already existed pre-`P6-B`), release/release-operation methods and their waiters, and closing the remaining 33 allowlist gaps | `P4-D`, `P5-D`, `P6-A` | OpenAPI inventory has no untracked Workspace route; examples drive package, review and development release without raw HTTP |
