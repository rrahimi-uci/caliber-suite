...
7. Add optional PAT project binding and identity credential context; require
   ...
   **Delivered** (`P1-E`): `caliber_personal_access_tokens` gained a nullable
   ...
   segment, and is a no-op when neither channel names a project at all. Not
   delivered: the "resource owner" and "persisted worker context" conjuncts
   section 9.2 also names, and "require project-bound PATs for CI import" --
   neither has a modeled check yet (no per-resource pinning, no
   worker-identity context, no CI-import route at all -- Phase 2/4's job);
   enforcing those now would be dressing up an unbuilt no-op as a real check.
