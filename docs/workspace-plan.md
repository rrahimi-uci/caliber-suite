...
version_applicability: reviewed against origin/main at 0300fb5c996; the Workspace, revision, environment, release, rework, and CALIBER SDK models described as target state remain proposed except for the explicitly marked delivered slices, including P4-E's provider-neutral source-control contract, offline fake adapter, injectable GitHub REST normalizer, durable source-event/actor-link persistence, and P5-A's provider-free Workspace release foundation on this branch
tags:
  - workspace
  - lifecycle
  - cicd
...
**This made isolation closure a hard prerequisite for shipping QA, and it
still is for the resource families `P2-P` did not touch.** Before `P2-P`,
the mutating prompt routes checked global scope only, with no project-role
consultation — granting QA `caliber.operator` let QA edit prompts in any
project, a role that looked restricted while being fully privileged.
**Delivered** (`P2-P`): `routes/prompts.py`, `routes/workflows.py`,
`routes/tools.py`, `routes/skills.py`, `routes/eval_datasets.py`, and
`routes/judges.py` now call `require_project_access_if_scoped` on their
create/update/delete routes, closing exactly the gap this paragraph
describes for those six modules — see the `P1-D` row/item 5 below for the
full route list and role composition.

### 2.4 Canonical action registry and scope requirements

The target registry below is authoritative and, as of `P2-P`, is what
...
`resource.write.evidence`, `resource.publish`, `resource.execute`,
`feedback.submit`); the rest are reserved for the Change-Request/
environment/release route families Phase 2-5 build, or (`rework.update`)
deliberately not yet wired onto a route that already exists
(`caliber_rework_tasks` has no `project_id` column yet).
...
stateDiagram-v2
    ...
    technically_approved --> accepted: QA go decision recorded, then change_request:accept wins the CAS
...
preserves the negative result. QA sign-off and Change Request acceptance are
two separate calls, not one atomic step: quality sign-off records a durable,
digest-bound `go`/`no_go` decision against the exact `technically_approved`
head, and the Change Request's own explicit acceptance call then derives that
recorded decision and attempts the CAS below. There is no intermediate
Change-Request-level "QA in progress" status; the head stays
`technically_approved` until acceptance succeeds or the base moves and it
becomes `out_of_date`.
...
| QA candidate version *(reserved, not created by the shipped MVP flow)* | `1.4.0-rc.2` | An earlier design step: an immutable prerelease tag assigned to the exact head entering QA, a changed head getting `rc.3` and never replacing `rc.2`. The schema and `create_version_tag(kind="qa_candidate")` still support this shape, but no shipped flow calls it -- quality sign-off is recorded directly against the `technically_approved` head, and the accepted package version below is the only tag the MVP ever creates |
...
sequenceDiagram
    ...
    D->>C: Call change_request:accept
...
1. **A rework assignment.** *Delivered by `P3-A`.* A failed gate produces an
   owned, visible `caliber_rework_tasks` row rather than a terminal `rejected`
   row with nothing pointing at it. The original global routes remain
   compatible, and the project-scoped `/projects/{id}/rework-tasks` collection
   now derives ownership from the source agent's project binding. Its
   `claim`/`resolve`/`reassign` routes enforce project membership and the
   `rework.update` role action; `resolve` optionally links the superseding job
   that fixed it.
...
permissions, and policy version — a distinct concept from the HTTP error
envelope's per-error `reason_code` (section 13.6, Phase 0 item 6): this one
explains an authorization verdict, that one explains why a `409`/`412`
happened. Sensitive not-found cases return an
...
`(project_id, tag)`. Acceptance creates `<version>` for the accepted digest
and marks the claim accepted; this is the only tag kind the shipped MVP flow
ever creates. `qa_candidate`'s `<version>-rc.<head_generation>` shape remains
in the schema and in `create_version_tag(kind="qa_candidate")` as a reserved,
superseded design step: the original design had entering QA mint that
immutable candidate tag and move the Change Request to a distinct
`qa_in_progress` status before quality sign-off. Neither shipped --
`create_version_tag(kind="qa_candidate")` has no production caller and no
Change Request is ever assigned the `qa_in_progress` status. The shipped flow
instead records the QA `go`/`no_go` decision directly against the
`technically_approved` head
(`workspace_release_governance.py::record_workspace_release_decision`), and
the Change Request's own separate, explicit acceptance route (`POST
.../change-requests/{id}:accept`, `P4-D`) derives that recorded decision and
attempts the acceptance CAS. Tags cannot be moved or deleted in the MVP. `v`
is presentation syntax only; API storage and comparison use canonical SemVer.
...
| `P0-B` | API + SDK | **Delivered.** OpenAPI shapes: `x-caliber-required-scope` (`P0-A`'s route-scope inventory) is now rendered as a "Required scope" column in the published REST API reference (`docs-site/generate_rest_api_docs.py::_format_required_scope`), not just visible in the raw served JSON. Error envelope: ratified as frozen — `routes/_errors.py`'s `{detail, status_code}` / `{detail, status_code, errors[]}` shapes were already implemented, tested, OpenAPI-declared, and published before this PR; no code changed. Closed actions: ratified in the original Phase-0 baseline and subsequently expanded by P1-D and P4-C; the current live/reserved partition is maintained by `tests/test_route_scope_inventory.py`. This is *not* P1-B's "Closed action enum" (section 2.4's much larger future `WorkspaceAction` vocabulary), which remains unstarted and gated on `P1-A`. Reason codes: named as a real, deliberately deferred gap when this row was first delivered; **closed for a first slice in a later follow-up** (PR #416) — `routes/_errors.py::CaliberHTTPException` and an optional, additive `reason_code` envelope field, the SDK's matching `CaliberAPIError.reason_code`/`ErrorBody.reason_code` and new `CaliberPreconditionError` (412), and three of `routes/workspace.py::snapshot_revision`'s already colon-prefixed managed-snapshot-adapter failures migrated to set it explicitly — with the repo-wide route migration left as a distinct, explicitly named follow-up (not silently done for free); see Phase 0 item 6 for the full accounting. Pagination: inventoried and ratified (`routes/pagination_inventory.py`, 6 distinct shapes across ~27 routes pinned, `list_limit()` ratified as the near-term baseline, deviations named not reconciled). ETag/CAS/idempotency: legacy `409`-based mechanisms remain the current precursor to section 13.6's target contract; P4-C additionally introduces header-based source ETags and declared `412` preconditions. Change Request/version contracts: sections 3.2/9.2's already-written design ratified as frozen, plus `workspace_change_requests.py`'s model-based transition fixtures (Phase 4 still owns all real implementation). Manifest schema and golden vectors: `workspace_manifest.py` implements and tests the `v1alpha1` schema and canonicalization (22 golden-vector tests) — the manifest-document half of the eventual `revision_sha256`; full materialization remains Phase 4. Multipart field names, maximum bundle size (a new, reasoned policy decision), and sync/async streaming (decision ratified) round out item 6 in full — the heartbeat-test verification this row once named as pending against real async upload code is now closed by `test_workspace_import_worker.py::test_heartbeat_loop_renews_once_then_stops`/`::test_heartbeat_loop_stops_renewing_once_the_lease_is_lost`/`::test_heartbeat_loop_swallows_unexpected_renew_errors`/`::test_real_heartbeat_renews_the_lease_of_a_running_job`, once the import worker itself existed (see `P4-B`'s row). **Explicitly out of scope for this ticket** (Phase 0 items with no PR-sequence ticket, not owned by "API + SDK," left exactly as open as before): item 3's remaining ~~nullability/uniqueness audit and bare-name-resolver inventory~~ (both halves are now delivered — see `P2-A`'s row: the nullability/uniqueness half by slice 10, and the bare-name-resolver-inventory half — the same missing-DB-constraint gap that row named separately — by slice 11), item 5's ~~resource adapter capability contract~~ (now delivered — see `P5-C`'s row: `workspace_release_adapters.py`'s provider-neutral, capability-declaring `WorkspaceResourceAdapter` Protocol (`resolve`/`snapshot`/`validate`/`prepare_release`/`apply_release`/`observe_release`/`rollback_release`) plus its explicit `WorkspaceResourceAdapterRegistry`, which fails closed for any unregistered resource type rather than defaulting to one; `P5-E`'s row for the first real, non-fake adapter (`WorkflowWorkspaceResourceAdapter`) proving the contract against a genuine provider effect. "Supported MVP types" was itself already ratified separately, in `P0-A`, section 19.2's "Which resource types are required in the first Git import?" decision), item 7's ~~deterministic fixtures~~ (now delivered — `tests/workspace_fixtures.py::build_phase0_workspace_fixture` composes every named element (two colliding-logical-name workspaces, a primary+secondary Admin, Developer, QA, Viewer and scope-ineligible member, four fixed environments per workspace, a provider-only prompt, mutable judge/tool rows, a shared no-project catalog tool, a partial `FakeWorkspaceResourceAdapter` provider effect, and a legacy null-`environment_id` audit row) as one reusable, deterministic, offline builder against already-tested route/service surfaces, proven by `tests/test_workspace_fixtures.py`), item 8's ~~two typed decision contracts~~ (now delivered — see `P5-B`'s row: `workspace_release_governance.py`'s `DECISION_QUALITY`/`DECISION_RELEASE` typed decisions, its `_require_distinct_actor` one-actor-separation-axis check, the `CaliberWorkspaceRelease`/`CaliberWorkspaceReleaseOperation` release-versus-operation state split, and a bounded (`_MAX_BREAK_GLASS_TTL`, one hour), interactive-session-only break-glass authorization), item 9's *partially* addressed ~~per-adapter reconstructability strategy~~ (the `prompt` adapter now has a real, load-bearing `resolve()`/`snapshot()` per `P4-C`'s managed-snapshot-slice-1 row — see that row for the full design; every other MVP resource type's adapter, and the `workflow` adapter's own still-placeholder `snapshot()`, remain the open remainder), item 11's ~~`SourceControlProvider` capabilities~~ (now delivered — see `P4-C`'s row for the `SourceControlCapabilities`/`SourceControlProvider` contract itself (`workspace_source_control.py`, 8 named capabilities each independently declarable/unavailable) and its registry; `P4-E`'s row for the actor-link trust model (`workspace_source_events.py`'s revocable, live-query-checked `CaliberWorkspaceSourceActorLink`), the signed webhook/delivery-idempotency inbox, operator-triggered reconciliation (`github_webhook_reconciliation.py`), and fail-closed provider-outage error translation (`github_source_control.py`/`github_http_transport.py`'s `GitHubTransportError`/`GitHubAuthenticationError`) that round it out) | `P0-A` | Contract fixtures execute offline; no unresolved name or state appears in implementation tickets |
| `P1-A` | Data/backend | **Partially delivered, now closed out.** Project slug/source-mode/archive/accepted-revision fields, the minimal `caliber_workspace_environments` table, project-member deactivation fields, and audit-log environment correlation are all added (migration `0093`), with a deterministic additive backfill for every existing project. Workspace creation now requires operator **and** approver (a new `require_all_scopes` primitive) and transactionally seeds the four fixed environments. The environment-class registry gained a genuine `qa` class without touching legacy alias behavior. **PostgreSQL migration CI (follow-up slice after `0106`): delivered.** A dedicated `migration-parity-postgres` CI job (`.github/workflows/ci.yml`, a `pgvector/pgvector:pg16` service container — the stock `postgres` image lacks the pgvector extension migration `0060` requires) runs `tests/test_migrations.py`'s fresh-install + full-upgrade-chain + ORM-metadata-parity test and its `0093` upgrade-from-preceding-revision data-backfill test against a real, ephemeral PostgreSQL server (both tests parametrized by dialect via `CALIBER_TEST_POSTGRES_URL`, set only in that job); mirrored locally by `scripts/ci-local.sh migration-parity-postgres`. Scope was deliberately narrow — the rest of the SQLite-based suite was not ported. Doing this surfaced two real, previously-undetected dialect gaps, both now fixed in the test: SQLite does not enforce foreign keys by default (masking an FK a fixture inserted against a nonexistent parent row) and PostgreSQL reflects a partial-index predicate with explicit casts rather than SQLite's literal source text. **Workspace idempotency ledger: investigated, decided not to build.** Full reasoning is with Phase 1 item 1's own text below, where the migration it would have extended is described | `P0-B` | Fresh/upgrade parity on SQLite and real PostgreSQL; idempotency conflict/replay is durable; new Workspace seeds dev active and qa/staging/prod disabled; migrated live aliases remain `baseline_required` |
...
| `P1-D` | Security/backend | **Delivered (slice 1 of Phase 1's remaining split).** `PROJECT_ACTIONS` closed over section 2.4's full 29-key target vocabulary (kept as a `dict[str, frozenset[str]]`, not a Python `Enum` — `routes/scope_inference.py`'s AST-based inventory only recognizes literal string constants as action values). `resource.write` retired in favor of `resource.write.runtime`/`.evidence`; the 5 existing generic project-file-storage call sites migrated to `.runtime`. `resource.execute` and `feedback.submit` wired onto real routes via a new `require_project_access_if_scoped()` helper (a no-op when the target resource's `project_id` is `None` — workflows/workflow runs/eval datasets/review queues are all optionally project-scoped): `routes/workflow_runs.py` (create/trigger-event/cancel/retry/resume/resume-by-event), `routes/evaluations.py::create_evaluation`, `routes/review_queues.py::submit_item`. Deliberately still reserved at the time, not wired: `resource.write.evidence` onto Prompt/Workflow/Tool/Skill/Test-set/Judge CRUD (section 2.3's isolation-closure warning — Phase 2's job) and `rework.update` (`caliber_rework_tasks` has no `project_id` column yet). `resource.write.evidence` is now wired -- see `P2-P` | `P1-B` | `test_route_scope_inventory.py::test_the_live_vs_reserved_action_partition_is_pinned` names exactly which of the 29 keys are live vs. reserved; a route that starts using any reserved key is instantly covered by an already-correct role set, not an implicit allow |
| `P2-P` | Security/backend | **Delivered.** Closes `P1-D`'s own deferred item: `resource.write.runtime` and `resource.write.evidence` (both closed as action keys since `P1-D`, but wired only onto the 5 generic project-file-storage routes and nowhere else) now gate the real create/update/delete routes for all 6 resource-family modules section 2.3's isolation-closure warning named. `.runtime` (owner/editor): `routes/prompts.py`'s `create_prompt`, `create_prompt_version`, `delete_prompt`; `routes/workflows.py`'s `create_workflow`, `update_workflow`, `delete_workflow`, `import_workflow`; `routes/tools.py`'s `register_tool`, `update_tool`, `archive_tool`; `routes/skills.py`'s `create_skill`, `update_skill`, `import_skill_package`, `import_skill_package_zip`. `.evidence` (owner/editor/reviewer): `routes/eval_datasets.py`'s `create_dataset`, `update_dataset`, `create_example`, `create_example_from_trace`, `supersede_example`, `revise_example`, `restore_dataset_version`; `routes/judges.py`'s `create_judge`, `update_judge` -- a judge is this codebase's "scorer" (its own docstring: "a reusable, operator-authored scorer"), there being no separate `CaliberScorer` model, so this closes the "Scorer" half of the deferred item too; `routes/gateway.py`'s MLflow-guardrail "scorer" templates are a distinct, platform-global concept with no `project_id` column at all and are out of scope (nothing to gate against). Every touched entity model is *optionally* project-scoped (nullable `project_id`, "null = global/personal"), so every call site uses `require_project_access_if_scoped` -- a no-op, not a bypass, when no project is active -- composed with, not replacing, each route's pre-existing global-scope floor (`caliber.operator`/`caliber.admin`, including the routes that are deliberately stricter than section 2.4's baseline: `delete_prompt`, `register_tool`/`update_tool`/`archive_tool`, `update_skill`, `update_dataset`/`supersede_example` all still require `caliber.admin`). A create route (no row exists yet) gates the caller's *active* project (`identity.active_project_id`, the same value persisted onto the new row); an update/delete route gates the *existing* row's own `project_id`. `judges.py::update_judge`'s scope requirement is dynamic (`SCOPE_ADMIN` iff the request body touches `status`, else `SCOPE_OPERATOR`) and unchanged; adding the project-role call flips its `scope_inference.py` classification from `"dynamic"` to `"project_role"` (that inference reports one classification per handler, and checks project-access calls first), so its `_DYNAMIC_SCOPE_NOTES` entry was retired as stale rather than left to linger. `tests/test_p2_resource_write_action_wiring.py` proves, per route touched: a project viewer with the right global scope is refused (403); an admin with no project membership is refused (404, via the visibility-bypass/role-independence pattern `test_p1d_action_registry_wiring.py` established); a correctly-roled member succeeds; the canonical create route per family also proves the unscoped (`project_id is None`) no-op path. **Not yet delivered at the time:** `routes/gateway.py`'s guardrail CRUD (out of scope, no project binding exists; still true); `routes/eval_datasets.py::sync_dataset_to_mlflow` (an MLflow export/sync action, not a create/update/delete of the CALIBER row itself) -- **now closed**, same `resource.write.evidence` action, checked before the MLflow write below the `get_visible` lookup `P2-I` already added (identical ordering `create_prompt` established); proven by `tests/test_p2_resource_write_action_wiring.py::test_sync_dataset_to_mlflow_denies_a_project_viewer`/`::test_sync_dataset_to_mlflow_allows_a_project_editor` | `P1-D` | A Developer (`editor`)/QA (`reviewer`, evidence-only)/Admin (`owner`) member can write; a Viewer member and a non-member holding only the matching global scope cannot, for every route named above; an unscoped resource is unaffected |
...
| `P1-E` | Auth/backend | **Delivered (slice 2 of Phase 1's remaining split).** `caliber_personal_access_tokens` gained a nullable `project_id` (migration `0094`). `CaliberIdentity` gained `credential_kind`/`credential_id`/`credential_project_id`, populated during identity resolution. `POST /auth/tokens` accepts an optional `project_id`, validated against a real project role at issuance (denies binding to a project the caller has no relationship with) and preserved across rotation. `auth.py::resolve_identity` centrally refuses (403) any request where a project-bound PAT's bound project differs from the project named by the `X-CALIBER-Project` header or a `{project_id}` path segment; a request naming no project at all is unaffected. "Require project-bound PATs for CI import" is delivered too, now that `P4-C`'s CI-import route exists to enforce it: `routes/workspace.py::_require_project_bound_pat_for_import` guards `create_import` and `reconcile_import`, refusing (403) a PAT-authenticated caller (`credential_kind == "pat"`) whose `credential_project_id` is not the import's target project -- including an *unbound* PAT, which `resolve_identity`'s generic check alone does not refuse. A session/trusted-header caller is unaffected. Not delivered: the "resource owner"/"persisted worker context" conjuncts section 9.2 also names. Traced to ground rather than left as an open placeholder (this session's audit): the "resource owner" conjunct is not a second, separately-designed gap -- it is the identical unbuilt piece `resource_access.py::authorize()`'s own `resource=` parameter already names and refuses via `NotImplementedError` ("per-resource authorization context is not modeled yet"), since neither has a `ResourceContext` type to check a request's target resource's own `project_id` against. Once a future Phase 4/5 slice builds `ResourceContext` for `authorize()` generally, extending `resolve_identity`'s PAT-binding check to consult it is a small follow-on, not new architecture, and should land in that same slice rather than being re-invented here. The "persisted worker context" conjunct is a distinct, still-fully-unbuilt gap: no durable job/work record persists the initiating request's `credential_kind`/`credential_project_id` for a background worker to re-check once execution happens outside any live request/header -- `db/scoping.py::synthetic_identity`/`build_agent_identity`, the only existing background-job identity primitives, carry no credential fields at all (`scopes=frozenset()` always). This is adjacent to but distinct from `P1-F`'s already-built-but-unwired `actor_provenance.py::ActorProvenance` (a separation-of-duties originator-versus-decision-maker check, not PAT-project-binding persistence) and remains unowned by any row today; it belongs with whichever future Phase 5 slice gives background/worker execution a persisted identity model. Enforcing either conjunct now, ahead of its respective foundation, would only dress up an unbuilt no-op as a real check. **Re-investigated and reaffirmed (this session's final completion audit): deliberately deferred, not silently dropped.** Neither conjunct has a foundation to build against yet (`ResourceContext` doesn't exist; there is no persisted background-job identity model at all), so implementing either now would mean inventing that foundation ad hoc inside an unrelated slice rather than as the deliberate architectural decision it deserves -- the same reasoning `P1-A`'s row already applied to the Workspace idempotency ledger ("investigated, decided not to build" is itself a valid closure, not an evasion, when the alternative is fabricating a placeholder foundation). Revisit only when a real Phase 4/5 slice needs `ResourceContext` or persisted worker identity for its own reasons | `P1-C` | A project-bound PAT is refused for a different project via path (delivered); via header (delivered); an unbound PAT and a request naming no project are both unaffected on non-import routes (delivered); issuance requires a real project role, not just project existence (delivered); a project-bound PAT imports into its own project (delivered); is refused importing into a different project (delivered); an unbound PAT is refused for import specifically (delivered); a session-authenticated import is unaffected (delivered) |
...
| `P2-A` | Backend | **Partially delivered (slice 1, plus slices 9-14).** Item 1: closed 13 bare-child-lookup gaps in `routes/skills.py` and 4 in `routes/prompts.py` (siblings of `routes/tools.py`'s already-correct pattern), plus 2 in `routes/workflows.py`'s session-memory routes -- all now resolve through `get_visible`/`apply_visibility_filter`, the same primitives already proven elsewhere, not new authorization machinery. Item 6: `assistant/plans.py::PlanService.list_plans` now applies the same `apply_visibility_filter` `GET /aria/plans/{id}` already used, closing the one gap found where list and detail disagreed (a project-shared plan was invisible to a teammate's list). Item 9: `db/legacy_data_report.py` -- `legacy_null_report()`/`duplicate_name_report()`, read-only evidence for items 2/3's later decisions. **Slice 9:** JSON and ZIP skill-package imports now derive owner/project/visibility from the authenticated identity, so an active workspace produces a project-owned row while no-project imports retain the explicit personal-library path; package-supplied owner metadata is ignored. **Slice 10 (item 2's remaining root-creation paths, plus item 3's nullability/uniqueness audit):** re-verified the exact route list rather than reusing `P1-D`/`P2-P`'s write-authorization list (scoped to writes generally, not creation specifically) — inventoried every root-creation path for all 15 `db/resource_inventory.py::SCOPING_VISIBILITY` models (prompts' hidden target, workflows, tools, skills, eval datasets/"test sets" — confirmed the same family, `P1-D`'s own row already calls `eval_datasets.py` "Test-set" — judges, knowledge bases, MCP servers, Aria plans, OpenAPI integrations, release candidates, LLM pricing, workflow benchmark reports, eval runs, review queues) and read every route's actual code. Every one already derives `project_id`/`owner`/`visibility` from `identity.active_project_id`/the authenticated actor and ignores any caller-supplied owner override — item 2's "other root-creation paths" were, in fact, already closed by `P2-B` (slice 5, the Aria capability surface) and `P2-C` (the assistant publisher context) before this row's text was last updated to still say otherwise. No production code changed. New route-level tests (`tests/test_root_creation_scoping.py`) close the *test-coverage* gap the audit found instead: `register_tool`, `create_dataset`, `create_judge`, and `create_prompt` had no route-level test proving the invariant directly, unlike skills (`test_routes_scoping.py`) or MCP servers (`test_mcp_servers_routes.py`); `create_workflow`'s project-derivation half was untested too (only the owner-ignored half was, via `test_routes_workflows.py`). The nullability/uniqueness audit (the same underlying item as `P0-B`'s row's "item 3's remaining nullability/uniqueness audit," not a separate one — see that row) is closed for its nullability half: every one of the 15 models' `project_id` column is nullable (personal/global path legitimate) and every owner/`created_by` column is `NOT NULL` (no orphan-owner rows possible) — a clean bill of health, no bug found. The existing global (not per-project) name-uniqueness constraints (`uq_skill_name`, `uq_judge_name`, `uq_eval_dataset_name`, `uq_review_queue_name`, `uq_mcp_server_name`, `uq_tool_name_version`) are deliberate, already-documented design (a shared fleet-wide handle, the same precedent `P2-G` ratified for MLflow's own global prompt namespace) confirmed by existing code comments and tests, not a bug — no global-vs-per-project mismatch found. **Slice 11 (the missing-DB-constraint gap, closed):** `caliber_workflows`, `caliber_knowledge_bases`, `caliber_workflow_benchmark_reports`, and `caliber_openapi_integrations` previously had no database-level unique constraint on `name` at all — a pre-existing TOCTOU-race gap between each route's app-level "select, then insert if absent" check and the actual insert (also matching `P0-B`'s "bare-name-resolver inventory" half — see that row). Migration `0110` adds a DB-level backstop for each table, preserving whatever scope its existing app-level check already enforced rather than changing uniqueness semantics: `caliber_workflows` (`uq_workflow_name`) and `caliber_openapi_integrations` (`uq_openapi_integration_name`) are global, matching `caliber_workflows`'s own pre-existing app-level check and the same "shared fleet-wide handle" convention as `uq_skill_name` et al. (`caliber_openapi_integrations` had no app-level check to preserve, so it follows that same family by structural analogy — a plain `project_visibility` composite index, not the `owner_status`/`project_status` pair below); `caliber_knowledge_bases` and `caliber_workflow_benchmark_reports` are scoped per (project-or-no-project, owner, name) via two dialect-conditional partial indexes each (`uq_knowledge_base_project_owner_name`/`uq_knowledge_base_owner_name_no_project`, `uq_wf_benchmark_report_project_owner_name`/`uq_wf_benchmark_report_owner_name_no_project`), matching `caliber_knowledge_bases`'s pre-existing `_assert_unique_name` check exactly (`caliber_workflow_benchmark_reports` had no app-level check either, but shares that table's exact index-pair convention). The migration's defensive preflight follows `0079_unique_open_incident.py`'s established prior art (a bounded `GROUP BY ... HAVING COUNT(*) > 1` query that aborts with a readable preview rather than guessing a winner); no real production deployment exists yet, so in practice it found nothing. All four routes' app-level pre-checks now also catch a raced `IntegrityError` and translate it into the same friendly conflict message, via `session.begin_nested()`, mirroring `workspace_source_events.py`/`workspace_source_connections.py`'s already-established pattern. **Slice 12 (the "convert root routes by resource family to centralized authorization" remainder, largely closed):** investigated which resource-family root routes still perform bespoke or absent authorization instead of the shared `require_project_access_if_scoped`/`require_project_access` helpers `P1-D` introduced (`resource.write.runtime`/`.evidence`/`.execute`), already wired onto prompts/workflows/tools/skills/eval-datasets/judges/workflow-runs/evaluations/review-queues per `P1-D`'s and `P2-E`'s own rows. Found and closed four genuinely open gaps of the identical shape -- a project *viewer* (visible via membership, not by role) could still write where only editor+ should: `routes/knowledge_bases.py`'s `create_knowledge_base`/`update_knowledge_base`/`delete_knowledge_base` (zero project-role checks anywhere in `knowledge/service.py`); `routes/openapi_integrations.py`'s `create_openapi_integration`/`update_openapi_integration`/`archive_openapi_integration` (this same file's own `publish_openapi_tool_draft` sibling already called `require_project_access(..., "resource.publish")`, so the gap was a provable inconsistency within one file); `routes/llm_pricing.py`'s `create_pricing` and `update_pricing` (the latter also swapped a bare `session.get` for `get_visible`, closing a visibility gap too -- `routes/skills.py::update_skill`, itself `SCOPE_ADMIN`-only, already established that an admin-gated mutation on a project-scoped resource still needs its own role check, since `caliber.admin` is not an implicit project role per `resource_access.py::project_role`'s documented `P1-B` removal); and `routes/aria_plans.py`'s `update_plan`/`approve_plan` (siblings of `execute_plan`/`poll_plan`, which `P2-E` already gated on `resource.execute` -- this closes the same gap on the two sibling routes `P2-E` didn't touch, so a viewer can no longer relax a teammate's plan autonomy, cancel it, or approve it for execution). All four reuse the exact existing `PROJECT_ACTIONS` entries -- no new authorization machinery. **Slice 13 (the `routes/mcp_servers.py` admin-only surface slice 12 named concretely and deliberately deferred, now closed):** confirmed `CaliberMcpServer` is in fact a `db/resource_inventory.py::SCOPING_VISIBILITY` model (nullable `project_id` + non-null `visibility` + `owner`, per `P2-O`'s own migration `0096`), and that its list/detail/history routes already resolve through `apply_visibility_filter`/`get_visible` (`P2-O`'s "MCP visibility slice") -- but every one of `create_mcp_server`/`update_mcp_server`/`delete_mcp_server`/`test_connection`/`discover_tools`/`invoke_tool`/`update_tool_policy` checked only the global `caliber.admin` scope, with no project-role check on the resource's own project at all, the identical `update_skill`/`update_pricing` shape: a project viewer (visible via membership, not by role) holding `caliber.admin` could still create/update/delete a server, or trigger a live test-connection/discover-tools/invoke-tool/tool-policy action against one, in any project they merely belong to. `create_mcp_server`/`update_mcp_server`/`delete_mcp_server`/`update_tool_policy` (persisted-config writes on the registry row) now use `resource.write.runtime`, the same action `update_pricing`/`update_knowledge_base` already use; `test_connection`/`discover_tools`/`invoke_tool` (each reaches out to the server's live configured transport) now use `resource.execute` instead, the same "operate on a live resource" shape already covering workflow runs/evaluations/Aria plan execution. Two existing route-level tests (`test_mcp_servers_routes.py::test_mcp_server_create_uses_authenticated_project_context` and `::test_deleted_mcp_history_checks_snapshot_visibility`) exercised a project id/row with no backing `CaliberProject` row, which the new create-time/delete-time check now 404s as "project not found" -- fixed by seeding a real project (the same fix already applied to `test_zip_import_persists_authenticated_project_context` for `import_skill_package_zip`), not by weakening the check. **Slice 14 (the KB/OpenAPI child-mutation-route remainder slice 12 named concretely, closed):** slice 12 explicitly deferred each touched family's own child-mutation routes (operations that act on an *existing* resource, as opposed to that resource's own root create/update/delete) as visibility-only. Closed all of them, plus one more of the identical shape slice 12's audit didn't separately name: `routes/knowledge_bases.py`'s `create_version`/`activate_version`/`rollback_version`/`sync_version_to_age`/`calibrate_knowledge_base`/`set_knowledge_base_baseline`, and `routes/openapi_integrations.py`'s `import_openapi_version`/`reimport_openapi_version`/`review_openapi_dependency`/`generate_openapi_tool_drafts`/`update_openapi_tool_draft`/`preview_openapi_tool_draft` now all call `require_project_access_if_scoped` before delegating to the service (or, for `activate_version`/`rollback_version`/`sync_version_to_age`, before the KB-visibility-only lookup the route previously stopped at), gated on the resource's own project rather than the caller's active one. `resource.write.runtime` for every route that only persists/edits a row (mirroring the root routes' own family-wide choice); `resource.execute` for the two that fire a genuine execution side effect matching `workflow_runs.py`/`evaluations.py`'s existing precedent for that action -- `calibrate_knowledge_base`'s real retrieve+judge loop, and `preview_openapi_tool_draft`'s live upstream HTTP call (its own docstring already called this "a live effect, not a simulation"). All reuse the exact existing `PROJECT_ACTIONS` entries -- no new authorization machinery, and no PR394-established root-route pattern deviated from. `reimport_openapi_version`/`review_openapi_dependency` dispatch their real work to a `run_in_threadpool`-offloaded module-level helper; the check had to land in the handler itself, not the helper, for `scope_inference.py`'s single-function AST walk (and therefore the generated REST API reference) to see it -- the same reason `import_skill_package_zip`'s check placement was documented, back at the `257` ratchet bump. `test_async_offload_ratchet.py::_BASELINE` bumped 261 -> 263 for the two new inline sessions this required; every other touched route added its check to a session block (or handler body) already counted. `routes/mcp_servers.py`'s own `save_mcp_tool_test_cases`/`calibrate_mcp_tool` routes (the `.../tools/{tool}/test-cases` and `.../calibrate` sub-resource routes), named concretely and deliberately left open at slice 13's own time (`SCOPE_OPERATOR`-gated tool sub-resources nested two levels under the server root, the same "child-mutation routes ... deliberately scoped out to keep this slice to 'root routes'" precedent this slice itself applied to the KB/OpenAPI families it closed) -- **now closed**: `save_mcp_tool_test_cases` persists governance config on the server row (`resource.write.runtime`, same action `update_tool_policy` uses); `calibrate_mcp_tool` fires the same live-invocation loop `invoke_tool` uses (`resource.execute`, matching `calibrate_knowledge_base`'s own classification), each gated on the server's own `project_id`. Proven by `tests/test_p2_resource_write_action_wiring.py::test_save_mcp_tool_test_cases_denies_a_project_viewer`/`::test_save_mcp_tool_test_cases_allows_a_project_editor`/`::test_calibrate_mcp_tool_denies_a_project_viewer`/`::test_calibrate_mcp_tool_allows_a_project_editor`; the REST API reference regenerated for both routes' classification flipping from `caliber.operator` to `project role`. And the separate "full repo-wide bare-lookup sweep beyond the specific gaps these slices found" (item 1's own distinct sub-clause), which was the `P2-H`-through-`P2-O` lineage's job, not this one's -- **now complete, see `P2-Q`**. | `P1-C` | Each of the closed gaps has a route-level test proving a non-member (with real operator scope, not just a viewer) is refused; `list_plans`/`get_plan` now agree; the legacy-null/duplicate-name report is tested against a seeded fixture DB; skill package imports prove project and personal ownership contexts; `tests/test_root_creation_scoping.py` proves, per family without prior direct route-level coverage (tools/eval-datasets/judges/prompts, plus workflow's project-derivation half), that an active project produces a project-scoped row, no active project produces the explicit personal-library row, and a caller-supplied owner (where the schema even accepts one) is discarded in favor of the authenticated actor; slice 11 adds, per table, a test proving the DB-level constraint rejects a duplicate insert that bypasses the app-level check entirely (a monkeypatched pre-check plus a directly-inserted conflicting row, the same race-construction PR #380 used) and a test proving the ordinary duplicate-name flow still produces its existing friendly error; slice 12 adds, per route, the same deny(viewer)/allow(editor) pair `P1-D`'s own wiring tests use (plus the admin-with-no-membership/unscoped-no-op cases on each family's primary route), extending `tests/test_p2_resource_write_action_wiring.py` for knowledge-bases/OpenAPI-integrations/LLM-pricing and `tests/test_aria_plans.py` (alongside `P2-E`'s existing execute/poll proof) for the plan routes; slice 13 extends the same file further for all seven `routes/mcp_servers.py` routes (deny-viewer/allow-editor pairs, plus the admin-with-no-membership/unscoped-no-op cases on `create_mcp_server`), and fixes the two pre-existing `test_mcp_servers_routes.py` tests the new create-time/delete-time check would otherwise have broken by seeding the real `CaliberProject` row they were missing; slice 14 adds the same deny(viewer)/allow(editor) pair for each of the twelve newly closed routes in a new `tests/test_p2a_kb_openapi_child_route_wiring.py` (no admin-with-no-membership/unscoped-no-op cases -- both role sets and the no-op-when-unscoped shape are already fully proven by `P1-D`'s/slice 12's/`P2-E`'s own wiring tests, so only the check's *presence* at each new call site needed proving), reusing this suite's established moto/dummy-embedder/fake-judge/loopback-spec-server recipes for the allow cases that need the service's real downstream logic to succeed |
| `P2-B` | Runtime | **Partially delivered (slice 5).** Slice 1: `workflows/deployment_bundle.py::build_deployment_bundle`'s three remaining bare-name/id lookups (eval dataset, skill, knowledge base) and `workflows/promoter.py`'s two siblings in the same compile/promote pipeline now resolve through `apply_visibility_filter`/`get_visible`, keyed off a new `db/scoping.py::synthetic_identity(owner, project_id)`. Slice 2: `workflows/promoter.py::resolver_from_session` -- the tool-registry-to-resolver builder used everywhere a workflow references a tool -- queried every tool globally with no visibility filter; now scoped at all 12 call sites, and `routes/workflows.py`'s independently-reinvented (and subtly incomplete) `_scoped_tool_resolver` is retired in favor of it. Slice 3 ("run queue + plan executor", the highest-severity remainder): the subworkflow branches of `build_deployment_bundle` and `build_plan`'s `_resolve_version` (a manifest naming another project's workflow could get pinned at compile time and **executed at run time**), `build_plan`'s `_enqueue_review`/`add_review_items_records` (the `review_queue_enqueue` runtime node could write into another project's queue), `_dataset_inputs`/`_resolve_active_dataset` (the same unscoped dataset-by-name gate `evaluate_deploy_gates` already had fixed), and the Aria plan worker (`poll_waiting_plans` resumed every parked plan as `@system`/no-project regardless of the plan's own recorded owner/project) are all closed. Slice 4 (the refinement-worker pipeline, `orchestrator/candidate.py`/`evidence.py`/`triage.py`/`artifact_store.py`): every skill-by-name lookup and agent enumeration across the diagnosis → triage → candidate → eval stages had the identical unscoped-query gap, now closed via a new `db/scoping.py::build_agent_identity(session, agent_id)` (the `agent_id`-keyed sibling of `build_workflow_identity`) or `synthetic_identity(agent.owner, agent.project_id)` when the agent row is already in hand; `ArtifactStore.get_active_skill` (the protocol both the production MLflow-backed store and the test fake implement) gained a required `identity` parameter since its skill-content read was the same gap one layer down. Slice 5 (`assistant/agent_tools.py` -- Aria's per-turn agentic tool surface): 13 call sites across the read tools (`list_skills`/`get_skill`/`list_tools`/`list_workflows`/`get_workflow_manifest`/`list_workflow_runs`/`get_workflow_run`/`get_workflow_run_trace`) and the mutate tools (`run_quick_eval`'s dataset read, `create_eval_dataset`'s creation, `run_workflow`'s version/workflow resolution) queried globally with no visibility check, unlike this same file's own `_load_published_openapi_tools`, which already scoped correctly via `self._capability_context().identity()`. All 13 now reuse that identical, already-proven per-turn identity instead of a second pattern; `_kb_identity` (used by every KB tool) built its own hand-rolled identity with no `active_project_id` *and* a hardcoded scope set regardless of the real caller -- replaced with the same `_capability_context().identity()` call, which also happens to fix the scopes to the caller's real ones (`scopes_for_user`) rather than an unconditional operator+viewer grant. `create_eval_dataset` additionally now sets `project_id`/`visibility` on the row it creates (the bare default was permanently unresolvable by any later lookup) -- but its *duplicate-name* probe was deliberately left global, since `CaliberEvalDataset.name` carries a real database `UniqueConstraint` (a globally unique handle, not a per-project namespace, confirmed by a test that hit a raw `IntegrityError` before the probe was reverted to unscoped). **Nothing remains open on this row.** It previously named two "still open" items, both now stale: "prompt-provider lookup has no CALIBER-side prompt resource to scope" -- resolved by `P2-G`'s now-delivered namespace-collision refusal (a prompt name is exclusively owned by whichever project's hidden target claims it first, closing the scoping ambiguity a provider-side lookup would otherwise hit) -- and "MCP bindings remain structurally blocked on a schema migration," which is simply no longer true: `P2-O`'s migration `0096` added `project_id`/`visibility` to `CaliberMcpServer`, and `build_deployment_bundle`'s `McpResourceNode`/`McpToolBinding` branches already resolve `CaliberMcpServer` rows through `get_visible` (verified directly in `workflows/deployment_bundle.py`), the identical pattern every other resource family in this row uses. Nobody updated this row's own trailer when `P2-O` shipped. | `P2-A` | Two new tests (slice 1) prove a same-named knowledge base in a different project is hidden from bundle resolution; tests (slice 3) prove a same-named child workflow in a different project is hidden from both compile-time bundling and an actual run-worker execution, `add_review_items_records` refuses a queue in a different project even when called directly, and a plan resumed by `poll_waiting_plans` runs its next step as the plan's own owner/project; a test (slice 4) proves a same-named skill in a different project is hidden from the candidate stage's metadata lookup even though the (separately-scoped) artifact-store content read still resolves; new tests (slice 5) prove `get_skill`/`list_skills` hide a same-named skill in a different project, `run_workflow` refuses to enqueue a run against a workflow in a different project, and a created eval dataset stays readable through the same visibility check other lookups use; full backend suite green with coverage comfortably above the repo's 90% floor after all five slices |
| `P2-C` | Integrations/storage | **Partially delivered (assistant publisher context slice, plus a storage/file isolation audit).** Route-backed assistant publishing now threads the authenticated `CaliberIdentity` into tool, skill, workflow, and MCP-server creation, persisting the active project and the same no-project visibility defaults as their REST routes; prompt publishing preflights the CALIBER-side hidden prompt target before the MLflow write and provisions a new target with the caller's project context. Direct publisher callers retain compatibility behavior. **Storage/file isolation is now verified and closed for this row's own acceptance criterion.** `storage/service.py::WorkingDirectoryService.resolve_scoped_file` is the single project/tenant-scoping chokepoint (raises `StoragePermissionError` on a `tenant_id`/`project_id` mismatch); every entry point that resolves a caller-supplied `file_id`/`file_ref` was traced and confirmed to already route through it or an equivalent inline ownership check: `routes/files.py` (`_file_for_run_or_404`, `_pg_file_or_404` -- both already IDOR-guarded against a guessed run/file id per their own docstrings), `routes/projects.py` (`_file_for_project_or_404`), `routes/workspace.py` (the import-reconcile snapshot lookup), and the run-time managed-file resolver (`workflows/file_tools.py::ScopedManagedFileResolver`, which every actual byte read goes through, re-verifying digest/size against the pinned snapshot on top of the scope check). One real gap was found and fixed: `workflows/deployment_bundle.py::build_deployment_bundle`'s `FileInputNode` dependency-inventory branch used a bare `session.get(CaliberWorkflowFile, ref.file_id)` with no project scoping at all -- unlike the `SubworkflowNode`/`McpResourceNode` branches beside it (already closed under `P2-B` slice 1/item 5) and unlike `routes/workflows.py::_preflight_managed_file_node`'s already-correct check for the same node type on the import-preflight path. A manifest pinning another project's real file_id/file_ref/sha256/size (a guessed or leaked ref) was reported `"resolved"` during deploy preflight; the run-time resolver's own scope check meant this was never a content leak, but it was a false-positive resolution signal and a cross-project file-existence oracle. Fixed to mirror the same project-scope check the sibling branches and the import-preflight path already use. **"Immutable pin/reconstructability semantics" investigated and mostly closed; this is a distinct, narrower gap from the `P4-B`/`P4-C` managed-snapshot epic, not the same one under a different name.** `P4-B`/`P4-C` (PR #393) is `WorkspaceRevision`/`WorkspaceSource` resource-adapter `resolve()`/`snapshot()` machinery for CALIBER-managed workspace imports (prompts today); it has nothing to do with `CaliberWorkflowFile`/`FileInputNode`, and no future slice of it was found to subsume this row's gap -- the cross-reference some earlier rows needed (a genuine stale reference to the wrong epic) does not apply here. For a *file* reference specifically, "reconstructability" splits into two independent questions, and only one of them was still open: (1) can the pinned bytes silently change under a fixed ref? No -- already closed. `storage/service.py::WorkingDirectoryService.register_project_file` (what `routes/projects.py::upload_project_file` calls, and the only path that produces the `projects/...` refs a `FileInputNode.file_ref` is required to use, per both `build_deployment_bundle`'s and `routes/workflows.py::_preflight_managed_file_node`'s own `parsed_ref.resource_type == "projects"` checks) is already content-addressed: identical bytes re-uploaded to the same logical path are deduped, differing bytes get a content-addressed sibling path instead of overwriting the original object, and the physical object key itself is keyed by the content digest -- a concurrent upload can race on the DB ref but can never overwrite the bytes a pinned reference already points at. `ScopedManagedFileResolver._verified_bytes` (runtime) and `_preflight_managed_file_node` (import preflight) both then re-verify the actual object bytes' sha256/size against the pinned snapshot on every read, so even a hypothetical metadata/bytes mismatch fails closed rather than serving substituted content. (2) can the pinned file simply cease to exist/resolve later? Yes -- this was the real, still-open half. `routes/projects.py::delete_project_file` let any project member with `resource.write.runtime` soft-delete a project file with **no check at all** for whether an active deployment's `file_input` node still pins it -- an ordinary file cleanup could silently orphan a live deployment's managed-file dependency, discovered only at the next preflight/run as a newly-`"unresolved"` dependency, with no warning at delete time. Fixed by mirroring the exact existing precedent for this same failure mode on other resource kinds -- `routes/tools.py::_referencing_deployments` (tool archive) and `routes/mcp_servers.py::_deployments_referencing_server` (MCP server delete) -- with a new `routes/projects.py::_deployments_referencing_project_file`: `delete_project_file` now refuses (409) a delete that would orphan an *active* deployment's pinned `file_input` node, naming the blocking `{alias}@{workflow_id}`. Scoped identically to the `tools.py` precedent (root manifest only, `status == "active"` only) rather than `mcp_servers.py`'s more thorough rollback-checkpoint/subworkflow-transitive walk -- extending this check to rollback checkpoints and transitive subworkflow file references is a reasonable, precisely-scoped follow-up (matching the pre-existing inconsistency between those two precedents already in the codebase), not something this slice invented or needs to close. (This row previously also named "the provider-side prompt namespace decision" here -- resolved by `P2-G`'s now-delivered namespace-collision refusal.) | `P2-A`, `P2-B` | A project-scoped assistant publish creates visible, project-bound registry resources; a prompt name owned by another project is refused before the provider write; storage/file isolation is proven by new regression tests exercising the guessed-ref-across-projects attack against `build_deployment_bundle`'s managed-file dependency check (own-project resolves; a foreign project's real file_id/file_ref/sha256/size, and a malformed-but-`caliber://`-prefixed ref, both fail closed to `"unresolved"`), alongside the pre-existing cross-run/cross-project coverage in `test_storage_service.py`/`test_storage_hardening.py` for the shared `resolve_scoped_file` chokepoint; the reconstructability audit is proven by new `test_projects.py` regression tests: deleting a project file an active deployment's `file_input` node pins is refused (409, naming the deployment) and leaves the file undeleted/visible, while deleting one pinned only by an inactive (e.g. `rolled_back`) deployment, or pinned by a *different* file_id in an otherwise-matching manifest, still succeeds |
| `P2-D` | SDK/CLI | **Delivered (partial).** `resources/projects.py`'s 18 `/projects/{project_id}/...` call sites now pin `X-CALIBER-Project` to the path's own id via a new tri-state `project` parameter on `Transport.request()`/`download()` (sync and async); a mismatched explicit override vs. a manual header raises `CaliberConfigError` before I/O. `caliberctl`'s 4 project-scoped writes with no other way to name a project (`workflow run`, `cookbook install`, `service publish`, `gate-verdict record`) now refuse without `--project`/`$CALIBER_PROJECT` instead of silently writing an unscoped row. The server now rejects a disagreement between `X-CALIBER-Project` and a `{project_id}` path segment before endpoint/database work, returning a `400` error whose detail starts with the stable `workspace_context_mismatch` reason. **Async project parity is now delivered:** `aio/projects.py` exposes `AsyncProjectsAPI`/`AsyncProjectFilesAPI` with the complete current sync project/file surface, shared models, explicit path scope pins, multipart upload and raw-byte download; parity and wire-contract tests cover every method. The ambient scope is `ContextVar`-backed and task/thread-local. **This row's own "not yet delivered" note ("the broader async workspace tree for future source/import/revision/release resources") is now moot, not open:** `P6-C` (delivered) documents a deliberate, considered policy that this SDK does not mirror every typed resource module into async -- only resources with a `wait()`-shaped long-running-poll operation (imports, releases, release-operations) earn async coverage, and the remaining source/revision/change-request/version-tag/member/rework-task resources stay sync-only by that same stated design, not by omission. Nothing further is planned or owed here. | -- | A `PRJ-A`-scoped client asking about `PRJ-B` by id sends `PRJ-B`; the pin does not leak into the next call; sync and async project APIs expose the same current signatures and route semantics; multipart uploads and raw-byte downloads remain non-JSON; a workspace-bound CLI write with no resolved project is refused with `exits.USAGE`, not silently unscoped; contradictory server header/path context is refused with `400 workspace_context_mismatch` |
| `P2-E` | Backend | **Delivered.** Item 7 (route Aria capabilities and durable plan execution through the central authorization contract): `routes/aria_plans.py::execute_plan`/`poll_plan` checked resource *visibility* only -- any active project member (including a plain `viewer`) could execute or poll a teammate's project-scoped plan. Both routes now also call `require_project_access` (via a shared `_authorize_plan_execution` helper) against the plan's own `project_id` with the `resource.execute` action, the same role floor (owner/editor/reviewer) `create_workflow_run` already enforces; a personal (`project_id is None`) plan is unaffected. Separately, `assistant/agent_tools.py::_dispatch_capability` (the synchronous per-turn tool-call path) never consulted a capability's declared `required_scopes` at all -- only the async plan executor did; now it consults the same `scopes_for_user` primitive. The residual "first-class scope/role decision recorded on the plan/step rows rather than re-derived per call" is now closed too, mirroring `CaliberWorkspaceReleaseDecision.actor_role_snapshot`/`.effective_scope_snapshot`: migration `0107` adds `CaliberAriaPlan.actor_role_snapshot`/`.effective_scope_snapshot`/`.authorization_recorded_at` (recorded once, the first time `_authorize_plan_execution` clears the plan to run -- `None` role for a personal plan) and `CaliberAriaPlanStep.capability_scope_decision` (recorded by `PlanExecutor` at both places it already checks a capability's `required_scopes` against the plan owner: `_plan_next_action`'s early-admission check and `_run_step`'s own re-check immediately before dispatch). Neither snapshot replaces the live per-call check -- both routes and the executor keep re-deriving authorization on every call, since a still-active plan's authorization can genuinely change between calls (a role edit, a membership change); the snapshot is purely an auditable "what authorized this to run" record, the same judgment `CaliberWorkspaceReleaseDecision` applies alongside its own fresh CAS checks. `_dispatch_capability` deliberately gets no snapshot: `AgentToolset` carries a session_id and an acting identity, not a plan_id, and this synchronous per-turn path can run with no `CaliberAriaPlan`/`CaliberAriaPlanStep` row in existence at all -- there is no durable row to attach one to, so that check stays live-only, which is a boundary, not a gap. | `P1-D` | A project member with only the `viewer` role is refused (403) on `execute`/`poll` for a project-scoped plan they can see but not act on; a member with `editor`/`owner`/`reviewer` succeeds and the plan row's `actor_role_snapshot`/`effective_scope_snapshot`/`authorization_recorded_at` are populated exactly once (a later `execute`/`poll` call does not overwrite them); a refused caller leaves the snapshot unset; a capability requiring a scope the caller lacks is refused by `_dispatch_capability`, not silently run; a dispatched plan step's `capability_scope_decision` records the exact `required_scopes`/`checked_scopes`/`satisfied` outcome the executor checked, on both the success and failure path |
| `P2-F` | Backend | **Partially delivered (indexes + orphan audit).** Item 3: migration `0095` (additive, no column changes, no backfill) adds a `(project_id, visibility)` composite index to the 11 "visibility"-shaped tables (per `db/resource_inventory.py`'s classification) that had no index covering `project_id` at all, and a plain `(project_id,)` index to the 4 "project_only" run/file tables in the same position (`caliber_workflow_runs`/`_run_events`/`_run_checkpoints`/`_files`); 3 visibility tables that already carried a `(project_id, status)` composite are left untouched. `db/legacy_data_report.py::orphan_project_id_report()` now provides read-only, inventory-derived grouped counts for every non-null project id that has no matching `CaliberProject`, including deleted rows, in stable model/id order. **Not yet delivered:** the FKs half; applying them remains a separate migration decision after the report is run against representative production-like data and every orphan is classified or repaired | `P2-A` | `orphan_project_id_report()` omits null and resolvable references, groups duplicate orphan ids, and covers every project-scoped model; `alembic upgrade head` creates every declared index on every declared table with a `project_id`-leading column list (a real migration run, not just `Base.metadata`); `test_alembic_upgrade_head_matches_metadata` still passes (fresh-vs-migrated schema parity) |
| `P2-G` | Backend | **Delivered.** Item 4: no `CaliberPrompt` model exists (a prompt is purely an MLflow Prompt Registry entity, flat and globally named) -- the only CALIBER-side data is a hidden runtime target (`CaliberAgentConfig` row keyed by `agent_id == prompt_name`) that carries `project_id`/`visibility`. `prompt_targets.py::ensure_prompt_target` takes an optional `identity` and refuses (404) to reuse/mutate an existing target invisible to the caller; `get_prompt`/`list_prompts` both hide a name whose target belongs to another project while a bare provider-only/legacy name (no target row) stays visible to everyone. **The MLflow-side namespace scheme this row named as undelivered is now decided and implemented: refuse the second registration.** `routes/prompts.py::create_prompt` already refused (404) before its MLflow write when an existing target is *invisible* to the caller; it now also compares that target's `project_id` directly against the caller's active project and refuses with `409` whenever they genuinely differ, even when the target is *visible* -- closing the one path the visibility check alone could not: `db/scoping.py::apply_visibility_filter` deliberately gives an admin identity an unconditional cross-project visibility bypass (so admins can inspect/manage every project), which would otherwise let an admin's `create_prompt` call sail past project A's ownership and silently add a version onto project A's own MLflow entity while acting for project B -- exactly "two projects land in one interleaved MLflow entity". A target with no `project_id` at all (a personal/"My Library" target, or a bare provider-only/legacy MLflow entity with no CALIBER row and so no project owner to compare against) is deliberately *not* treated as a collision -- the same carve-out `create_prompt_version`'s own `resource.write.runtime` check already documents for this identical shape, consistent with the bare-name visibility precedent above. Checked before the MLflow write, same as the pre-existing check beside it, so a refused attempt never registers an orphaned version | `P2-A` | A stranger cannot take over, read, or list another project's prompt via its hidden target; a bare MLflow-only prompt with no CALIBER row stays visible to everyone; `create_prompt`'s MLflow `register_prompt` is never called when either pre-check refuses; a second, genuinely different project's registration of an already-claimed name is refused (`409`) even from an admin identity that would otherwise bypass the visibility check; the same project re-registering/re-versioning its own name is unaffected |
| `P2-H` | Backend | **Partially delivered (slice 2 of item 1's repo-wide sweep).** A systematic pass over every `session.get(<Model>, ...)` in `routes/*.py` for a visibility/project-scoped model (per `db/resource_inventory.py`), cross-referenced against what `P2-A`/`P2-B`/`P2-E`/`P2-G` already closed. Closed the highest-severity cluster: `routes/evaluations.py::create_evaluation`'s subject/dataset resolution -- `_resolve_subject`'s `skill` branch (mirroring its already-fixed `workflow` branch sibling), `_hydrate_judge_runners`, and the dataset lookup itself (previously only role-checked via `require_project_access_if_scoped`, not visibility-checked, so a `visibility="user"` dataset owned by a project-mate, or a personal dataset owned by someone else, was wrongly admitted) -- all three now go through `get_visible`. **Not yet delivered:** several more genuine gaps of the identical shape, deliberately deferred as separate follow-up slices to keep this one bounded: `routes/eval_datasets.py`'s 6 mutation routes, `routes/prompts.py::enqueue_prompt_optimization_run`'s own dataset lookup, `routes/workflow_calibration.py` (imports neither visibility primitive at all), `routes/rollback.py`, and `routes/skills.py::bind_skill`'s `kind="agent"` branch | `P2-A` | A non-member/hidden-target caller is refused (404) evaluating against a hidden skill subject, a hidden judge scorer, or a `visibility="user"`/personal dataset owned by someone else -- even a real project member with an insufficient role, or one with the right role but no active-project header, is refused where the resource is genuinely invisible to them; a real project editor with the header set still succeeds |
| `P2-I` | Backend | **Partially delivered (slice 3 of item 1's repo-wide sweep).** The first of `P2-H`'s named follow-ups: `routes/eval_datasets.py`'s 6 mutation routes -- `create_example`, `create_example_from_trace`, `revise_example`, `sync_dataset_to_mlflow`, and `restore_dataset_version` all looked their dataset up with a bare `session.get`; `get_dataset`/`list_examples` in the same file were already correct and are the reference pattern reused. `sync_dataset_to_mlflow`'s check runs before its MLflow write (same ordering `create_prompt` established), so a refusal never lets another project's content reach MLflow first. `update_dataset`/`supersede_example` are deliberately untouched (`SCOPE_ADMIN`-only; admin bypasses visibility unconditionally, so wrapping them changes nothing). **Not yet delivered:** the other 4 gaps `P2-H` named (`prompts.py::enqueue_prompt_optimization_run`, `workflow_calibration.py`, `rollback.py`, `skills.py::bind_skill`) | `P2-A` | A non-member operator is refused (404) appending/revising/syncing/restoring a hidden dataset's examples/versions; `sync_dataset_to_mlflow`'s fake MLflow client records zero calls when refused; the dataset's own owner (a personal, `project_id=None` dataset) is unaffected |
| `P2-K` | Backend | **Delivered.** `routes/workflow_calibration.py` imported neither visibility primitive at all. `get_options` and `enqueue_workflow_calibration_run` (the `create_run` helper) both looked their workflow up with a bare `session.get`, and the helper did the same for the target agent. `identity` is optional on the helper: `create_run` and the Aria capability (`_workflow_calibrate`, via `ctx.identity()`) both pass a real one. This row's own trailer went stale the day it was written: `assistant/service.py`'s legacy intent-plan dispatch path was named here as still passing `None` with "no per-turn identity concept yet," but `P2-M` (commit `ff390c39a731`, merged the next day) gave `execute_intent_plan` a real `identity` parameter and threaded it into `_execute_workflow_calibration` -> this same helper -- closing the gap this row described without ever updating this row's own text (the same pattern already found and fixed on `P2-B`'s trailer). The other 2 gaps `P2-H`'s sweep named (`rollback.py`, `skills.py::bind_skill`) were separately closed by `P2-L`. A follow-up audit (this row's current text) added the missing end-to-end proof -- `caliber/tests/test_assistant_intent_plan_visibility.py` exercises the live `POST .../plans/execute` route through `execute_intent_plan`'s real dispatch, not just the private adapter -- and corrected the now-false docstring on `enqueue_workflow_calibration_run` that still claimed the identity gap was open. **Nothing remains open on this row.** | `P2-A` | A non-member is refused reading calibration options for, or queuing a run against, a hidden workflow (404 via the direct REST route; an embedded "failed" result via the intent-plan dispatch route, which wraps all adapter errors in a uniform envelope); a caller with a visible workflow but a hidden target agent is refused ("not registered") before any job is queued, through either route |
...
| `P2-L` | Backend | **Delivered.** `routes/rollback.py` now resolves the agent target through `get_visible` for checkpoint listing and destructive rollback, and scopes its legacy skill-name existence fallback through `apply_visibility_filter`. `routes/skills.py::bind_skill` now resolves a requested agent through `get_visible` before mutating its `optimizer_config`. This row previously carried forward `P2-K`'s "legacy `assistant/service.py` intent-plan dispatch identity gap remains a separate, larger plumbing task" note; that was already stale when written (see `P2-K`'s resolved trailer) -- `P2-M` (commit `ff390c39a731`) closed it the day after `P2-K` was authored. **Nothing remains open on this row.** | `P2-A` | A non-member cannot list or consume rollback checkpoints for a hidden agent, and cannot bind a visible skill to a hidden agent; each is returned as an indistinguishable 404 |
| `P2-M` | Backend/assistant | **Partially delivered (slice 7 of item 1's repo-wide sweep).** Route-backed assistant turns now retain the resolved `CaliberIdentity` through skill selection, capability dispatch, and intent-plan execution. Workflow calibration and prompt optimization adapters pass it to their existing visibility-aware route helpers; default workflow-calibration agent selection is scoped; assistant-created eval datasets retain the active project and visibility tier; and PAT-limited turn scopes are honored by capability dispatch. This is also the slice (commit `ff390c39a731`) that closed the "legacy `assistant/service.py` intent-plan dispatch identity gap" `P2-K`/`P2-L` each separately named as a still-open plumbing task -- both rows' trailers went unedited when this slice shipped, and stayed stale until a later audit reconciled them and added the missing end-to-end route-level proof (`caliber/tests/test_assistant_intent_plan_visibility.py`). The remaining assistant registry/library/review reads are tracked in `P2-N`; MCP bindings remain a schema/resource-context task. | `P2-A`, `P2-K`, `P2-L` | A hidden project skill is not selected; a scope-limited turn cannot dispatch an operator capability; intent-plan calibration refuses a hidden workflow; project-created assistant datasets remain visible through the normal project predicate |
| `P2-N` | Backend/assistant | **Partially delivered (slice 8 of item 1's repo-wide sweep).** The legacy `assistant/tools.py` registry dispatcher now scopes skill/tool list and detail reads through a request-bound identity, and `OpenAIAssistantEngine` derives that binding from the per-turn task context without sharing caller state across requests. Assistant library attachments scope skill/tool/workflow/knowledge-base snapshots; optimization/workflow result readers scope legacy refinement jobs through their visibility-aware agent parent; and promotion proposals refuse invisible agent targets. Direct callers without identity retain compatibility behavior. **Prompt-provider lookup is now closed** (the "no CALIBER-side prompt resource to scope" reasoning here was stale: `P2-G` already gave every prompt name an optional hidden `CaliberAgentConfig` runtime-target row, and `routes/prompts.py::get_prompt`/`create_prompt_version`/`delete_prompt` already gated on it). The gap was narrower than this row implied: two sibling *read* routes -- `list_prompt_versions` and `get_prompt_version` -- queried MLflow's flat, globally-named Prompt Registry directly with no CALIBER-side check at all, so a caller already refused by `GET /prompts/{name}` could still enumerate that project's version history or read the full template content of a specific version by name. Both routes now share `get_prompt`'s check via a new `_prompt_target_invisible` helper before touching MLflow; a bare provider-only/legacy name (no target row) stays visible to everyone, unchanged. MCP visibility is delivered in `P2-O`. | `P2-M` | A non-member cannot enumerate or attach another project's skill/tool/workflow/knowledge base, cannot inspect its refinement result through an assistant plan, and cannot propose promotion against its hidden agent; same-project and legacy direct callers remain functional; a non-member cannot list another project's prompt version history or read a specific version's template via `GET /prompts/{name}/versions[/…]` |
| `P2-O` | Backend/integrations | **Partially delivered (MCP visibility slice).** `CaliberMcpServer` now has nullable `project_id`, non-null `visibility`, and a `(project_id, visibility)` index (migration `0096`). Existing rows migrate to private user visibility without inventing a project binding; new HTTP-created rows take their owner/project from the authenticated identity, while the globally unique server name remains a compatibility handle. List/detail/history/tool operations and workflow import preflight use the shared visibility predicate; deployment bundles, deployment gates, runtime plans, and the run worker resolve MCP servers using the workflow's owner/project context before execution. Deleted-server history fails closed when its audit snapshot lacks visible resource context. | `P2-B`, `P2-N` | A caller cannot list, inspect, bind, invoke, calibrate, or execute another project's MCP server by guessed ID; a project owner/member can use a visible server; legacy rows remain owner-only; credentials remain write-only and absent from responses/audit details. Assistant publisher context is delivered in `P2-C`; provider prompt lookup is closed by `P2-N`'s `list_prompt_versions`/`get_prompt_version` fix |
| `P2-Q` | Backend | **Delivered -- the repo-wide bare-lookup sweep (item 1's own distinct sub-clause, `P2-A`'s row) is now complete.** Performed the sweep `P2-A`'s row said remained the `P2-H`-through-`P2-O` lineage's job: grepped every `session.get(<Model>, ...)` across `routes/*.py`, plus every other module a route handler resolves a resource through, for all 15 `db/resource_inventory.py::SCOPING_VISIBILITY` models, and read each hit's surrounding code rather than pattern-matching alone. Cross-referenced against every gap `P2-H` through `P2-O` (and `P2-A` slice 12) already found and fixed so none was re-flagged. Found and closed one new genuine gap: `routes/workflow_versions.py::_enrich_subworkflow_validation` (reached only through `POST /workflow-versions/{id}/validate`) resolved a `SubworkflowNode`'s target `workflow_id` with a bare `session.get` -- unlike every other place a subworkflow reference is resolved (`workflows/promoter.py::_resolve_version` at publish/run time, `routes/workflows.py::_preflight_subworkflow_node` at deployment-preflight time, both already `get_visible`-gated with their own `P2` item-5 comment). A caller only needs create access to *some* workflow of their own (a personal one is enough -- `create_version` only checks the parent is visible, never the child a manifest names) to embed another project's private `workflow_id` in a `SubworkflowNode` and learn, via this validate endpoint, that it exists plus its active deployment's version id/number/status. Now resolved through `get_visible`, so an invisible target reports the identical `subworkflow_unknown_workflow` code a genuinely missing one does. Every other bare lookup the sweep found on a `SCOPING_VISIBILITY` model was confirmed already-safe, for one of these reasons: (a) already `get_visible`/`apply_visibility_filter`-gated; (b) a pre-insert existence-only uniqueness check on the *new* row's own generated id (`agents.py::register_agent`, `workflows.py::create_workflow`/`create_workflow_benchmark_report` -- returns no data and discloses nothing about an existing row); (c) a role-gated `require_project_access_if_scoped` check keyed on the bare-looked-up row's own `project_id` (`routes/prompts.py`'s hidden-target family -- the already-documented "bare provider-only prompt has no target to hide behind" carve-out); (d) `SCOPE_ADMIN`-only routes, where admin already bypasses visibility unconditionally (`eval_datasets.py`'s `update_dataset`/`supersede_example`); (e) a same-request re-fetch of a row already visibility-checked earlier in that same handler (`evaluations.py`'s workflow re-fetch after its own `get_visible` parent check; `eval_datasets.py::sync_dataset_to_mlflow`'s post-MLflow-write re-fetch); or (f) a route gated by a wholly separate, deliberate auth mechanism instead of caller identity -- `routes/services.py`'s public service-invocation/OpenAPI-spec routes authenticate via a per-service bearer token (`_lock_service_configuration`/`_validate_service_token_in_session`), so the invoked workflow's own visibility is not the applicable gate there, the same way an MLflow-side bare provider prompt has none to apply either. **Noted, but explicitly out of this sweep's scope:** `CaliberVerificationItem` carries no `project_id`/`visibility`/owner column at all (`SCOPING_UNSCOPED` per `resource_inventory()`, not one of the 15 models this sweep covers) -- `routes/verification.py`'s entire queue (list/get/create/verify/dismiss/batch, `P3-B`'s row) has no per-project isolation, by the same design `P3-B`'s own row already described as a flat, non-project-scoped surface. That looks like a pre-existing, deliberate "shared ops triage queue" rather than a regression, but is a genuine future product/security decision for whichever row picks it up -- not silently swept under this one. No repo-wide static/AST regression check (mirroring `routes/scope_inference.py`'s pattern) was added at the time: a reliable one would need to tell every carve-out above apart from a real gap without false-positiving on (b)-(f), which is a meaningfully larger, separate design effort than this slice's fix -- named here as an explicit follow-up candidate, not attempted. **Closed (slice 2):** the product decision this row deferred is resolved -- verification items *are* project-scoped, not an intentionally global cross-project queue, matching every other artifact-adjacent triage surface in the codebase. `CaliberVerificationItem` gained a bare, nullable `project_id` (migration `0113`, `project_only` per `db/resource_inventory.py`, the same shape `CaliberReleaseOperation` uses per `P2-R`) derived from its own `agent_id`'s `CaliberAgentConfig` row -- every item's `agent_id` is `NOT NULL` and FK-constrained, and every one of today's four job-creation paths (`routes/prompts.py`, `routes/skills.py`, `routes/workflow_calibration.py`, `assistant/service.py`'s promotion proposal) plus the manual `POST /verification-queue` route already resolves that agent through an identity-scoped lookup before referencing it, so no job-creation path needed to change its own authorization shape, only supply the one extra field. `routes/verification.py`'s entire list/get/verify/dismiss/duplicate/batch surface now scopes through a hand-derived `_scope_verification_items`/`_scoped_get` pair mirroring `routes/releases.py::_scope_release_operations` (`P2-R`) exactly. The manual create route's own `session.get(CaliberAgentConfig, ...)` lookup -- an adjacent gap this slice's own scoping work would otherwise have made worse, since it let a caller silently inherit another project's `project_id` onto a new item -- now goes through `get_visible` too. **Follow-up closed (slice 3):** `routes/bare_lookup_inventory.py` now builds that checker. It walks every top-level function in every `caliber.routes.*` module (via `ast`, reading each module's own live source, matching `scope_inference.py`'s own established shape) and finds every `session.get(<Model>, ...)` call on one of the 15 `SCOPING_VISIBILITY` models, then auto-classifies each site into one of four safe shapes mirroring (a)-(e) above -- `"gated"` (a same-function `get_visible`/`apply_visibility_filter` call on the same model, and for `get_visible` the identical primary-key expression), `"admin_only"` (a literal `SCOPE_ADMIN`-only route), `"role_gated_own_project"` (a later project-access call keyed on the looked-up row's own `.project_id`), and `"existence_only"` (the result is only ever compared to `None`, never inspected). Deliberately does **not** also attempt the `select(<Model>).where(<Model>.<pk> == ...)` shape: a scan of the live tree found three dozen such call sites, and reading a sample showed the overwhelming majority are already-visibility-filtered list queries or name-uniqueness pre-checks, not bare PK lookups -- telling those apart reliably needs real dataflow analysis, not a quick AST walk, so that shape is documented as explicitly out of scope rather than covered unreliably. Of today's 24 live `session.get` call sites on these 15 models, 19 auto-classify into the four safe shapes and 5 needed a human-reviewed, justified `_REVIEWED_BARE_LOOKUPS` allowlist entry (mirroring `_DYNAMIC_SCOPE_NOTES`/`_PUBLIC_ROUTES`'s justification-required convention): `aria_plans.py::_record_plan_authorization_snapshot` and `workflow_versions.py::_run_workflow_version_sync` (both a same-request re-fetch of a row already visibility-checked through a different function/module this checker deliberately does not chase into, matching `scope_inference.py`'s own "do not chase into helpers" scope), `services.py::invoke_service`/`service_openapi` (category (f)'s per-service bearer-token auth axis), and `skills.py::get_skill_workspace` (a hidden-target id deterministically derived from an already-visibility-checked parent skill's own name, not caller-steerable independently). `tests/test_bare_lookup_inventory.py` is the enforcement gate: it fails by name on any future bare lookup that doesn't auto-classify and has no allowlist entry, plus a stale-entry check on the allowlist itself | `P2-A`, `P2-H`, `P2-N`, `P2-O` | `tests/test_routes_workflow_versions.py::test_validate_subworkflow_hides_target_from_non_member` seeds a hidden workflow (owned by someone else, `visibility="user"`) with a real published version and an active `prod` deployment, references it from a non-member's own draft's `SubworkflowNode`, and proves the validate response reports the same `subworkflow_unknown_workflow` code a nonexistent workflow would -- never the hidden workflow's real deployment/version facts. `tests/test_bare_lookup_inventory.py` adds the regression-checker coverage described above (AST-walker unit tests per classification kind, plus the live-tree enforcement/staleness/ratchet tests) |
| `P2-R` | Backend | **Delivered.** Item 6's own long-standing blocker, closed: `CaliberReleaseOperation` (the legacy prompt-alias release-intent table, distinct from `CaliberWorkspaceReleaseOperation`) had no `project_id` at all, so `GET /caliber/releases/operations` and `POST .../operations/{id}/resolve` were globally unscoped -- any operator could list, retry, or abandon every project's prompt release history. `P2-G` removed the "prompts have no project binding" rationale that used to block this by giving every prompt a hidden, project-scoped `CaliberAgentConfig` runtime target. Migration `0112` adds a bare, nullable `project_id` (no `visibility`/owner column -- `db/resource_inventory.py` now classifies it `project_only`, the same shape `CaliberWorkspaceReleaseOperation` already uses, confirmed by direct look rather than defaulting to the full 3-tier model) and backfills existing rows from each operation's `resource_name` resolved through `caliber_agent_config.agent_id`; a row with no resolvable target (a bare provider-only/legacy prompt) backfills `NULL`, the same "no target = personal/global" carve-out `prompt_targets.py` already applies. `release_operations.py::prepare_prompt_alias_release` now sets `project_id` the same way going forward. `routes/releases.py`'s new `_scope_release_operations` helper hand-derives the equivalent "project" tier (`apply_visibility_filter` itself can't be reused -- it hard-requires a `visibility` plus owner/`created_by` column neither this table has): a `NULL` `project_id` stays visible to everyone, a non-null one only to an active member of that exact project, admins keep the same unconditional bypass `_scope_timeline_rows`/`_visible_ids` already grant elsewhere in that file. Wired onto both `release_operations` (list) and `resolve_release_operation` (retry/abandon by id, re-queried through the filter the same "re-fetch by id" idiom `get_visible` uses, rather than the prior bare `session.get`). Deliberately left unscoped: `reconcile_release_operations`, a global maintenance sweep over every incomplete external effect regardless of project, noted as a documented residual rather than silently narrowed. `tests/test_resource_inventory.py`'s ratchet updated (`unscoped` -1, `project_only` +1) | `P2-G` | A project-scoped prompt release operation is invisible to list and 404s on resolve for a non-member, even with the right global operator scope and the target project set via header with no real membership row; an active member of that same project sees and can resolve it; a projectless (bare-prompt) operation stays visible to everyone; an admin identity is unaffected either way |
...
| `P3-B` | Workflow/quality | **Delivered.** List/get/create/verify/dismiss/duplicate/batch verification-queue routes and CALIBER SDK methods against the existing `CaliberVerificationItem` model and schemas; no new table. **Isolation gap closed (see `P2-Q` slice 2):** this row originally shipped the queue with no per-project isolation at all -- `P2-Q`'s own sweep later found and disclosed that gap rather than silently sweeping it under itself. `P2-Q` slice 2 closed it: a nullable `project_id` (migration `0113`) derived from each item's own `agent_id` target, with every read/action route now scoped by it | `P0-B` | A human can verify or dismiss a pending item they did not create; none of today's four job-creation paths was required to change (and none did); ingestion (a poller creating `pending` items from real signals) remains explicitly out of scope per the Phase 0 decision; a verify/dismiss/duplicate/batch action on another project's item is now refused (`tests/test_routes_verification_queue.py`'s `P2-Q` isolation coverage), which did not hold when this row first shipped |
...
| `P4-A` | Data/backend | **Partially delivered (schema/invariant slice).** Added the dormant source, import-job, revision, and revision-resource tables plus `WSS-`/`WSI-`/`WSR-`/`WSRR-` ID generators; wired `caliber_projects.next_revision_number` and its deferred accepted-revision FK; added the provider/state check constraints, project/content idempotency keys, and the portable read-then-conditional-update allocator (`workspace_revisions.py`) that does not depend on `SELECT FOR UPDATE` or `RETURNING`. Terminal revisions reject ORM mutation, pins cannot be added/changed/removed after terminal validation, and ready revisions retain both source and resource snapshot files. A worker now consumes this schema for `push`-mode imports (see the `P4-B`/`P4-C` rows); no route or feature flag beyond the worker's own `workspace_import_worker_enabled` is added by this slice. **Cross-dialect concurrent allocation: closed.** `tests/test_workspace_revision_allocation_concurrency.py` races 16 real threads, each its own DB session, against `allocate_revision_number` for the same project — no coordination beyond the allocator's own compare-and-set retry loop, the same way `run_in_threadpool` hands each request its own session in production. It proves both that a full-contention run hands out `1..16` with no duplicate or skipped number, and that the bounded-retry failure path (`max_attempts` exhausted) still fails closed under genuine contention rather than the hand-written always-contended fake `test_workspace_revision_foundation.py` used until now. The dialect-parametrization scaffolding was extracted from `test_migrations.py` into shared `tests/dialect_helpers.py` so this case runs unconditionally on SQLite and, like `P1-A`'s own two cases, also against a real ephemeral PostgreSQL server in the `migration-parity-postgres` CI job (confirmed locally via `scripts/ci-local.sh migration-parity-postgres` against a real `pgvector/pgvector:pg16` container) — no dialect-specific bug was found; the CAS design is provably race-free on both. This closes the allocator primitive's own concurrency gap shared by both the import materializer and the managed-snapshot route (`P4-B`/`P4-C`); a full concurrent-HTTP-request test of either route's own `IntegrityError`-replay wrapping (`routes/workspace.py::_create_snapshot_sync`'s documented "a concurrent request racing to create the same content is caught by `begin_nested()`/`IntegrityError`... and replayed") remains a reasonable follow-up, deliberately left out of this schema/invariant slice since it would require production identity/authorization/adapter-registry scaffolding this row does not otherwise touch. | `P2-B`, `P2-C` | Schema metadata and Alembic parity are green; allocator is monotonic and CAS-shaped and now proven race-free under real concurrent sessions on SQLite and PostgreSQL; terminal mutation and ready-snapshot retention guards fail closed; provider/import execution remains dormant until the later phase-4 slices |
| `P4-B` | Import/backend | **Partially delivered (source materialization, import-lease, bounded-retry, and test-journey slices).** Added provider-free ZIP source validation with the ratified 100 MB compressed-bundle, 10,000-entry, 25 MB-file, 1 GiB-total, and 50:1 decompression-ratio limits; normalized repository-relative paths, duplicate-path and symlink rejection; deterministic media-type/executable-aware source-tree digests; and fixed tar snapshots suitable for content-addressed storage. Added an idempotent source/commit observation check that raises `source_commit_digest_conflict` on equivocation while ignoring ZIP ordering/compression/timestamp metadata. Added provider-neutral durable job arbitration: oldest queued imports are atomically claimed, live leases can be renewed only by their owner, terminal writes are lease-fenced, and expired workers move to `reconcile_required` without blind retry. Added durable total-attempt budgets: claims increment `attempt_count`, only jobs below `max_attempts` enter the queue, known `failed` outcomes can be deliberately requeued while budget remains, and exhausted or `reconcile_required` jobs refuse retry. Added model-based Change Request transition tests, an offline manifest-to-revision import lifecycle test, selector failure/cancel tests, and a Playwright workspace-scoping journey. **The import worker itself is now delivered** (`workspace_import_worker.py`/`workspace_import_materializer.py`): an asyncio background worker, started/stopped with app lifespan exactly like `WorkflowRunWorker`/`KnowledgeBaseWorker` and gated by its own `workspace_import_worker_enabled` config flag, claims queued `push`-mode import jobs via the primitives above, reads back the already-validated, already-retained source snapshot `_create_import_sync` stored at submission (no provider/network fetch is involved for `push` mode -- there is no currently-wired adapter contract that returns file content; see the materializer module docstring), recomputes and cross-checks the manifest and every declared resource's content digest against that snapshot, and creates the immutable `WorkspaceRevision`/`WorkspaceRevisionResource` rows before calling `finish_import_job`. Crash-safety: the revision/resource inserts and the job's terminal `succeeded` transition share one session and are committed together by `finish_import_job`'s own commit, so a crash before that point persists nothing (no half-created revision), and `workspace_source.assert_source_commit_digest` makes a second attempt at the same content idempotent (replays the existing revision rather than duplicating). A best-effort, non-blocking call to the already-wired `WorkspaceSourceProviderRegistry.verify()` (`P4-C`) records provider attestation on the revision when a provider happens to be registered, without ever gating success on it (no adapter is registered in `server.py` today). Public API exposure is delivered incrementally in `P4-C`; mutable-resource adapter snapshots remain open (real provider adapters are closed -- see `P4-C`'s row). **`provider_pull` import mode: decided, not building (repo owner's standing scope, applied by analogy to the already-ratified GitLab/Bitbucket precedent).** Section 9.2's own schema comment named `provider_pull` as a later mode from the start ("`import_mode` (`push` or later `provider_pull`)"), the identical "reserved for later" phrasing section 6.5.7 uses for GitLab/Bitbucket adapters — and section 20's Definition of Done already ratifies "GitHub as the only verified provider adapter in the MVP" as a complete, acceptable limitation. A content-fetching pull mode is that same later-adapter-maturity axis, not a distinct MVP gap: today's `push` mode (a CI-triggered upload of an already-validated, already-retained snapshot) satisfies every Definition-of-Done bullet about a "configured-provider Git commit ... creat[ing] a deterministic ready revision," so `provider_pull` is left exactly as open as multi-provider support — a legitimate future extension, not a `P4-B` shortfall. **`model_dependency` policy closed (P4-B follow-up slice, extends `P5-D`'s runtime lineage mechanism):** section 9.2 requires a runtime model dependency to pin an immutable model/deployment snapshot for QA, staging, and production, rejecting mutable aliases such as `latest`. `P5-D`'s `create_runtime_lineage()` (`workspace_runtime_lineage.py`, see that row) already stamps an immutable lineage record with a caller-supplied `model_id` before strict execution; the one open half of the policy was that `model_id` was accepted completely unvalidated. `create_runtime_lineage()` now rejects a `model_id` that looks like a mutable alias -- the literal string `latest` (case-insensitive) or a `:latest`/`-latest` tag suffix, the convention several container registries and model-hosting APIs use -- for any environment other than `dev` (keyed on `environment.environment_class != DEVELOPMENT`, `deployment_environments.py`), raising `RuntimeLineageError` and naming both the offending `model_id` and the environment. `dev` and a `model_id=None` (no declared model dependency at all) are both left untouched, matching the plan's own "QA, staging and production" scope and keeping the field optional rather than mandatory. The heuristic is deliberately narrow and documented in code rather than exhaustive: it does not attempt to enumerate every provider's own mutable-alias vocabulary (a provider-specific `default`/`stable`/`preview` tag, for example). The paragraph's other half -- "if a provider cannot offer an immutable model identifier, CALIBER records the strongest observable fingerprint and refuses production unless policy explicitly accepts that weaker guarantee" -- is a deliberately deferred future extension, not attempted in this slice: it requires integrating a specific model provider's identity/fingerprinting API, which the plan does not specify, mirroring the posture section 20 already ratifies for provider adapters generally ("GitHub as the only verified provider adapter in the MVP"). Tested in `test_workspace_runtime_lineage.py`: `latest`-shaped `model_id` values (including `:latest`/`-latest` suffixes and case/whitespace variants) are rejected for `qa`/`staging`/`prod` and accepted for `dev`; a genuinely pinned, snapshot-suffixed `model_id` (e.g. `gpt-4-2024-08-06`) is accepted for every protected environment; and `model_id=None` is unaffected in every environment, including `dev`. Audited every existing `create_runtime_lineage()` caller (`workspace_release_service.py`, `workspace_release_operation_service.py`, `workflows/run_launch.py`, `routes/evaluations.py`, `routes/workflow_runs.py`, `routes/workflow_versions.py`): none hardcodes a mutable-alias-shaped `model_id` today, so no caller required a change; the two release-service call sites never pass a `model_id` at all. **CALIBER-managed snapshot revisions delivered (post-slice follow-up, slice 1 of a multi-slice epic):** `POST /projects/{id}/revisions:snapshot` (the `P0-B` item 9/`P4-C` gap named below) pins an explicit, caller-supplied list of live CALIBER resource versions into an immutable, source-less revision -- see the `P4-C` row for the schema decision, route contract, and the first real (non-placeholder) `resolve()`/`snapshot()` adapter (`prompt`). **`tool` closed (slice 2 of the multi-slice epic):** `workspace_release_tool_adapter.py::ToolWorkspaceResourceAdapter` is a second real adapter, registered in `server.py` alongside `workflow`/`prompt` -- see the `P4-C` row for the design. **`judge` closed (slice 3 of the multi-slice epic):** `workspace_release_judge_adapter.py::JudgeWorkspaceResourceAdapter` is a third real adapter, registered in `server.py` alongside `workflow`/`prompt`/`tool` -- see the `P4-C` row for the resource-id/version-ref scheme and why it differs from both. **`eval_dataset` closed (slice 4 of the multi-slice epic):** `workspace_release_eval_dataset_adapter.py::EvalDatasetWorkspaceResourceAdapter` is a fourth real adapter, registered in `server.py` alongside `workflow`/`prompt`/`tool`/`judge` -- the first for a genuine *collection* resource (a `CaliberEvalDataset` row plus its append-only `CaliberEvalDatasetExample` children) rather than a single row; see the `P4-C` row for the resource-id/version-ref scheme and content-hash design. **`mcp_server` closed (slice 5 of the multi-slice epic):** `workspace_release_mcp_server_adapter.py::McpServerWorkspaceResourceAdapter` is a fifth real adapter, registered in `server.py` alongside `workflow`/`prompt`/`tool`/`judge`/`eval_dataset` -- see the `P4-C` row for the design (a `judge`-shaped single mutable row with no version history, content-addressed over its connection config, discovered tool catalog, and tool policy). **`skill` closed (slice 6 of the multi-slice epic):** `workspace_release_skill_adapter.py::SkillWorkspaceResourceAdapter` is a sixth real adapter, registered in `server.py` alongside `workflow`/`prompt`/`tool`/`judge`/`eval_dataset`/`mcp_server` -- see the `P4-C` row for why a skill is a genuinely fourth resource shape (a mutable live row plus a real, internal, immutable, numbered content history). **`workflow` closed (slice 7 of the multi-slice epic, and its final closure):** `workspace_release_workflow_adapter.py::WorkflowWorkspaceResourceAdapter`'s own `resolve()`/`snapshot()` -- the `P5-E` adapter that predates this whole epic, whose `resolve()` previously only did a bare unscoped `session.get()` and whose `snapshot()` was an explicit placeholder that returned its input unchanged -- are now real, closing the one item every earlier slice's row text named as still remaining. See the `P4-C` row for the design (the same mutable-parent-plus-immutable-history shape `skill` established, content-addressed over the pinned version's own compiled manifest). Every resource type this row's own follow-up list ever named is now closed; no further resource-type slice remains open in this epic | `P4-A` | Canonical tree and retained snapshot bytes are stable across ZIP metadata changes; path traversal, symlink, duplicate-path, archive-bomb, size, and digest-equivocation inputs fail closed; claim races allow one winner, stale workers cannot publish late results, bounded retry never exceeds its durable budget, and expired imports require explicit observation/reconciliation before retry; targeted Workspace module coverage is 99% and the WorkspaceSelector unit target is 100% across statements, branches, functions, and lines; the import worker and its materializer are each 100% covered by offline tests driven against real local-backend storage, including a simulated crash/lease-expiry recovery via `reconcile_expired_import_jobs` |
| `P4-C` | API/integration | **Partially delivered (provider-neutral source/import/revision API slice).** Added `WorkspaceSourceProvider`/registry contracts with no implicit network client; source GET/configure/enable/disable/capabilities/reconcile routes with project-role enforcement, secret-free projections, ETags and `412` preconditions; durable multipart push-import submission with canonical ZIP validation, content-addressed snapshot retention, idempotent replay, same-commit digest-conflict rejection, project-scoped status/list/reconcile APIs, and an explicit no-blind-retry recovery observation; cursor-paged revision list/detail/diff routes with project isolation and deterministic resource-pin comparisons; generated OpenAPI response metadata and route-scope inventory updates; and 39 focused offline route/provider/contract tests. Targeted coverage is 99% for the new route module and 100% for the provider registry. **Import-worker materialization into immutable revisions is now delivered** -- see the `P4-B` row for the worker itself. Still open: mutable-resource/model/dependency adapters (a real `WorkspaceResourceAdapter.resolve()`/`snapshot()` Protocol and `WorkspaceResourceAdapterRegistry` already exist in `workspace_release_adapters.py`, but only one resource type -- `workflow`, via `workspace_release_workflow_adapter.py` -- has a real registered adapter, and even that adapter's own `snapshot()` is an explicit, documented placeholder that just returns its input unchanged), and Change Requests/version claims in `P4-D`/`P4-E`. **"Real (non-fake) source-provider implementations" closed -- stale plural wording, not a live gap (investigated and confirmed):** this row previously carried that phrase in its "Still open" list. The plan's own ratified scope decisions settle what it meant: section 20's Definition of Done ratifies "GitHub as the only verified provider adapter in the MVP" as an acceptable, complete limitation, and section 6.5.7's `SourceControlProvider` design states "GitHub is the first adapter; GitLab, Bitbucket, self-hosted variants and `none` are later adapters" -- multi-provider support was never a required-for-MVP item, only a deliberately deferred later extension of the same capability contract. `workspace_source_control.py`'s own module docstring frames provider-neutrality as an architectural boundary ("The Workspace service must not know whether authored source lives in GitHub, GitLab, Bitbucket, or a local test double"), not a multi-provider delivery requirement. Read against that scope, "real (non-fake)" meant proving the `SourceControlProvider`/`WorkspaceSourceProvider` contract end to end against a genuine provider rather than only the in-memory fake -- and `P4-E`'s row already closes exactly that: `github_source_control.py`, `github_workspace_provider.py`, `github_http_transport.py` and `github_app_auth.py` are a real, network-backed (`httpx`), non-fake GitHub adapter implementing the full contract, including production webhook ingress and operator-triggered reconciliation. No second (e.g. GitLab) adapter is a tracked gap for this document; one remains a legitimate future later-adapter extension per the ratified decision above, not a `P4-C` shortfall. **`revisions:snapshot` delivered -- slice 1 of a multi-slice epic (post-slice follow-up, closes the gap this row previously investigated and confirmed genuinely unbuilt):** `POST /projects/{id}/revisions:snapshot` (`routes/workspace.py::snapshot_revision`) is now real, resolving the design questions the plan's own one-line API table entry (section 12.2) left open. Request/response contract: the caller supplies an explicit list of `{resource_type, resource_id, version_ref}` pins (`logical_name`/`purpose` optional) -- deterministic and auditable, matching how Change Requests/releases already work with explicit pins elsewhere in this codebase, rather than CALIBER silently resolving "current" for everything (which would make the same snapshot request produce different content depending on when/how fast it runs -- an integrity/reproducibility problem this design avoids by construction). Schema decision (migration `0111`): `caliber_workspace_revisions` gains a `source_kind` discriminator (`'git'` or `'managed'`, `ck_workspace_revision_source_kind`), and `manifest_sha256`/`source_bundle_sha256` become nullable, tied to `source_kind` by `ck_workspace_revision_source_kind_digest` (a `'git'` row still requires both, exactly as before; a `'managed'` row requires neither) -- mirroring migration `0106`'s `ck_rework_task_exactly_one_source` XOR-style CHECK precedent named by `P0-B` item 9. `revision_sha256` (unconditionally required, unchanged) remains the single content-integrity anchor for both kinds; a managed revision's package descriptor is a smaller ingredient list (project id, sorted resource pins) than the Git-import descriptor, with no source/commit/bundle fields. Idempotency is content-derived rather than header-based: the resource list's own digest becomes `revision_sha256`, so a byte-identical retry converges on the already-created revision via `uq_workspace_revision_digest` (project_id, revision_sha256) instead of duplicating, and a concurrent racing request is caught by a `begin_nested()`/`IntegrityError` guard (mirroring `routes/workflows.py`'s benchmark-report create) and replayed from the winner's committed row. First real, non-placeholder adapter (`P0-B` item 9, "per-adapter reconstructability strategy" -- partially delivered): `workspace_release_prompt_adapter.py::PromptWorkspaceResourceAdapter` proves `resolve()`/`snapshot()` end to end against a genuine live CALIBER resource type. `prompt` was chosen over `workflow` (already registered, but `P5-E`'s own `snapshot()` remains an explicit placeholder) because it is the simplest well-understood resource type per `P2-G`'s already-closed hidden-prompt-target work: no `CaliberPrompt` model exists at all (a prompt is purely an MLflow Prompt Registry entity), so `resolve()` does a genuine two-step lookup -- a CALIBER-side project-scoping check against the hidden `CaliberAgentConfig` prompt target (mirroring `routes/prompts.py::create_prompt`'s `P2-G` collision refusal) and a live, explicit-version-only MLflow Prompt Registry read (never an alias, for the same reproducibility reason as the request contract above) -- and `snapshot()` computes a genuine `content_sha256` over the loaded template rather than returning its input unchanged. The route fails closed (`409`) for any resource type with no registered adapter, or whose adapter's `snapshot()` does not return the new shared `SnapshotPin` type (`workspace_release_adapters.py`) -- at slice 1, only `prompt` qualified; `workflow` stays registered for releases but is not yet snapshot-eligible. **Second real, non-placeholder adapter closed (slice 2 of the multi-slice epic): `tool`.** `workspace_release_tool_adapter.py::ToolWorkspaceResourceAdapter` proves the same `resolve()`/`snapshot()` contract against `CaliberToolRegistry` (`routes/tools.py`). Unlike a prompt (no `CaliberPrompt` model at all, so `resolve()` needs a CALIBER-side visibility lookup *and* a separate external MLflow Prompt Registry read), a tool's single `(name, version)` registry row is simultaneously the identity, the visibility record (`project`/`user`/`public`, the same 3-tier columns `routes/tools.py::_visible_tool_or_404` already scopes by), and the content store -- so `resolve()` is genuinely one visibility-filtered query, not two. `version_ref` stays the tool's own free-form `version` string (no MLflow-style integer conversion, since there is no second version identity to resolve). `content_sha256` is computed over the fields that determine what the tool actually does and under what safety policy (`module_path`/`callable_name`/`execution_backend`/`backend_config`/`input_schema`/`output_schema`/`side_effect_level`/`requires_approval`/`allow_in_preview`/`secret_refs`), deliberately excluding administrative fields (`description`/`owner`/`status`/`successor_tool_id`) that do not change tool behavior. Because a `CaliberToolRegistry` row (unlike an MLflow prompt version) remains mutable after registration via `routes/tools.py::update_tool`, `apply_release`/`rollback_release`/`observe_release` re-read the live row at release time and fail closed (`tool_content_drifted`) if its content no longer matches the pin, rather than silently deploying content a release never actually reviewed; they also refuse a since-archived or since-deleted tool (`tool_archived`/`tool_not_found`). A tool has no runtime alias to rotate the way a prompt or workflow deployment does (a workflow references an exact tool version directly in its own manifest), so a tool release's `action` is `"verify"` rather than `"promote"` -- there is nothing to mutate, only to confirm. **Third real, non-placeholder adapter closed (slice 3 of the multi-slice epic): `judge`.** `workspace_release_judge_adapter.py::JudgeWorkspaceResourceAdapter` is for a resource shape neither the `prompt`/`workflow` precedents nor `tool`'s own single-row design covers -- `CaliberJudge` is a single mutable row with **no version history at all** (no external registry like a prompt, no internal version table like a workflow). Since the request schema (`WorkspaceRevisionSnapshotResourceRequest`) requires a non-empty `version_ref` for every resource type generically, the adapter requires callers to declare the sentinel `version_ref="current"` ("pin whatever this judge's live row currently is" -- the only truthful thing a caller can ask for when there is no numbered version to be explicit about), and `snapshot()` returns the judge's own content digest as the *persisted* `version_ref` instead, so the immutable pin is genuinely reproducible even though the declaration wasn't asking for a specific version. `content_sha256` is computed over `name`/`instructions`/`model`/`feedback_value_type` -- the exact fields `routes/judges.py::build_judge` reads to reconstruct the judge via `mlflow.genai.make_judge`, i.e. the fields that determine grading behavior (`description`/`tags`/`status` are editorial metadata, excluded, mirroring the `prompt` adapter hashing only its template). `resolve()` applies the same `get_visible` 3-tier visibility fix from the start (no separate follow-up PR needed, unlike `prompt`'s PR #393) -- simpler here since `CaliberJudge` carries `project_id`/`visibility` directly rather than through a hidden target row, the same shape `tool`'s own single-registry-row design already uses. Because a judge has no per-environment alias or external target at all (an eval run references it directly by name), `apply_release`/`rollback_release`/`observe_release` are a content-integrity check against the caller's own session (a same-database provider, mirroring `workflow`'s reasoning) rather than an alias-rotation mutation like `prompt`/`workflow` perform -- it fails closed with `judge_content_drifted` if the live row no longer matches what was pinned, rather than silently overwriting it. Registered in `server.py` alongside `workflow`/`prompt`/`tool`; tested in `test_workspace_release_judge_adapter.py` (unit, mirroring `test_workspace_release_prompt_adapter.py`'s scenario coverage including the personal/public/project-member/admin visibility matrix) and `test_workspace_revisions_snapshot_judge.py` (route-level, mirroring `test_workspace_revisions_snapshot.py`'s pattern), kept as separate files from their `prompt`-focused counterparts so a concurrently-landing sibling resource-type slice does not also need to touch them. **Fourth real, non-placeholder adapter closed (slice 4 of the multi-slice epic): `eval_dataset`.** `workspace_release_eval_dataset_adapter.py::EvalDatasetWorkspaceResourceAdapter` is for a resource shape none of `prompt`/`tool`/`judge` cover: a *collection* resource, not a single row. `CaliberEvalDataset` (`routes/eval_datasets.py`, "test set" in some route docstrings) is metadata plus an integer `version` counter; the actual content -- the example set an eval run scores against -- lives in a related, append-only child table, `CaliberEvalDatasetExample`. Unlike a judge, an eval dataset has a real, already-relied-upon numbered version history (`routes/evaluations.py::_load_example_rows` and `routes/eval_datasets.py::list_examples`'s `as_of_version` filter both already reconstruct "the example set active as of version N" for pinned/preview reads, and that reconstruction is permanently stable once N has passed because an example's `dataset_version`/`superseded_version` fields are write-once) -- so `resolve()` requires an explicit caller-supplied integer `version_ref` (never an alias, mirroring `prompt`'s own reproducibility reasoning), not the judge's `"current"` sentinel. `content_sha256` hashes an ordered `{example_id, input, expected, weight, tags}` list for every example reconstructed "as of version N" (the same predicate `_load_example_rows` uses, reimplemented rather than imported from a route module) -- not the dataset row's own administrative columns -- so adding, removing, or editing any example changes the digest, `example_id` included so a content-identical replacement (`revise_example` always mints a fresh id) still counts as a change. `resolve()` applies the same `get_visible` 3-tier visibility fix from the start (no separate follow-up PR needed). Because the example-set reconstruction for an already-passed version is append-only/write-once and therefore should be immutable, `apply_release`/`rollback_release`/`observe_release` (same-database, no per-environment alias, mirroring `judge`'s reasoning) treat a content mismatch as defense-in-depth rather than an expected condition the way `tool`'s mutable-row drift check is, and additionally refuse a since-archived dataset (mirrors `tool`'s `tool_archived` check) or a pinned version the live row hasn't reached (defensive). Registered in `server.py` alongside `workflow`/`prompt`/`tool`/`judge`; tested in `test_workspace_release_eval_dataset_adapter.py` (unit, including the historical-version reconstruction and the personal/public/project-member/admin visibility matrix) and `test_workspace_revisions_snapshot_eval_dataset.py` (route-level), kept as separate files for the same concurrent-slice reason as `judge`'s. **Fifth real, non-placeholder adapter closed (slice 5 of the multi-slice epic): `mcp_server`.** `workspace_release_mcp_server_adapter.py::McpServerWorkspaceResourceAdapter` reuses `judge`'s exact "single mutable row, no version history" shape -- `CaliberMcpServer` (`routes/mcp_servers.py`) carries `project_id`/`visibility`/`owner` directly on the row (like a judge, not through a hidden target like a prompt) and has no numbered version (`routes/mcp_servers.py::update_mcp_server` can rewrite the same row's connection fields in place), so the adapter requires the same `version_ref="current"` sentinel on the way in and returns the row's own content digest as the persisted `version_ref` on the way out. `content_sha256` is computed over exactly the fields `mcp_gateway.py::McpServerConfig.from_row` reads to actually dial the server and gate tool calls -- `transport`/`uri`/`command`/`args`/`env`/`headers`/`auth_type`/`auth_config` (the connection), `discovered_tools` (the callable tool catalog, this resource's analogue of a tool's `input_schema`/`output_schema`), and `tool_policies` (the per-tool `allowed`/`side_effect_level`/`requires_approval` governance map, this resource's analogue of the `tool` adapter's own safety-field inclusion) -- deliberately excluding `name` (immutable after creation and, unlike a judge's `name`, never used as a runtime reference: a workflow's MCP node binds to the stable `server_id`, not `name`), `description`, `icon`, `owner`, `status`, `connection_error`, `last_connected_at`, and the diagnostic `tool_test_cases`/`tool_calibrations` maps. `resolve()` applies the same `get_visible` 3-tier visibility check from the start, the same shape `judge` already established. Because `env`/`headers`/`auth_config` may still hold literal credentials on legacy rows (`mcp_secrets.py`'s write-only-sentinel containment is an API/audit-boundary contract, not encryption at rest), the adapter hashes the literal values but never places them -- only the digest and small non-sensitive summary fields (transport, tool count, status) -- into the persisted `resolution`. `apply_release`/`rollback_release`/`observe_release` are a content-integrity check against the caller's own session exactly like `judge`, additionally refusing a since-disabled server (`mcp_server_disabled`, mirroring `tool`'s own `tool_archived` lifecycle refusal) and failing closed with `mcp_server_content_drifted` on any connection/tool-catalog/policy drift since the pin was taken. Registered in `server.py` alongside `workflow`/`prompt`/`tool`/`judge`/`eval_dataset`; tested in `test_workspace_release_mcp_server_adapter.py` (unit, mirroring `test_workspace_release_judge_adapter.py`'s scenario coverage including the personal/project-member/admin visibility matrix, content-addressing across every hashed field, and drift/disabled/tenancy release refusals) and `test_workspace_revisions_snapshot_mcp_server.py` (route-level, mirroring `test_workspace_revisions_snapshot_judge.py`'s pattern), again kept as separate files from their `judge`-focused counterparts for the same concurrent-slice reason. **Sixth real, non-placeholder adapter closed (slice 6 of the multi-slice epic, and the last named resource type in the original follow-up list): `skill`.** `workspace_release_skill_adapter.py::SkillWorkspaceResourceAdapter` is for a resource shape none of `prompt`/`tool`/`judge`/`eval_dataset` fully cover: a `CaliberSkill` (`routes/skills.py`) is **both** a mutable live row (identity + 3-tier visibility, the same `project`/`user`/`public` columns `tool`/`judge` already carry directly) **and** a real, internal, immutable, numbered content history -- `CaliberSkillVersion`/`caliber_skill_versions`, written by `skill_versions.py` on every create/update/rollback in the same transaction as the live-row mutation. That gives a skill a genuine version identity to pin explicitly (unlike a judge or MCP server, which have none and must fall back to the `"current"` sentinel), while that history stays internal to CALIBER's own database (unlike a prompt's external MLflow Prompt Registry) -- so `resolve()` is two lookups against the *same* database: a `get_visible` 3-tier visibility check against the live row, then an exact `(skill_id, version_number)` read against `caliber_skill_versions` for the pinned version's immutable `content`/`summary`. `content_sha256` is computed over exactly those two fields -- the only ones the history table captures per version (`name`/`category`/`tags`/`allowed_tools`/`depends_on` live only on the mutable row and are never historized, so including any of them would make an "immutable" pin not actually reproducible). Because a skill has no per-environment alias or external deployment target either (an agent composes a referenced skill directly by name, mirroring why `tool`/`judge`/`mcp_server` releases are verifications, not promotions), `apply_release`/`rollback_release`/`observe_release` are the same same-database content-integrity check as `tool`/`judge`/`mcp_server`, additionally refusing a since-archived skill (`skill_archived`) and a history row that has since disappeared (`skill_version_not_found`) before comparing content digests (`skill_content_drifted`). Registered in `server.py` alongside `workflow`/`prompt`/`tool`/`judge`/`eval_dataset`/`mcp_server`; tested in `test_workspace_release_skill_adapter.py` (unit, mirroring `test_workspace_release_judge_adapter.py`'s visibility-matrix coverage plus `test_workspace_release_prompt_adapter.py`'s explicit-version-only `resolve()` coverage) and `test_workspace_revisions_snapshot_skill.py` (route-level, mirroring `test_workspace_revisions_snapshot_judge.py`'s pattern), again kept as a separate file. **Seventh, and final, non-placeholder adapter closed (slice 7 of the multi-slice epic): `workflow`.** `workspace_release_workflow_adapter.py::WorkflowWorkspaceResourceAdapter` is the one adapter every earlier slice's row text named as the sole remaining item -- it predates this epic entirely (`P5-E`, built for release alias-rotation only), and its own `resolve()`/`snapshot()` had stayed explicit, documented placeholders (`resolve()` did a bare unscoped `session.get(CaliberWorkflow, ...)` with no visibility check at all; `snapshot()` returned its input unchanged) because nothing called them until this route existed. A `CaliberWorkflow` (`routes/workflows.py`) turns out to be `skill`'s exact shape: a mutable live row (identity, ownership, the same `project`/`user`/`public` 3-tier visibility columns) plus a real, internal, immutable, numbered content history -- `CaliberWorkflowVersion`/`caliber_workflow_versions`, whose *published* rows are already documented elsewhere as immutable (`routes/workflow_versions.py`'s own module docstring: "Published versions are immutable"). `resolve()` is accordingly the same two-lookup shape: a `get_visible` 3-tier visibility check against the live `CaliberWorkflow` row (applied from the start, the non-negotiable fix `prompt`'s own PR #393 needed as a follow-up), then an exact `(workflow_id, version_id)` read against `caliber_workflow_versions`, refusing a draft version outright (a draft's `manifest` can still be edited in place via `update_version`, so pinning one would not actually be content-addressed). Unlike every other resource type's `version_ref` (an integer version number, or a content-digest/`"current"` sentinel for a resource with no numbered history), this adapter keeps its own pre-existing, already-relied-upon convention instead of adopting a new one: `version_ref`/`provider_ref` carry the version's own `version_id` primary key, exactly what `validate()`/`prepare_release()`/`apply_release()`'s already-shipped alias-rotation logic (`rotate_alias_to_version`) already expects -- so a pin `resolve()`/`snapshot()` produce plugs into the existing release path with no translation step, and neither `apply_release`/`rollback_release`/`observe_release` (real, working alias-rotation logic since `P5-E`) needed any change. `content_sha256` hashes the version's own `manifest` -- the compiled node graph that determines what the workflow actually does when deployed/run -- reusing `workflows/manifest.py::compute_manifest_hash`, the exact canonical-hashing routine every workflow route already uses for a version's own `manifest_hash` column (`routes/workflows.py::import_workflow`, `routes/workflow_versions.py::_insert_version_with_retry`/`update_version`), rather than inventing a second, adapter-local hashing convention for the same content the way `tool`/`judge`/`eval_dataset`/`mcp_server`/`skill` each had to (none of them had a pre-existing domain hash to reuse; `workflow` did). Registered in `server.py` alongside every other resource type already (`workflow` has been registered since `P5-E`, for release execution -- this slice only makes its `resolve()`/`snapshot()` real); tested in `test_workspace_release_workflow_adapter.py` (unit, extending `P5-E`'s existing apply/rollback/observe/validate coverage with the same personal/public/project-member/admin visibility matrix, explicit-version and draft-refusal edge cases, and content-addressing coverage every sibling adapter's own test file carries) and `test_workspace_revisions_snapshot_workflow.py` (route-level, mirroring `test_workspace_revisions_snapshot_skill.py`'s pattern). Every resource type this row's own follow-up list ever named is now closed; no further resource-type slice remains open in this epic. **GitHub Actions/SDK example delivered (post-slice follow-up):** Phase 4 item 8 ("Add a GitHub Action and SDK example for push import using a project-bound CALIBER PAT") is closed by `sdk/caliber-sdk/examples/workspace_github_source.py::connect_github_source_and_import_on_push`, which demonstrates `client.workspaces.source_connection.configure()` (`P4-E`) followed by `client.workspaces.imports.create()`/`.wait()` (`P6-B`), embedded in `docs/sdk/beta.md`'s new "GitHub-backed Workspace import" section alongside the literal `.github/workflows/caliber-import.yml` a calling repository would add to submit a push import from CI using a project-bound PAT, per section 6.5.4's "GitHub Actions ... may validate source and invoke a CALIBER import with a workspace-bound short-lived credential." Tested the same way every other `P6-C` beta example is: exercised in `sdk/caliber-sdk/tests/test_examples.py` against a stub server, and enforced end to end by `caliber/tests/test_sdk_docs_contract.py` (the example exists, is referenced from `docs/sdk/`, is executed by the SDK test suite, and is not orphaned). | `P4-B` | Lost clients can rediscover durable jobs; source replacement/disable is blocked while imports are queued, running or reconciliation-required; stale ETags, context mismatches, provider identity mismatches, conflicting bytes and cross-project identifiers fail closed; ordinary tests require no network or credentials; remaining snapshot/adapter/provider/review work is explicit rather than represented as complete |
| `P4-D` | Data/API/security | **Delivered.** Added the Change Request/head/reviewer/comment/check/review/external-attestation/version-claim/version-tag schema and migration; selectable `caliber`/`source_provider` backend contracts; project-authorized create/submit/update-head/rebase/close, discussion, reviewer, review-history, check-history, attestation-history and version-tag routes; SemVer reservation; head-generation and lock-version invalidation; stale-base rebase; fail-closed provider attestation normalization; and the service-only acceptance CAS primitive. Native technical approval is head-bound, provider evidence cannot be synthesized by public callers, and historical rows remain readable. Added the public acceptance route (`POST .../change-requests/{id}:accept`, owner-only `change_request.accept` action): it derives `qa_evidence` itself from a durable, server-recorded `CaliberWorkspaceReleaseDecision` (`kind="quality"`, `decision="go"`) bound to the Change Request's current head via a `qa`-class `CaliberWorkspaceRelease`, never from a caller-supplied claim, and a typed `ProjectChangeRequestsAPI.accept()` SDK method (sync-only, matching `P6-B`'s existing surface). **Stale trailer corrected:** this row previously listed "a provider-neutral `SourceControlProvider` adapter/registry, GitHub integration, provider webhook/reconciliation, and Phase-5 QA/release decisions" as still open -- all four have since landed: the provider-neutral adapter/registry and GitHub integration are `P4-C`'s/`P4-E`'s rows, provider webhook/reconciliation is `P4-E`'s row, and Phase-5 QA/release decisions for the broader release lifecycle are `P5-B`'s (decisions)/`P5-C`'s (adapters/effects)/`P5-D`'s (runtime lineage) rows, all delivered. **CLI delegate for `:accept` (follow-up closure):** the small, tracked future gap noted above is closed -- `caliberctl workspace cr accept <change_request_id>` (`caliber-cli`'s `cli.py`/`commands.py`) delegates to the existing `ProjectChangeRequestsAPI.accept()` with no evidence flags of its own, matching the route's own trust model; its `409 qa_go_decision_required` now maps to a distinct `AWAITING_HUMAN` exit code (`api_error_exit`, keyed on that route-specific detail string) instead of collapsing into `FAILURE`, so a script can tell "QA hasn't gone yet" apart from a hard error, the same way `CHANGES_REQUESTED`/`OUT_OF_DATE` already do for the rest of this command group. | `P4-C`, `P1-C` | Focused lifecycle, authorization-inventory, async-offload, migration-parity and CAS tests pass; parallel acceptance cannot overwrite the accepted pointer; new heads invalidate authority while retaining history; non-author/stale-head/provider-policy/version conflicts deny deterministically. |
| `P4-E` | Integration/security | **Delivered except one explicitly deferred item (production status-publication wiring).** Added typed capabilities, commit/tree and reachability comparisons, change-request/review/check/ruleset normalization, exact-head and complete-range verification, signed webhook normalization, optional status publication, a bridge into P4-D's external-attestation input shape, durable normalized source-event inbox rows with delivery-idempotency/conflict detection, literal event transitions with explicit requeue, and revocable provider-actor links bound to the source/project. The fake adapter performs no network I/O and refuses unavailable capabilities explicitly. The GitHub adapter accepts an injected transport and credential providers, emits only normalized contract values, binds pull-request base repositories and webhooks to the configured canonical repository, and translates malformed/untrusted responses into typed fail-closed errors; ordinary tests remain credential-free. **Slice 1 (connection storage + real provider registration):** `CaliberWorkspaceSourceConnection` (`caliber_workspace_source_connections`, one active connection per source, partial-unique-indexed) persists a GitHub App connection's app id/installation id/source binding while its private key and webhook secret are written exclusively through `secret_store.py` and referenced only as `secret://name` -- never a plaintext column (`workspace_source_connections.py`); `github_app_auth.py` mints short-lived GitHub App installation access tokens via RS256 JWT exchange, cached only in memory for GitHub's own reported lifetime; `github_http_transport.py` is the first real (`httpx`-backed) `GitHubTransport` implementation; `github_workspace_provider.py` bridges a stored connection into a real `GitHubSourceControlProvider` and implements the `WorkspaceSourceProvider` protocol the registry expects, resolving credentials fresh per call and never caching them at construction; `routes/workspace_source_connections.py` adds `GET`/`PUT /projects/{id}/source/connection` and `POST .../source/connection:revoke`, gated by the same `caliber.operator` scope + `source.manage` project-role check as the P4-C source routes, with a statically secret-free response schema; `server.py` registers a real `GitHubWorkspaceSourceProvider` under `"github"` only when the new `github_source_control_enabled` flag is set and an encrypted secret store is bound, otherwise the registry stays empty exactly as before. **Slice 2 (production webhook ingress):** `routes/github_webhooks.py` adds `POST /webhooks/github`, authenticated purely by HMAC signature verification -- no CALIBER session/PAT, since GitHub is the caller. An inbound delivery's untrusted `installation.id` claim resolves at most one candidate connection via a new DB-level partial unique index on `(provider, installation_id)` for active connections (migration `0109`; `workspace_source_connections.py::get_connection_by_installation`); HMAC verification against that candidate's own stored secret (`verify_webhook`) is the actual admission decision, and "no matching installation" and "wrong signature" are indistinguishable in the response. `github_workspace_provider.py::build_webhook_verifier` and `workspace_source_connections.py::resolve_webhook_secret` touch only the webhook secret, never the private key, for this check. A verified delivery is inserted via the already-durable `workspace_source_events.py::record_source_event` inbox. The route is exempted from CSRF/rate-limit middleware by exact path match (`server.py`), the first POST route in this codebase to need that. **Slice 3 (pagination/reconciliation):** `github_webhook_reconciliation.py` adds operator-triggered catch-up for webhook deliveries CALIBER never received (downtime, GitHub's retry window exhausted): it lists a connection's App's recent deliveries via GitHub's own App-scoped `/app/hooks/deliveries` log (auth via `github_app_auth.py::mint_app_jwt`, the App's own JWT -- not an installation token, and only the private key is touched, never the webhook secret, via the new `workspace_source_connections.py::resolve_private_key`), compares delivery ids against the durable inbox, and asks GitHub to redeliver whatever is missing. Redelivery re-enters through the real `routes/github_webhooks.py` route exactly like a live delivery would -- this module never inserts into the inbox itself, so it inherits that route's own signature verification and idempotency for free. Exposed as `POST /projects/{id}/source/connection:reconcile-deliveries` (same `caliber.operator` + `source.manage` gate as the other connection routes). Bounded to `MAX_PAGES` (20) pages per call; periodic/scheduled reconciliation is not implemented, only operator-triggered -- a natural, small future extension if needed, not built here to keep this slice scoped. **Actor-link revalidation against provider identity: confirmed already satisfied by the existing data-access layer, no new code needed.** `workspace_source_events.py::get_active_source_actor_link` already performs a live database query filtered to `status == 'active'` on every call -- there is no cache to invalidate; a revoked link stops satisfying that filter on the very next call. However, this function currently has **no consumer anywhere in the codebase**: `workspace_source_control.py::verify_normalized_change_request`'s reviewer-eligibility check uses a separate, policy-configured `eligible_reviewer_ids` allowlist of raw provider actor ids, not the actor-link table at all. The mechanism is real, tested infrastructure with no consumer yet; wiring it into review-eligibility evaluation is new functional scope with its own design question (how it should interact with `eligible_reviewer_ids`). **Decided (repo owner, explicit): leave unwired.** The `eligible_reviewer_ids` allowlist already works and is what review-eligibility actually uses; wiring in a second, currently-unused mechanism without a clear driving need would create two sources of truth for the same decision with no proven benefit. Revisit only if a concrete requirement for stronger reviewer-identity assurance emerges. **First-package baseline procedure: confirmed already satisfied, no special-casing needed.** A source's first imported revision goes through the identical import -> revision -> Change Request -> review pipeline as every subsequent one; nothing in the import worker, revision persistence, or Change Request service queries for or assumes a predecessor revision exists. **Production status-publication wiring: explicitly deferred, not part of this series.** `github_source_control.py::publish_status()` is fully built and tested but intentionally left unwired to any caller -- no route or worker invokes it yet. Still open: production status publication wiring (the sole remaining item, deliberately deferred). **SDK coverage gate closure (post-slice-3 follow-up):** the four operator-facing `/projects/{id}/source/connection*` routes added above (`GET`/`PUT .../source/connection`, `POST .../source/connection:revoke`, `POST .../source/connection:reconcile-deliveries`) shipped with no `caliber-sdk` coverage and no `coverage_allowlist.toml` entry, which `main`'s push-only "Full backend suite" CI job (`tests/test_sdk_api_coverage.py::test_every_live_operation_is_covered_or_allowlisted`) caught failing closed after this series merged (not caught at PR time, since that gate only runs on push to `main`). Fixed by adding a typed `ProjectSourceConnectionAPI` (`get`/`configure`/`revoke`/`reconcile_deliveries`) to `caliber-sdk`, sibling to `ProjectSourceAPI` and exposed as `client.workspaces.source_connection`; `POST /webhooks/github` -- called by GitHub itself, never an SDK client, per this row's own webhook-ingress description above -- got a permanent `coverage_allowlist.toml` `[[exclusion]]` instead, the same treatment as `/auth/login`'s browser-cookie session. | `P4-D` | Offline contract tests prove exact-head and source-range binding, unknown/bot/untrusted evidence does not count, signature and repository checks fail closed, missing capabilities are typed refusals, duplicate deliveries converge without duplicate rows, and revoked actor links cannot remain active; secret material is never present in any connection/webhook-route response, log line, or error message; credentials resolve end to end through `secret_store.py` (never a plaintext column); a connection whose GitHub App authentication fails reports `verify()` as unavailable rather than "verified"; a webhook with a bad/missing signature or an unresolvable installation is refused generically and never reaches the inbox; a webhook signed with a different connection's secret is refused; duplicate/reconciled deliveries converge without duplicate inbox rows |
| `P5-A` | Data/backend | **Partially delivered (durable foundation slice).** Added Workspace release/evaluation/evidence/decision/break-glass and release-operation/item IDs, ORM models, migration `0102`, typed schemas, and provider-free release/evaluation and operation state-machine services with lock-version CAS and atomic evaluation lease claim/heartbeat arbitration. **Phase 4 item 11 (source-mode transition rules and Git-managed authority) closed -- a real, previously-unenforced gap an audit confirmed: `project.source_mode`/a revision's `source_kind` was never checked anywhere in `workspace_release_service.py`, `workspace_release_governance.py`, `routes/workspace_releases.py`, or `routes/workspace.py::snapshot_revision`, so a `git_managed` project could create and promote CALIBER-managed (non-Git) snapshot revisions through release with nothing enforcing Git-managed authority exclusivity.** `create_workspace_release` now calls a new `_require_source_mode_promotion_eligible` gate that implements this item's own wording verbatim -- "local drafts are non-promotable beyond development until imported from a commit." `caliber_projects.source_mode` (`P1-A`, migration `0093`) is a closed two-value field (confirmed against `schemas.py`'s `Literal["caliber_managed", "git_managed"]` and `routes/workspace.py::_put_source_sync`, the only site that ever writes `"git_managed"`, with no reverse transition -- there is no third mode and no reverse authority case to handle); a revision's `source_kind` (`P4-C`, migration `0111`) is the matching closed two-value discriminator (`"git"`, Git-import materialized, vs. `"managed"`, a CALIBER-managed snapshot from `routes/workspace.py::snapshot_revision` with no Git commit behind it). The gate only restricts the intersection of both: once a project is `git_managed`, a `"managed"` revision may still release to the `development` environment class but is refused (a new `WorkspaceReleaseSourceModeError`, a `WorkspaceReleaseConflictError` subclass) for `qa`/`staging`/`production`; a `"git"` revision is always eligible, and a `caliber_managed`-mode project is unaffected at every tier -- the plan names only Git-managed authority as exclusive, never the reverse. `WorkspaceReleaseSourceModeError` needed no new route wiring: `routes/workspace_releases.py::_create_release_sync`'s existing `WorkspaceReleaseConflictError`-to-`409` mapping already surfaces it. The gate fails closed if the project, revision, or environment cannot be resolved, and is skipped on an idempotent replay of an already-decided release (consistent with how every other immutable coordinate on the request is treated). | `P3-A`, `P4-D` | 86 focused tests (70 plus 16 covering this gate: refusal at `qa`/`staging`/`production` for a `git_managed` project's managed revision, success at `development`, success for a `git`-kind revision at every tier, success under the default `caliber_managed` mode at every tier, idempotent-replay non-interference, and fail-closed missing-project/-revision/-environment paths) cover illegal edges, durable lease recovery, idempotency, typed schemas, DB constraints, and lease CAS races; `workspace_release_service.py` itself is 100% covered (statements and branches); development/staging machine passes (with staging predecessor proof) reach `approved`, while QA/production stop at `awaiting_quality_signoff`; no provider calls or external effects. P5-B security decisions/actor binding/break-glass policy and P5-C adapters/effects/reconciliation/rollback/routes remain open. |
| `P5-B` | Security/backend | **Delivered.** Added migration `0103`, digest-bound append-only QA and production decision services, exact Change Request head/revision binding, role/scope snapshots, originator/requester separation, explicit recovery-policy state, severity-aware audit records, and a provider-free interactive break-glass authorization plus prepared apply intent with idempotent replay and atomic persistence. **Section 9.2/Phase 4 item 15/Phase 5 item 5 corrected (this audit):** those sections described QA entry minting an immutable `<version>-rc.<generation>` candidate tag and moving the Change Request to a `qa_in_progress` status before quality sign-off could act. Investigated as a possible unwired functional gap; neither ever shipped and neither has a real caller or test that depends on it existing -- `workspace_change_request_service.py::create_version_tag(kind="qa_candidate")` has zero production call sites, and `CR_QA_IN_PROGRESS` is never assigned to `request.status` anywhere in the codebase, though `record_workspace_release_decision` and `accept_change_request` both defensively tolerate it as one of several valid Change Request statuses. The shipped design is a deliberate simplification, not an accidental gap: quality sign-off records the QA `go`/`no_go` decision directly against the `technically_approved` head, and `P4-D`'s separate, explicit `:accept` route derives that decision and attempts the acceptance CAS itself, creating only the final `accepted` tag. Both the unused schema `kind` value and status literal remain as harmless, reserved extension points; docs corrected to describe the shipped flow instead of the superseded draft. | `P5-A` | Both decisions are digest-bound and append-only; author/requester self-approval and non-interactive break-glass deny; emergency production apply requires an interactive `caliber.admin` session, enabled recovery policy, accepted QA package, verified staging predecessor, production coordinates, bounded expiry, and high-severity audit evidence. P5-C still owns provider effects, environment pointers, reconciliation, rollback, and public routes. |
...
| `P5-E` | Release/integrations | **Delivered.** The first real (non-fake) `WorkspaceResourceAdapter`: `WorkflowWorkspaceResourceAdapter` rotates a workflow's deployment alias through the existing `promoter.rotate_alias_to_version()` primitive, fails closed rather than silently skip a configured deploy gate, and is registered for the `workflow` resource type in `server.py`'s `create_app()` — the release-operation service `P5-A` through `P5-D` built can now actually deploy something instead of only ever exercising `FakeWorkspaceResourceAdapter`. Changed `WorkspaceResourceAdapter.apply_release`/`observe_release`/`rollback_release` to receive the caller's own session rather than open a second connection: a same-database provider writing on a separate connection deadlocks against SQLite's single-writer lock while the caller's own transaction is open, and sharing the session is also more correct — the operation's state and the provider mutation commit as one atomic unit instead of leaning on `reconcile_required` to paper over a window where one succeeded and the other did not. A resource type with no registered adapter (every type but `workflow`, today) still fails closed exactly as before. | `P5-C` | Alias-rotation, deploy-gate-refusal, tenancy, no-op, rollback-checkpoint, and observation tests pass against a real deployment row, not a fake; the changed adapter Protocol signature is satisfied by every registered adapter and checked by `mypy src`; a resource type with no registered adapter still fails closed |
| `P5-F` | Release/integrations | **Delivered.** Added the Workspace release HTTP surface `P5-A` through `P5-D`'s service/governance layer never had: `caliber/src/caliber/routes/workspace_releases.py` (`GET`/`POST /projects/{id}/releases`, `GET .../releases/{id}`, `GET .../evidence`, `POST .../evaluate`, `GET .../evaluations`, `GET .../evaluations/{id}`, `POST .../quality-signoff`, `POST .../approve`, `POST .../break-glass-apply`), wrapping `workspace_release_service.py`/`workspace_release_governance.py` exactly as they already worked and were already tested -- no new business logic. `workspace_change_requests.py`'s own module docstring named this exact gap: "the Phase-5 acceptance primitive is intentionally not routed here." Create/read/evaluate authorize via `require_project_access` in the route layer (`release.request`/`release.evaluate`/`read`, already-registered actions); quality-signoff/approve/break-glass delegate authorization entirely to the governance module, which needs identity-specific role checks a scope check cannot express. List endpoints use `limit`/`offset`, matching the sibling release-operations routes rather than the cursor convention newer Workspace lists use -- the two conventions already coexist in shipped code. **Named, deliberate gap -- now closed (PR #391):** break-glass-apply's `machine_gates_passed`/`integrity_checks_passed` booleans no longer hardcode `True`. `workspace_release_governance.py::derive_break_glass_gate_evidence` re-derives both live from the release's own currently-persisted status and evidence digest, called from `routes/workspace_releases.py::_break_glass_apply_sync` immediately before `create_workspace_break_glass_apply`, per that function's own docstring instruction. `create_workspace_break_glass_apply`'s existing unconditional deny-on-false check is unchanged: break-glass still never bypasses a failed machine gate or integrity check -- the fix only stops fabricating a passing one when the persisted evidence says otherwise. Every other break-glass precondition (gate-digest binding, verified QA->staging->prod chain, distinct-actor, one-hour TTL, real session credential) was, and remains, fully enforced. | `P5-D` | Route-level tests pin request parsing, status codes, the authorization-boundary split, and exception-to-HTTP mapping (the state machine and decision rules themselves are already exhaustively covered at the service layer); `test_route_scope_inventory.py`/`test_routes_openapi.py` pass with zero manual inventory edits; break-glass-apply is proven callable only by a real interactive-session credential, never trusted-header/PAT auth. The re-derivation is proven both at the governance-helper level (a genuinely passed release derives `True`/`True` and still applies; a release whose evidence digest or status has since moved on derives `False` and is denied) and end-to-end through the real route wiring (a release whose evidence goes stale after the chain was built is denied over `_break_glass_apply_sync` itself, not just the helper in isolation) |
| `P6-A` | SDK | **Delivered (scope and resource foundation slice).** Both transports now expose the tri-state project override (`UNSET_PROJECT`, explicit project ID, and explicit `None`) over `ContextVar`-backed ambient state; both clients expose `workspace_scope()`/`library_scope()` with nested restoration, retain `project_scope()` as a compatibility delegate, and expose `workspaces is projects`. Sync/async `WorkspacesAPI` aliases point at the existing project resource so no second transport or model tree can drift. | `P0-B` | Focused sync/async scope-isolation, explicit-library-header, alias-identity, and legacy-compatibility tests pass; no unsupported public lifecycle method is exported. Remaining typed Workspace models and lifecycle resources are owned by `P6-B`/`P6-C`. |
| `P6-B` | SDK + API | **Delivered (sync surface; async parity is `P6-C`).** Added `CursorPage[T]` (the opaque-token pagination primitive Workspace's cursor-based list endpoints need, distinct from the rest of the SDK's `limit`/`offset` `Page`); `WorkspaceImportJob`/`WorkspaceRevision`/`WorkspaceRevisionResource`/`WorkspaceRevisionDiff`/`WorkspaceImportReconciliation`/`WorkspaceSource`/`WorkspaceSourceState`/`WorkspaceSourceCapabilities`/`WorkspaceChangeRequest`/`WorkspaceChangeRequestHead`/`WorkspaceChangeRequestReviewer`/`WorkspaceChangeRequestComment`/`WorkspaceChangeRequestCheck`/`WorkspaceChangeRequestReview`/`WorkspaceExternalReviewAttestation`/`WorkspaceVersionTag`/`WorkspaceRelease`/`WorkspaceReleaseEvaluation`/`WorkspaceReleaseEvidence`/`WorkspaceReleaseDecision`/`WorkspaceBreakGlassApplyResult`/`WorkspaceReleaseOperation`/`WorkspaceReleaseOperationItem`/`WorkspaceReleaseOperationResult` models; and `ProjectSourceAPI` (`get`/`configure`/`enable`/`disable`/`reconcile`/`capabilities`, every mutation `If-Match`-checked)/`ProjectImportsAPI` (`list`/`get`/`create`/`reconcile`/`wait`, multipart bundle upload, digest-safe-replay `Idempotency-Key`)/`ProjectRevisionsAPI` (`list`/`get`/`diff`)/`ProjectChangeRequestsAPI` (`list`/`get`/`create`/`submit`/`update_head`/`rebase`/`close`, comments, reviewers, reviews, external-review attestations/refresh, checks -- lock-based mutations take `expected_lock_version`, the server's own optimistic-concurrency field for this resource, distinct from `ProjectSourceAPI`'s `If-Match` etag)/`ProjectVersionTagsAPI` (`list`/`get`)/`ProjectReleasesAPI` (`list`/`create`/`get`, `list_evidence`, `evaluate`, `list_evaluations`/`get_evaluation`, `quality_signoff`/`approve`, `break_glass_apply`, `wait_for_evaluation`)/`ProjectReleaseOperationsAPI` (`list`/`get`/`create`/`apply`/`observe`/`cancel_expired`, `wait`), exposed as `client.workspaces.source`/`.imports`/`.revisions`/`.change_requests`/`.version_tags`/`.releases`/`.release_operations`. Closes all 38 of `coverage_allowlist.toml`'s originally-tracked `P6-B` gaps (0 remain; the file now holds only permanent `[[exclusion]]` entries) -- the last 16 (6 release-operation, 10 release-lifecycle) closed in a follow-up slice, one bug caught by the coverage gate itself along the way: `cancel_expired`'s literal path was first written split across two adjacent f-string literals, which `docs-site/sdk_coverage.py`'s single-quoted-literal regex only partially matched, silently under-covering that one route until the gate test failed and named it. The evaluation/apply/rollback waiters the fuller Phase 6 narrative names are now also delivered: `ProjectReleasesAPI.wait_for_evaluation()` and `ProjectReleaseOperationsAPI.wait()` (the latter covers both `apply` and `rollback` operations, which share one status machine distinguished only by `kind` -- one waiter, not two). Deliberately sync-only, matching the table's own phase split (`P6-C` owns async parity) rather than the fuller Phase 6 narrative's "sync and async together" phrasing for this step -- revisiting both at once would have coupled two independently reviewable changes. Also added `ProjectMembersAPI` (`list`/`add`/`update`/`remove`/`transfer_ownership`), grouping membership/ownership management the same way `ProjectSourceAPI`/`ProjectImportsAPI`/etc. already group their own routes; `ProjectsAPI`'s own `list_members`/`add_member`/`update_member`/`remove_member`/`transfer_ownership` now delegate to it rather than duplicating the request logic -- a root convenience, not a second implementation, and unchanged behavior for every existing caller (proven by the pre-existing flat-method tests passing unmodified against the new delegates). Environment `update`/`rollback` with ETags is now also closed: added `PATCH /projects/{project_id}/environments/{name}` (`caliber/src/caliber/routes/projects.py::update_project_environment`) -- the server genuinely had no route for this before (only `list`/`get`/`enable`/`disable` existed), so this is new server surface, not just a typed client over something that already existed. It CASes on `lock_version` directly (409 on a stale `expected_lock_version`) rather than an `If-Match` header, matching `ProjectReleaseOperationsAPI`'s own `expected_environment_lock_version` convention against this exact column rather than inventing a second concurrency mechanism for the same row; `policy_sha256` is a caller-supplied digest the server stores and does not independently verify, the same trust model every other release-lifecycle digest field already uses. Identity fields (`name`/`environment_class`/`promotion_order`) remain immutable -- enforced by the request schema forbidding extra fields, not merely by convention -- so section 12.2's "environment identity fields cannot be edited or deleted" still holds; the one pre-existing test asserting a blanket 405 for `PATCH` on this route was updated to assert PUT/DELETE still 405 and PATCH rejects identity fields, rather than PATCH being absent entirely. "Rollback" is judged already delivered via the existing `kind="rollback"` release-operation (its own `wait()` shipped earlier in this same effort) -- the plan's phrasing is read as describing that mechanism from the environment's perspective, not a second, separate endpoint. Also added the typed `ProjectsAPI.update_environment()`/`AsyncProjectsAPI.update_environment()` pair (async included here, not deferred to `P6-C`, because `test_async_project_public_signatures_match_the_sync_surface` enforces flat -- non-grouped -- `ProjectsAPI` methods stay in sync/async lockstep at all times; grouped sub-resources like `ProjectMembersAPI` are exempt from that specific check, which is why their own async parity could wait) | `P4-D`, `P5-D`, `P5-F`, `P6-A` | OpenAPI inventory has no untracked Workspace route; examples drive package, review and development release without raw HTTP |
| `P6-C` | SDK/docs | **Delivered.** Added `AsyncProjectImportsAPI`/`AsyncProjectReleasesAPI`/`AsyncProjectReleaseOperationsAPI`, mounted as `client.workspaces.imports`/`.releases`/`.release_operations` on `AsyncProjectsAPI` alongside the pre-existing async root/member/storage/file/environment surface. **Deliberate correction to the plan's own "async parity" phrasing:** `caliber_sdk.aio.client`'s own module docstring documents a considered, historically-grounded decision that this SDK does *not* mirror every typed resource module into async -- "two hand-written copies of the same resource tree is two places for a path to be renamed and one place for it to be forgotten" -- and scopes async coverage to where it "earns its complexity": long-running work you poll, and streaming/concurrency. A blanket mirror of every `P6-B` grouped resource (`ProjectSourceAPI`, `ProjectRevisionsAPI`, `ProjectChangeRequestsAPI`, `ProjectVersionTagsAPI`, `ProjectMembersAPI`, `ProjectReworkTasksAPI`) would have directly contradicted that documented, reasoned policy rather than fulfilled the Phase's "full lifecycle... sync and async" outcome in the spirit it was written. Scoped instead to the three resources with a `wait()`-shaped long-running-poll operation -- `ProjectImportsAPI.wait()`, `ProjectReleasesAPI.wait_for_evaluation()`, `ProjectReleaseOperationsAPI.wait()` -- exactly the stated criterion, plus `ProjectReleasesAPI`/`ProjectReleaseOperationsAPI`'s plausible concurrency case (awaiting many projects' releases together, the same shape `AsyncWorkflowRunsAPI` already covers for workflow runs). The remaining six grouped resources stay sync-only by the same already-documented reasoning that excludes ~40 other resource modules platform-wide; `client.raw` plus `caliber_sdk.models.decode` remains the escape hatch, per that module's own stated design. Also added an executable example, `sdk/caliber-sdk/examples/workspace_release.py::promote_a_reviewed_revision` -- Change Request open/submit, release create, evaluate, `wait_for_evaluation`, quality-signoff, approve, apply, and `wait`, in one typed-calls-only function -- embedded in `docs/sdk/beta.md`'s new "Workspace release governance" section (a real `python-example` fence, not hand-written prose, so `test_the_examples_directory_is_not_dead_code`/`test_every_referenced_example_is_executed_by_the_test_suite` keep it honest) and wired into `docs-site/generate_sdk_docs.py`'s `CLASS_EXAMPLES` for `ProjectChangeRequestsAPI`/`ProjectReleasesAPI`/`ProjectReleaseOperationsAPI`. This is what Phase 6's own acceptance criterion means by "a revision can be packaged, reviewed and promoted through development with typed calls only." Also added the `caliberctl workspace` command group required by section 14: thin argparse delegates in `sdk/caliber-cli/src/caliber_cli/cli.py`/`commands.py`, one subgroup per resource family -- `import` (`create`/`status`/`list`/`reconcile`), `package` (`list`/`show`/`diff`, over `ProjectRevisionsAPI` -- "package" is section 13's own name for a ready `WorkspaceRevision`), `cr` (`list`/`show`/`create`/`submit`/`update`/`rebase`/`review`/`close`), and `release` (`list`/`status`/`versions`/`create`/`evaluate`/`quality-signoff`/`approve`/`apply`/`rollback`/`operation-status`) -- with no local authorization logic, exactly as required: every command is a direct call into the `P6-B`/`P6-C` typed SDK surface and the exit code is always read off a status the server returned, never computed by the CLI. Every leaf sets `requires_project=True` (not only the writes, unlike every pre-existing group): Workspace routes take `project_id` as a required path segment the SDK method itself demands, so an operator who forgot `--project` needs a clear usage refusal rather than a request built against a literal `/projects/None/...` URL. Section 14's required exit-state mapping is implemented as three new `caliber_cli.exits` codes -- `CHANGES_REQUESTED` (7), `OUT_OF_DATE` (8), `RECONCILE_REQUIRED` (9) -- plus a deliberate reuse of two existing ones where the meaning already matches exactly: a Change Request or release sitting on a human decision (`draft`/`open`/`technically_approved`/`qa_in_progress`, or `awaiting_quality_signoff`/`awaiting_approval`) maps onto the already-existing `AWAITING_HUMAN`, and a `blocked` release evaluation onto the already-existing `GATE_FAILED`, the same code `release quality-signoff`/`release approve` already use for a recorded `no_go`. Five distinct non-success codes for five distinct states, without doubling a table that already had the right shape for two of them. `release apply`/`release rollback` prepare-then-execute against `ProjectReleaseOperationsAPI`, and report the result `apply()` already returned synchronously (through the provider adapter) rather than forcing `--no-wait` to mean "assume success": only a genuinely not-yet-terminal result is deferred to `--no-wait`'s "submitted, check later" meaning, so a `failed`/`reconcile_required` outcome the call already knows about is never misreported as `OK`. The remaining two items are also closed: a new `sdk` CI step, "Assert the built wheel exposes the Workspace surface," installs the wheel the job's own build step just produced (`--no-deps`, over the editable install every earlier step in the job used) and imports `client.workspaces`'s sync sub-resources and the async client's three async ones from it, plus inspects the wheel's file listing for `caliber_sdk/models/workspace.py`/`caliber_sdk/aio/projects.py` -- catching a build-config regression (a module dropped from the wheel's `packages` list, an export removed from `__all__`) that every editable-install-backed check in the job would otherwise miss. And `docs/sdk/beta.md` gained a "Compatibility and deprecations" section (between "Workspace release governance" and "The agentic loop") documenting the two backward-compatibility mechanisms `P6-A`/`P6-B` shipped in code but never surfaced in the SDK's own docs: `ProjectsAPI.update(status=...)`'s deprecation in favor of `archive()`/`restore()`, and `project_scope()`/`client.projects` as compatibility aliases for `workspace_scope()`/`library_scope()`/`client.workspaces`. | `P6-B` | New `AsyncProjectImportsAPI`/`AsyncProjectReleasesAPI`/`AsyncProjectReleaseOperationsAPI` signatures match their sync counterparts exactly (parameter name/kind/default/annotation, asserted by an extended `test_async_project_public_signatures_match_the_sync_surface`); a new behavioral test exercises every method on all three; the new example's own test runs the full governed path end to end against a mock transport; docs contracts pass; `sdk/caliber-cli/tests/test_workspace_cli.py` exercises every `caliberctl workspace` leaf against a stubbed transport, parametrized over every Change-Request and release status the server defines, asserting each of the five required non-success states its own exit code rather than a collapsed `FAILURE`; `ruff`/`mypy --strict`/`pytest`/wheel build all pass for `caliber-cli`; the new `sdk` CI step installs the real built wheel and confirms the Workspace surface imports from it, not just the editable install; docs-generation/contract tests pass for the new compatibility section |
| `P7-A` | Data/operations | **Dependency satisfied, not started (deliberate).** `P6-C` and `P4-E` are both delivered (P4-E fully, except the one explicitly deferred status-publication item), so this row's stated dependency no longer blocks it. Not started anyway, by explicit repo-owner decision: unlike every other phase in this plan, `P7-A`'s own scope -- "production-like inventory," "backup/restore," "baseline revisions" against real data -- is an operational rollout procedure to run against an actual production-like environment and real data this repo does not yet have, not a coding task a development session can complete or usefully simulate. Phases 0 through 6 (including all of `P4-E`) are considered code-complete as of this plan's current state; `P7-A`/`P7-B` remain the team's own rollout process to execute when a real deployment target exists, not further autonomous implementation work. Production-like inventory, exception resolution, baseline revisions, provider-event recovery, backup/restore and feature flags | `P6-C`, `P4-E` | Zero unexplained ownership/collision rows; provider reconciliation restores missed state; disabling flags stops new effects without hiding history |
...
`P7-A` dependency check (updated after `P4-E`'s connection-storage,
webhook-ingress, and reconciliation slices): `P6-C` is delivered. `P4-E`
itself is now delivered except one item deliberately deferred as an
explicit, tracked follow-up (production status-publication wiring --
`github_source_control.py::publish_status()` is built and tested but
intentionally left unwired). Every other item this row's "still open"
list once named is now closed: production webhook ingress, durable
provider-payload ingestion, pagination/reconciliation, actor-link
revalidation (confirmed already satisfied by the existing data-access
layer), and first-package baseline procedure (confirmed already
satisfied by the existing pipeline) -- see `P4-E`'s own row for detail.
`P7-A`'s stated dependency is therefore substantively satisfied; this
note flags that for a deliberate decision to start Phase 7, rather than
starting it here.
...
2. Build a machine-readable inventory of every route, worker, Aria capability,
   ...
   **The action-closure slice is delivered** (`P0-B`): the route/scope
   inventory derives each `project_role`-classified route's `action` literal
   (`x-caliber-required-scope.action`) and asserts that set is closed against
   `resource_access.py::PROJECT_ACTIONS` — the actual enforcement registry
   `decide_project_access()` checks against. The original P0-B baseline had
   5 live actions; subsequent P1-D/P1-F/P3-A/P4-C slices expanded the current
   inventory to 14 live actions (`read`, project lifecycle/member actions,
   `source.manage`, `revision.import`, the runtime/publish/execute/feedback
   actions, `rework.update`, and `environment.manage`) with 15 declared
   actions still reserved. `tests/test_route_scope_inventory.py::test_every_live_project_role_action_is_a_known_action`
   fails by name if a future route ever requires an action string outside
   `PROJECT_ACTIONS` (an unrecognized action would otherwise be silently
   absent from every role's permission set), and
   `test_the_live_vs_reserved_action_partition_is_pinned` ratchets the
   current live/reserved split. This remains distinct from section 2.4's
   larger future Workspace action contract, whose remaining Change Request,
   release, version, and evidence actions are owned by later phase slices.
3. Inventory every table's project FK, nullability, visibility and uniqueness,
   ...
   **The project-FK/visibility slice is delivered** (`P0-A`, section 16):
   ...
   — which holds cleanly across all 85 models today. Both follow-ups this
   item once named as "not started" are now closed: the full
   nullability/uniqueness audit by `P2-A` slice 10, and the "bare-name
   resolvers" inventory (the missing-DB-constraint gap that same audit
   found) by `P2-A` slice 11 — see that row.
...
6. Freeze the SDK-facing contracts: list pagination shape, error envelope and
   ...
   streaming behavior.
   **The error-envelope slice is delivered** (`P0-B`): the transport-level
   shape — `{detail, status_code}` for a generic `HTTPException`, `{detail,
   status_code, errors: [{loc, msg, type}]}` for a request-body validation
   failure — is already implemented (`routes/_errors.py`), locked by 4 tests
   (`tests/test_routes_errors.py`), declared as OpenAPI component schemas
   (`Error`, `ValidationError` in `routes/openapi.py::_components()`,
   referenced by every `_SHARED_RESPONSES` entry), and already rendered into
   the published REST API reference's "Shared JSON shapes" table — no code
   changed to reach any of that; this item ratifies it as frozen. **Reason
   codes are now a real, additive transport field, closed for a first
   slice**: `routes/_errors.py::CaliberHTTPException` extends
   `HTTPException` with an optional `reason_code` attribute, and
   `http_exception_handler` renders it into the envelope only when set --
   a bare `HTTPException` (still the overwhelming majority of call sites)
   renders byte-for-byte unchanged, so every existing `detail`-parsing
   caller keeps working. The `Error` OpenAPI component documents
   `reason_code` as optional (not added to `required`, so an unmigrated
   route's response stays conformant with its own declared schema). The
   SDK's exception hierarchy (`sdk/caliber-sdk/src/caliber_sdk/errors.py`)
   gained a `reason_code` attribute on `CaliberAPIError` (populated by
   `error_for_response` when the payload carries one, `None` otherwise) and
   a new `CaliberPreconditionError`, now mapped from `412` (previously
   unmapped entirely, silently falling back to a bare `CaliberAPIError`) --
   closing out section 13.6's `409`/`412` table row. `ErrorBody` (the SDK's
   separate typed-decode view over a raw error body) grew the same optional
   `reason_code` field for callers who decode a body directly instead of
   catching the exception. `routes/workspace.py::snapshot_revision`'s three
   already colon-prefixed managed-snapshot-adapter failures
   (`resource_type_adapter_unavailable`, `resource_resolve_failed`,
   `resource_snapshot_failed`) were migrated to set `reason_code` explicitly
   alongside their unchanged `detail` strings, proving the field end-to-end
   (route -> envelope -> SDK exception) against a real, already-merged
   failure family rather than a synthetic example. **Explicitly left as a
   distinct, named follow-up, not done here**: every other route across the
   repo -- the large majority, including this same file's other
   `HTTPException` call sites (`archived_workspace`, the IntegrityError-replay
   `workspace_revision_snapshot_conflict`, `workspace_release_operations.py`'s
   `_conflict()` helper, and everything outside `workspace.py` entirely) --
   still raises a bare `HTTPException` with `detail` only and no
   `reason_code`. Closing that needs either a repo-wide migration or a
   decision to derive `reason_code` generically (e.g. parsing existing
   colon-prefixed `detail` strings) for every remaining call site, plus a
   taxonomy/naming decision for routes with no existing stable-code
   convention at all -- a materially larger, separate undertaking than this
   slice, left as a distinct future follow-up (no new planning-handle ID
   invented, matching how PR #279 and the agent-registration scope widening
   it ratified were both left as unbundled follow-ups).
   ...
   **Decided (repo owner's standing scope, applied by analogy):** the
   ~27 pre-existing, non-Workspace list routes' six-shape inconsistency is
   legacy surface this plan's own Phase 0-6 scope never owned — every new
   Workspace list method already ships on the ratified `CursorPage[T]`
   contract (`P4-C`, `P6-B`), which is what this document is actually
   accountable for. Reconciling the ~27 older routes into one shape remains
   exactly what this row already called it: a distinct, later, general
   API-consistency initiative, not a Workspace deliverable — left open on
   purpose, the same way Phase 7's rollout procedure is left open on
   purpose.
   ...
   **ETag/CAS/idempotency: today's real mechanisms are ratified as the
   ...
   and `/services` invocation. Those legacy mechanisms are `409`-based against
   a body field, not header-based. P4-C source routes now read `If-Match` and
   declare/emit `412` for stale or missing validators
   (`tests/test_route_scope_inventory.py::test_source_etag_routes_declare_412_response`
   pins this), but no general Workspace idempotency ledger exists — confirmed
   ...
   **Multipart field names are ratified as frozen**: the `metadata`+`bundle`
   ...
   the frozen contract; P4-C now implements the first durable multipart push
   route against the repository/commit/bundle subset. The metadata field and
   full SDK parity remain later work in Phase 4/6.
...
1. Add project slug, source mode, archive provenance, audit correlation, and
   ...
   **Decided, not built** (follow-up slice after `0106`): the Workspace
   idempotency ledger this ticket's own PR-sequence row also names.
   Investigated rather than deferred again: section 9.2's
   `caliber_workspace_idempotency_records` design adds three things over
   today's per-table columns (`WorkspaceImportJob.idempotency_key`,
   `WorkspaceRelease.request_idempotency_key`,
   `WorkspaceReleaseOperation.idempotency_key`, all already shipped,
   unique-constrained per table) — cross-resource-type key uniqueness, a
   canonical request-digest comparison that replays on match and returns
   `409 idempotency_conflict` on mismatch, and an `in_progress`/`completed`
   status for concurrent-claim detection. Checked against the actual
   design text and the actual code: the design's own uniqueness key is
   `(project_id, action, idempotency_key)` — scoped *by action*, i.e. by
   table, not global — so "cross-resource-type" uniqueness is not
   something the design itself provides either; every shipped per-table
   column already implements the same select-or-409 replay/conflict
   comparison against the caller's original request fields
   (`workspace_release_service.py::create_workspace_release` is the
   clearest instance); and every action that currently requires
   `Idempotency-Key` creates exactly one new row, so a natural per-table
   home for the key has existed for each of them so far. The one thing a
   shared ledger would add that per-table columns structurally cannot —
   idempotency for an action that *mutates* existing state rather than
   creating a new row (e.g. a future approve/submit transition with no
   natural owning insert) — has no shipped consumer yet, and today's
   mutations are already guarded by CAS (`If-Match`/`expected_etag`/
   `manifest_hash`), a different, already-real mechanism for exactly that
   case. Building the ledger now would be scaffolding for a consumer that
   does not exist, which `CLAUDE.md` names directly ("don't design for
   hypothetical future requirements"). Conclusion: **not built**. Revisit
   if/when a real action needs idempotency across a mutation with no
   owning row, or if a genuine cross-action key-uniqueness requirement is
   ever specified (today's design text does not specify one).
...
5. Replace free-form action strings with a closed `WorkspaceAction` registry,
   ...
   **Delivered** (`P1-D`): `resource_access.py::PROJECT_ACTIONS` now carries
   ...
   At the time, still reserved, not wired: `resource.write.evidence` onto
   the real Prompt/Workflow/Tool/Skill/Test-set/Judge/Scorer CRUD routes
   (section 2.3's own warning that isolation closure is a hard
   prerequisite) and `rework.update` (`caliber_rework_tasks` has no
   ...
   **Delivered** (`P2-P`): both `resource.write.runtime` and
   `resource.write.evidence` now gate the real routes for all 6 resource-
   family modules this item's own deferred note named -- `routes/prompts.py`,
   `routes/workflows.py`, `routes/tools.py`, and `routes/skills.py` under
   `.runtime`; `routes/eval_datasets.py` and `routes/judges.py` (this
   codebase's "scorer": its own docstring calls a judge "a reusable,
   operator-authored scorer", there being no separate `CaliberScorer`
   model) under `.evidence`. Every call site uses
   `require_project_access_if_scoped`, a no-op for an unscoped
   (`project_id is None`) resource, composed with each route's
   pre-existing global-scope floor rather than replacing it. See the
   `P2-P` row in section 16 for the full route list, the create-vs-update
   project-resolution rule, and the `scope_inference.py` classification
   note for `judges.py::update_judge`. `rework.update` remains reserved
...
7. Add optional PAT project binding and identity credential context; require
   ...
   **Delivered** (`P1-E`): `caliber_personal_access_tokens` gained a nullable
   `project_id` (migration `0094`; bare string, no FK, matching the same
   "optionally project-scoped" columns `CaliberWorkflow`/`CaliberEvalDataset`/
   `CaliberReviewQueue` already carry). `CaliberIdentity` gained
   `credential_kind` (`"session"` | `"pat"` | `"trusted_header"`),
   `credential_id`, and `credential_project_id`, populated by
   `auth.py::_session_user`/`_resolve_user` as a request's identity resolves.
   `POST /auth/tokens` accepts an optional `project_id`, validated against a
   real project role on issuance (`routes/auth.py::_validate_pat_project_binding`,
   reusing `require_project_access`'s own 404-for-hidden-or-nonexistent
   check with the `"read"` action -- granted to every role) rather than
   silently minting a permanently-inert credential; rotation preserves the
   binding. `auth.py::resolve_identity` enforces it centrally: a
   project-bound PAT is refused (403) for any request naming a *different*
   project via the `X-CALIBER-Project` header or a `{project_id}` path
   segment, and is a no-op when neither channel names a project at all.
   "Require project-bound PATs for CI import" is now also delivered, once
   `P4-C`'s CI-import route existed to enforce it on:
   `routes/workspace.py::_require_project_bound_pat_for_import` runs in
   `create_import` and `reconcile_import` (the two writes on the
   `/revision-imports` surface that durably mutate import state) right after
   `resolve_identity`. It is a no-op for `credential_kind != "pat"` --
   section 4's workflow table allows *either* an interactive "Developer"
   session *or* "CI automation user with project-bound PAT," so a
   session/trusted-header caller is unaffected -- and it refuses (403) a
   PAT whose `credential_project_id` does not equal the import's target
   `project_id`, which now also covers the case `resolve_identity`'s
   generic check does not: an *unbound* PAT (`credential_project_id is
   None`). Section 9.1's "new CI import tokens must be project-bound" only
   has teeth if an unbound PAT cannot substitute for a bound one on this
   surface, so that case is refused here specifically, while the same
   unbound PAT remains valid everywhere else `P1-E` already covers (reads,
   non-import writes). Not delivered: the "resource owner" and "persisted
   worker context" conjuncts section 9.2 also names -- neither has a
   modeled check yet (no per-resource pinning, no worker-identity context --
   Phase 2/5's job); enforcing those now would be dressing up an unbuilt
   no-op as a real check.
8. Implement `authorize(...)` per section 5.4 with stable reason codes.
   **Delivered** (`P1-B`, slice 1): `resource_access.py::authorize(session,
   principal, action, workspace_id, *, resource=None, environment=None,
   release=None)` implements section 2.4's effective-decision AND-chain for
   the conjuncts this codebase has data for today — the "authenticated
   principal"/"credential ceiling" conjuncts are already satisfied upstream
   by `require_scopes`/`require_all_scopes` before this function runs; the
   "resource belongs to workspace," "environment policy," and
   "release-instance rules" conjuncts have no `ResourceContext`/
   `EnvironmentContext`/`ReleaseContext` type to check against yet (no
   per-resource pinning model, no environment-scoped route, no
   release/Change-Request model — Phase 4/5's job), so passing a non-`None`
   value for any of them raises `NotImplementedError` naming the unmodeled
...
9. Preserve `require_project_access` as a compatibility wrapper over the new
   ...
   **Delivered** (`P1-B`, slice 1): `require_project_access` now calls
   `authorize()` instead of `decide_project_access` directly; its own
   signature, 404/403 behavior, and every existing call site are unchanged.
   `decide_project_access` itself is untouched and remains directly usable.
...
12. Remove the platform-admin-to-owner shortcut from ordinary decisions and
    ...
    **Delivered** (`P1-B`, slice 1): `resource_access.py::project_role()`'s
    `if identity.has_scope(SCOPE_ADMIN): return ROLE_OWNER` branch is
    removed — `caliber.admin` is no longer an implicit workspace role,
    matching section 5.4 exactly. `routes/projects.py::list_projects`'s own
    coupled admin-sees-all-projects filter is fixed in the same PR (it
    would otherwise crash with an uncaught `404` the moment the shortcut
    alone was removed) — the owner/member filter now applies to every
    caller, admin included.
    `routes/files.py::staging_upload`'s missing-active-project fallback
    ...
    **Bare fail-closed deny, by this ticket's own explicit design**: no
    ...
    Two other admin-bypass patterns exist elsewhere
    (`db/scoping.py::apply_visibility_filter`'s visibility-tier bypass,
    `assistant/executor.py::_authorize_answer`'s Aria-plan-level bypass) —
    a different axis each, **not** touched by this item, which is scoped to
...
1. Apply the Phase 0 inventory; replace bare child lookup with authorized parent
   ...
   **Delivered (partial, slice 2, `P2-H`).** The repo-wide sweep `P2-A`
   deferred, done systematically this time: every `session.get(<Model>,
   ...)` across `routes/*.py` for a `db/resource_inventory.py`-classified
   visibility/project-scoped model, cross-referenced against what's
   already fixed. Closed the highest-severity cluster found:
   `routes/evaluations.py::create_evaluation`'s subject/dataset
   resolution -- `_resolve_subject`'s `skill` branch (its `workflow`
   branch sibling, `_build_workflow_predict`, was already fixed and is
   the reference pattern this reuses), `_hydrate_judge_runners`, and the
   dataset lookup itself (which had only a project-*role* check,
   `require_project_access_if_scoped`, not the separate visibility check
   -- a `visibility="user"` dataset owned by a project-mate, or a
   personal `project_id is None` dataset owned by someone else, was
   wrongly admitted). All three now resolve through `get_visible`, the
   same primitive every other fix in this item uses. **Not yet
   delivered:** the sweep found several more genuine gaps of the
   identical shape, deliberately left as separate follow-up slices (each
   its own file/resource-family, to keep this slice bounded the same way
   `P2-A` was): `routes/eval_datasets.py`'s 6 mutation routes
   (`create_example`/`create_example_from_trace`/`revise_example`/
   `sync_dataset_to_mlflow`/`restore_dataset_version`),
   `routes/prompts.py::enqueue_prompt_optimization_run`'s own dataset
   lookup (the same shape as `create_evaluation`'s, in a different
   file), `routes/workflow_calibration.py` (no visibility import at all),
   `routes/rollback.py` (existence-only target check backing a
   destructive rollback), and `routes/skills.py::bind_skill`'s
   `kind="agent"` branch (the *target* agent_id, not the already-checked
   skill).
   **Delivered (partial, slice 3, `P2-I`).** The first of slice 2's
   named follow-ups: `routes/eval_datasets.py`'s 6 mutation routes.
   `create_example`, `create_example_from_trace`, `revise_example`,
   `sync_dataset_to_mlflow`, and `restore_dataset_version` all looked
   their dataset up with a bare `session.get` -- `get_dataset`/
   `list_examples` in the same file were already correct and are the
   reference pattern reused here. `sync_dataset_to_mlflow`'s check runs
   before its MLflow write, the same ordering `routes/prompts.py::
   create_prompt` established, so a refusal never lets another project's
   dataset content reach MLflow first. `update_dataset`/
   `supersede_example` are deliberately untouched: `SCOPE_ADMIN`-only,
   and admin already bypasses visibility unconditionally, so wrapping
   them would change nothing. **Not yet delivered:** the other 4 gaps
   slice 2 named (`prompts.py::enqueue_prompt_optimization_run`,
   `workflow_calibration.py`, `rollback.py`, `skills.py::bind_skill`).
   **Delivered (partial, slice 5, `P2-K`).** `routes/workflow_calibration.py`
   -- imported neither visibility primitive at all. `get_options` and
   `enqueue_workflow_calibration_run` (the `create_run` route's helper)
   both looked their workflow up with a bare `session.get`, and the
   helper did the same for the target agent. `identity` is optional on
   the helper: the REST route (`create_run`) and the Aria capability
   (`assistant/capabilities.py::_workflow_calibrate`, via
   `ctx.identity()`) both now pass a real one; `assistant/service.py`'s
   older intent-plan dispatch path has no per-turn identity/project-id
   concept at all yet (the same already-deferred item-5 plumbing gap
   named elsewhere in this document), so it still passes `None` and gets
   today's unscoped lookup, unchanged -- named explicitly at that call
   site rather than silently left as-is. **Not yet delivered:** the other
   2 gaps slice 2 named (`rollback.py`, `skills.py::bind_skill`).
   **Delivered (partial, slice 4, `P2-J`).** The second of slice 2's
   named follow-ups: `routes/prompts.py::enqueue_prompt_optimization_run`'s
   own eval-dataset lookup -- the identical shape
   `create_evaluation`'s (`P2-H`) had, in a different file. Closed the
   same way, reusing the `identity` parameter this function already
   gained in `P2-G`. **Not yet delivered:** `workflow_calibration.py`,
   `rollback.py`, `skills.py::bind_skill`.
   **Delivered (partial, slice 6, `P2-L`).** `routes/rollback.py` now
   resolves agent targets through `get_visible` for checkpoint listing and
   destructive rollback, and scopes its legacy skill-name fallback through
   `apply_visibility_filter`. `routes/skills.py::bind_skill` resolves the
   requested agent through `get_visible` before mutating its optimizer
   configuration. The legacy `assistant/service.py` intent-plan dispatch
   identity gap remains a separate, larger plumbing task.
   **Delivered (partial, slice 7, `P2-M`).** Route-backed assistant turns
   now retain the resolved `CaliberIdentity` through skill selection,
   capability dispatch, and intent-plan execution. Workflow calibration and
   prompt optimization adapters pass it to their existing visibility-aware
   ...
   **Delivered (partial, slice 8, `P2-N`).** The legacy registry dispatcher
   ...
   an identity retain their compatibility behavior. **Stale trailer corrected:**
   this item previously said the prompt-provider lookup and MCP bindings
   were still open -- both are resolved (`P2-G`/`P2-N`/`P2-O` for the prompt
   lookup, `P2-O`'s migration `0096` for MCP bindings; see the `P2-B` row's
   own correction of the identical stale note).
   **Delivered (documentation/test reconciliation slice, `P2-K`/`P2-L`).**
   The "legacy `assistant/service.py` intent-plan dispatch identity gap"
   slice 5 (`P2-K`) named and slice 6 (`P2-L`) repeated as a still-open,
   separately-deferred plumbing task was already closed by slice 7
   (`P2-M`, commit `ff390c39a731`) the day after `P2-K` was authored --
   `execute_intent_plan` gained a real `identity` parameter and threads it
   into `_execute_workflow_calibration` -> `enqueue_workflow_calibration_
   run`, the exact call site `P2-K`'s paragraph above quoted as "still
   passes `None`." Neither `P2-K`'s nor `P2-L`'s row text (nor this
   helper's own docstring) was ever updated to say so, so the gap read as
   open long after the code closed it -- the same "nobody updated this
   row's own trailer" pattern already found and fixed on `P2-B`'s row.
   This slice reconciles both: the `P2-K`/`P2-L` table rows now read
   **Delivered**, the stale docstring on `enqueue_workflow_calibration_run`
   is corrected, and `caliber/tests/test_assistant_intent_plan_visibility.
   py` adds the missing end-to-end proof -- driving the live `POST
   .../plans/execute` route (not the private adapter directly) against a
   hidden workflow and a hidden agent behind a visible workflow, and
   asserting no verification item or refinement job is queued either way.
2. Require project IDs for new project-owned root records; retain explicit
   personal/public catalog paths.
   **Delivered (partial, slice 9).** JSON and ZIP skill-package imports now
   derive owner, project, and visibility from the authenticated identity. An
   active workspace therefore creates a project-owned skill, while an import
   without workspace context takes the explicit personal-library path;
   package metadata cannot choose a different owner. Other root-creation paths
   and the full nullability/uniqueness audit remain open pending their own
   bounded slices.
3. Add missing indexes and FKs where migration evidence permits.
   **Delivered (partial, `P2-F`).** The indexes half: a pass over
   `db/resource_inventory.py`'s classification found 11 "visibility"
   tables (`project_id` + `visibility`, queried everywhere through
   `db/scoping.py::apply_visibility_filter`) and 4 "project_only" run/file
   tables (`project_id` alone) with no index at all covering `project_id`
   -- every project-scoped list/detail query on these tables was a full
   table scan filtered in memory. Migration `0095` (additive only) adds a
   `(project_id, visibility)` composite index to the 11 visibility tables
   and a plain `(project_id,)` index to the 4 project_only tables; 3
   visibility tables that already carried a `(project_id, status)`
   composite (`caliber_knowledge_bases`/`caliber_release_candidates`/
   `caliber_workflow_benchmark_reports`) are left untouched. The orphan half
   is now delivered: `db/legacy_data_report.py::orphan_project_id_report()`
   groups every non-null `project_id` that has no matching
   `CaliberProject`, including deleted rows, using the same inventory-derived
   model set and stable model/id ordering. **Not yet delivered:** the FKs half.
   No `ForeignKey` was added on any `project_id` column -- an FK on a column
   that may already carry stray/legacy values (never enforced before now)
   would either fail to apply outright or silently corrupt the affected rows.
   Applying FKs remains a separate migration decision after this report is run
   against representative production-like data and every orphan is classified
   or repaired.
4. Add a CALIBER prompt binding and workspace/provider namespace strategy.
   **Delivered (partial, `P2-G`).** No `CaliberPrompt` model exists --
   a "prompt" is purely an MLflow Prompt Registry entity (flat, globally
   named), so there is no `project_id` column to add one to. The only
   CALIBER-side data touching a prompt is a hidden runtime target (a
   `CaliberAgentConfig` row keyed by `agent_id == prompt_name`,
   `prompt_targets.py::ensure_prompt_target`), which already carries
   `project_id`/`visibility` -- but the get-or-create/read paths never
   consulted them: whichever project registered/tested/bound a name
   first silently owned that target forever, any later caller (any
   project) could reuse or mutate it, `GET /prompts/{name}` returned the
   full template body to any authenticated user regardless of the
   target's visibility, and `GET /prompts` merged MLflow's unfiltered
   global prompt search into every response. `ensure_prompt_target` now
   takes an optional `identity` and refuses (404) when an existing
   target isn't visible to the caller (mirroring `PlanService.get_plan`'s
   identical optional-identity convention); `create_prompt` checks
   *before* its MLflow write so a refusal never leaves an orphaned
   ...
   **`CaliberReleaseOperation`'s own gap (item 6) is now delivered, see
   `P2-R`:** migration `0112` added a `project_id` column, backfilled from
   the referenced prompt's hidden target exactly as anticipated below, and
   scoped its list/resolve reads -- as a bare `project_only` binding (no
   `visibility`/owner column), not the full 3-tier `SCOPING_VISIBILITY`
   shape, since this is a derived/historical record rather than a
   first-class owned resource; see `P2-R`'s own row for the full design.
5. Make workflow compiler and runtime resolvers project- and revision-aware for
   ...
   **Delivered (partial, `P2-B`).** Slice 1: the compile/promote pipeline's
   remaining bare-name/id lookups -- `workflows/deployment_bundle.py::build_deployment_bundle`'s
   eval-dataset, skill, and knowledge-base resolution, plus
   `workflows/promoter.py`'s `_skill_contents_for` (skill content) and
   `evaluate_deploy_gates` (deploy-gate dataset) -- all now resolve through
   `apply_visibility_filter`/`get_visible`, scoped to a new
   `db/scoping.py::synthetic_identity(owner, project_id)` built from the
   workflow's own recorded owner/project, since there is no live requester
   identity at compile/promote time. Slice 2: the *tool* resolver had the
   same gap in a wider blast radius -- `workflows/promoter.py::resolver_from_session`
   (the compiler/runtime tool-registry builder) queried every tool globally
   with no visibility filter at all, from 12 call sites across
   `promoter.py` (compile/build-plan/promote), `orchestrator/workflow_stages.py`
   (candidate/calibration/eval refinement stages) and `routes/workflow_versions.py`
   (validate/propose-patch/copilot-edit/plan-build/export-python). It is now
   scoped at every call site: routes pass the real request identity
   (`resolve_identity(request)`); background/compile paths pass a synthetic
   one built from the relevant workflow (`promoter.py`'s pre-existing
   `build_workflow_identity`, reused rather than duplicated further --
   `_skill_contents_for` and `evaluate_deploy_gates` were also switched onto
   it, retiring the slice-1 `synthetic_identity`-based helper from
   `promoter.py` since this file already had its own canonical version).
   `routes/workflows.py`'s separate, independently-reinvented
   `_scoped_tool_resolver` (which had the visibility filter but silently
   dropped the non-archived-status filter and successor-tool resolution
   `resolver_from_session` already had) is retired in favor of the one
   canonical builder, closing the drift between them. Slice 3 ("run queue +
   plan executor"): the subworkflow branch of `build_deployment_bundle` and
   of `build_plan`'s `_resolve_version` closure both resolved a child
   `workflow_id` with no visibility check -- the run-time gap was the more
   severe of the two, since it meant a manifest naming another project's
   workflow_id would actually **execute that project's graph** when a run
   worker replayed it; `build_plan`'s `_enqueue_review` closure (backing the
   `review_queue_enqueue` runtime node) trusted its caller to have already
   checked queue visibility -- the runtime path never had, so the check now
   lives inside `routes/review_queues.py::add_review_items_records` itself,
   covering the route, Aria, and runtime callers at once;
   `orchestrator/workflow_stages.py::_dataset_inputs` and
   `workflows/calibration.py::_resolve_active_dataset` had the identical
   unscoped dataset-by-name lookup already fixed in `evaluate_deploy_gates`,
   closed the same way; and the Aria plan worker
   (`assistant/executor.py::poll_waiting_plans`) resumed every parked plan as
   `actor="@system"`/`project_id=None` regardless of the plan's own recorded
   owner/project, silently dropping the project tier for every capability a
   background-resumed step touched -- it now resumes as the plan's own
   identity. Slice 4 (the refinement-worker pipeline): every stage between
   triage and eval had the identical skill-by-name/agent-enumeration gap --
   `orchestrator/triage.py::_classify`'s explicit skill-hint lookup and
   `_find_tool_skill`'s implicit tool-use attribution; `orchestrator/evidence.py::_collect`'s
   skill-content-plus-affected-agents context for the diagnosis stage;
   `orchestrator/candidate.py::run_candidate`'s skill-metadata lookup and
   affected-agent enumeration fed to the LLM, plus `_load_trainset`'s
   DSPy trainset dataset resolution (`_DEFAULT_EVAL_DATASET`, the name most
   likely to collide across projects); and `orchestrator/eval_stage.py::_resolve_baseline_content`'s
   skill-content read for the eval replay's baseline. All now scoped via a
   new `db/scoping.py::build_agent_identity(session, agent_id)` (mirroring
   `build_workflow_identity`, for the sites that only have an `agent_id`
   string) or `synthetic_identity(agent.owner, agent.project_id)` directly
   (for the sites that already have the agent row). `artifact_store.py::ArtifactStore.get_active_skill`
   -- the skill-content read both `candidate.py` and `eval_stage.py` call
   through -- gained a required `identity` parameter on the protocol and
   both implementations (`MLflowArtifactStore`, `FakeArtifactStore`), since
   the content read was the same gap one layer below the metadata lookup;
   `get_active_prompt` needed no change (agent ids are globally unique
   primary keys, so there is no same-name collision to scope against).
   MCP bindings were unscoped (`CaliberMcpServer` had no
   `project_id`/`visibility` at all -- their schema migration is delivered in
   `P2-O`). Slice 5 (`assistant/agent_tools.py`, Aria's per-turn agentic tool
   surface): 13 call sites -- every read tool (`list_skills`/`get_skill`/
   `list_tools`/`list_workflows`/`get_workflow_manifest`/`list_workflow_runs`/
   `get_workflow_run`/`get_workflow_run_trace`) plus the mutate tools'
   dataset/workflow resolution (`run_quick_eval`, `create_eval_dataset`,
   `run_workflow`) -- queried globally with no visibility check, unlike this
   same file's own `_load_published_openapi_tools`, which already scoped
   correctly via `self._capability_context().identity()`. All 13 now reuse
   that identical per-turn identity. `_kb_identity` (every KB tool) built its
   own identity with no `active_project_id` *and* a hardcoded scope set
   regardless of the real caller -- replaced with the same
   `_capability_context().identity()` call, also correcting the scopes to
   the caller's real ones. `create_eval_dataset` now sets `project_id`/
   `visibility` on the row it creates (the bare default was permanently
   ...
   context. **Stale trailer corrected:** this item previously said "Still
   open: prompt-provider lookup has no CALIBER-side prompt resource to
   scope" -- resolved by `P2-G`'s now-delivered namespace-collision refusal
   (see the `P2-B` row's own correction of the same stale note) and further
   closed by `P2-N`/`P2-O`'s prompt-version-lookup visibility fix; nothing
   remains open here. **Delivered (partial, preview-context follow-up):** the
...
6. Scope files, evaluations, review queues, release candidates, plans, and all
   ...
   **Delivered (partial, `P2-A`).** Files/evaluations/review-queue/release-
   ...
   agree. **Release operations reads: delivered, `P2-R`.** Item 4's
   namespace decision (`P2-G`) removed the "prompts have no project
   binding" blocker this item's own text used to cite; `CaliberReleaseOperation`
   gained a `project_id` column (migration `0112`, backfilled from each
   operation's referenced prompt's hidden target) and its list/resolve
   reads are now scoped the same "project_only" way -- see `P2-R`'s
   slice-table row for the full design and why the full 3-tier
   `SCOPING_VISIBILITY` shape was deliberately not used here.
7. Route Aria capabilities and durable plan execution through the central
   authorization contract.
   **Delivered, `P2-E`.** Three independent gaps closed. First,
   `routes/aria_plans.py::execute_plan`/`poll_plan` only checked resource
   *visibility* (`PlanService.get_plan`) before running a project-scoped
   plan -- visibility grants any active project member (including a plain
   `viewer`) the ability to see the plan, but the routes never separately
   checked project *role*, unlike `routes/workflow_runs.py::create_workflow_run`,
   which already gates the equivalent action behind
   `resource_access.py`'s `resource.execute` (owner/editor/reviewer). Both
   routes now call `require_project_access` (via a shared
   `_authorize_plan_execution` helper) with the plan's own `project_id`
   right after the visibility check, closing the gap; a personal
   (`project_id is None`) plan is unaffected. Second,
   `assistant/agent_tools.py::_dispatch_capability` -- the synchronous
   per-turn tool-call path -- never consulted a capability's declared
   `required_scopes` at all; only the async plan executor
   (`PlanExecutor._run_step`) enforced that floor. Masked today because
   every registered mutate capability happens to require
   `caliber.operator`, which `routes/assistant.py::send_message` already
   requires for the whole turn -- but the declared contract itself was
   unenforced, so a future `approver`/`admin`-scoped capability would have
   silently run for anyone reaching `dispatch()` directly. Both call sites
   now consult the same `auth.scopes_for_user` primitive the executor
   already used. Third -- the durable-plan-execution half of this item,
   left open by the first pass -- a first-class scope/role decision is now
   recorded on the plan/step rows themselves rather than only re-derived at
   each call, mirroring `CaliberWorkspaceReleaseDecision.actor_role_snapshot`/
   `.effective_scope_snapshot`. Migration `0107` adds
   `CaliberAriaPlan.actor_role_snapshot`/`.effective_scope_snapshot`/
   `.authorization_recorded_at` (`_authorize_plan_execution` records these
   once, the first time it clears a plan to run; `actor_role_snapshot` is
   `None` for a personal plan, which has no project role to snapshot) and
   `CaliberAriaPlanStep.capability_scope_decision` (`PlanExecutor` records
   this at both places it already checks a capability's `required_scopes`
   against the plan owner: `_plan_next_action`'s early-admission check and
   `_run_step`'s own re-check immediately before dispatch). Neither
   snapshot replaces the live per-call check -- re-deriving authorization on
   every `execute`/`poll`/dispatch stays the safer default, since a
   still-active plan's authorization can genuinely change between calls (a
   role edit, a membership change); the snapshot exists purely so "under
   what authorization did this plan/step actually run" is answerable after
   the fact, the same judgment `CaliberWorkspaceReleaseDecision` applies
   alongside its own fresh CAS checks. `_dispatch_capability` deliberately
   gets no snapshot of its own: `AgentToolset` carries a session_id and an
   acting identity, not a plan_id, and this synchronous per-turn dispatch
   can run with no `CaliberAriaPlan`/`CaliberAriaPlanStep` row in existence
   at all, so there is no durable row to attach one to -- a boundary, not a
   remaining gap.
8. Make SDK and CLI automation send an explicit project scope for every
   workspace operation.
   **Delivered (partial).** A project-scope mechanism already existed
   end-to-end (env var/constructor → `Transport.project` → the
   `X-CALIBER-Project` header, plus a `project_scope()` context manager) --
   the gap was narrower than "build from scratch": `sdk/caliber_sdk/resources/projects.py`'s
   18 `/projects/{project_id}/...` call sites never pinned the header to the
   authoritative path segment, so a client ambiently scoped to one project
   asking about a *different* one by id silently sent its own scope instead.
   `Transport.request()`/`download()` (sync and async) gained a tri-state
   `project` parameter -- `UNSET_PROJECT` (the default, ambient scope
   applies), an explicit project id (pins the header regardless of ambient
   scope), or `None` (deliberately omits the header) -- and a mismatch
   between an explicit `project=` and a manually-supplied
   `X-CALIBER-Project` header on the same call now raises
   `CaliberConfigError` before any network I/O. All 18 `resources/projects.py`
   call sites now pass the path's own `project_id` explicitly. On the CLI
   side, `caliberctl` already had a global `--project`/`$CALIBER_PROJECT`,
   but nothing enforced it: a workspace-bound write with neither set
   silently produced an unscoped row (exactly what item 9's
   `legacy_null_report()` counts). The four commands with no other way to
   name a project (`workflow run`, `cookbook install`, `service publish`,
   `gate-verdict record`) now refuse with a usage error instead. The server
   also rejects a disagreement between `X-CALIBER-Project` and a
   `{project_id}` path segment before endpoint/database work, returning a
   `400` error whose detail starts with `workspace_context_mismatch`.
   **Delivered (follow-up slice, `P2-D`).** Ambient project selection now uses
   a per-transport `ContextVar` in both sync and async transports, and both
   clients' project-scope context managers restore with a token. Interleaved
   thread/task tests prove one caller cannot leak its project header into
   another caller sharing the client. **Delivered in this slice:** the async
   client now exposes `AsyncProjectsAPI` and `AsyncProjectFilesAPI`, which mirror the existing
   synchronous project surface: list/get/create/update, archive/restore/
   ownership transfer, member management, storage and environment operations,
   plus project file listing, multipart upload, folder creation, deletion and
   raw-byte download. Every path-named project operation pins the async
   transport's project header to that path segment, and the existing
   deprecated `update(status=...)` compatibility delegate is preserved.
   `test_async_projects.py` covers public signature parity, all typed routes,
   multipart/raw-byte behavior and ambient-scope override. **Still not
   delivered:** the broader async workspace tree for future
   source/import/revision/release resources.
...
15. Reserve one canonical SemVer per submitted request and implement the
    immutable version-tag repository. **Delivered as simplified:** Phase 5
    creates only the final `<version>` tag, on QA-backed acceptance
    (`accept_change_request`). The `<version>-rc.<generation>` QA-candidate
    tag this item originally specified for QA entry was never wired up and
    is a superseded design step: `create_version_tag(kind="qa_candidate")`
    remains callable but has no production caller, and no Change Request is
    ever moved to a distinct `qa_in_progress` status. Enforce
    greater-than-current accepted precedence; an out-of-date rebase may
    abandon and replace its active reservation, but never reuse an abandoned
    claim.
...
5. Make passing development and staging evaluations become `approved` under
   their predecessor policy. **Delivered as simplified:** QA entry does not
   create a candidate tag or move the Change Request to a distinct
   `qa_in_progress` status. Quality sign-off
   (`record_workspace_release_decision`) records a durable, digest-bound
   `go`/`no_go` decision against the `technically_approved` head and, on
   `go`, marks the QA release `approved`; the Change Request's own separate,
   explicit `POST .../change-requests/{id}:accept` route (`P4-D`) then
   derives that recorded decision and attempts the acceptance CAS, creating
   the final tag and settling the version claim. Production waits for fresh
   QA and Admin decisions. Machine or human rejection is terminal and
   creates a rework task.
...
1. **Delivered in `P6-A` foundation slice:** add tri-state per-request project
   scope to both transports and keep ambient scope `ContextVar`-backed.
2. **Delivered in `P6-A` foundation slice:** add `workspace_scope`,
   `library_scope` and compatibility delegates; expose `workspaces is projects`
   on both clients.
