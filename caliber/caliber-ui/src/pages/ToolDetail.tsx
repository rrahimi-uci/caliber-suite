...
export function ToolDetail(): JSX.Element {
  ...
  const toolQuery = useApiQuery(["tool", toolId], (s) => caliberApi.getTool(toolId!, s), {
    enabled: Boolean(toolId),
  });
  const usageQuery = useApiQuery(["tool-usage", toolId], (s) => caliberApi.getToolUsage(toolId!, s), {
    enabled: Boolean(toolId),
  });
  const sourceQuery = useApiQuery(["tool-source", toolId], (s) => caliberApi.getToolSource(toolId!, s), {
    enabled: Boolean(toolId),
  });
  ...
  const deprecateMut = useApiMutation(() => caliberApi.updateTool(toolId!, { status: "deprecated" }), {
    onSuccess: () => invalidate(["tool", toolId]),
  });
  const archiveMut = useApiMutation(() => caliberApi.archiveTool(toolId!), {
    onSuccess: () => invalidate(["tool", toolId]),
  });
  const saveMut = useApiMutation((payload: ToolUpdatePayload) => caliberApi.updateTool(toolId!, payload), {
  ...
  function updateEdit<K extends keyof ToolEditState>(key: K, value: ToolEditState[K]): void {
    ...
    <div data-testid="tool-detail">
      ...
      <h1 className="text-xl font-semibold text-gray-900">
        ...
        <span className="text-xs text-gray-400">
          v{tool.version} · {SIDE_EFFECT_BADGE[tool.side_effect_level] ?? tool.side_effect_level} ·{" "}
          {tool.status}
        </span>
      </h1>
      ...
      {isAdmin && (
        <div className="my-3 flex flex-wrap gap-2">
          ...
          {archiveMut.error && (
            <span data-testid="tool-archive-error" className="text-xs text-red-600">
        ...
        <section className="border-t border-gray-200 pt-3">
          ...
          <dl className="mt-2 grid grid-cols-1 gap-2 text-xs sm:grid-cols-2">
            <div>
              ...
              <dd data-testid="tool-id" className="flex items-center gap-1.5 font-mono text-gray-800">
                <span className="truncate">{tool.tool_id}</span>
                <CopyButton value={tool.tool_id} label="Copy tool ID" testId="copy-tool-id" />
              </dd>
            </div>
            <div>
            ...
            <div>
              ...
              <dd data-testid="tool-module" className="font-mono text-gray-800">{tool.module_path}</dd>
            </div>
            <div>
              <dt className="font-semibold text-gray-500">Callable</dt>
              <dd data-testid="tool-callable" className="font-mono text-gray-800">{tool.callable_name}</dd>
            </div>
            <div>
        ...
        <section className="border-t border-gray-200 pt-3">
          ...
          <div className="mt-2 space-y-2 text-xs">
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <label className="block">
                <span className="font-semibold text-gray-500">Side effect</span>
                <select
                  data-testid="tool-edit-side-effect"
                  value={edit.side_effect_level}
                  onChange={(event) =>
                    updateEdit("side_effect_level", event.target.value as ToolDefinition["side_effect_level"])
                  }
              ...
              <label className="block">
                ...
                <select
                  ...
                  value={edit.status}
                  onChange={(event) => updateEdit("status", event.target.value as ToolDefinition["status"])}
                  className="mt-1 w-full rounded border border-gray-300 px-2 py-1"
                >
            ...
            <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
              <label className="block">
                ...
                <input
                  ...
                  value={edit.owner}
                  onChange={(event) => updateEdit("owner", event.target.value)}
                  className="mt-1 w-full rounded border border-gray-300 px-2 py-1"
                />
              </label>
              <label className="block">
                <span className="font-semibold text-gray-500">Successor ID</span>
                <input
                  data-testid="tool-edit-successor"
                  value={edit.successor_tool_id}
                  onChange={(event) => updateEdit("successor_tool_id", event.target.value)}
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(event) => updateEdit("requires_approval", event.target.checked)}
              />
            ...
            <label className="flex items-center gap-2">
              <input
                ...
                onChange={(event) => updateEdit("allow_in_preview", event.target.checked)}
              />
            ...
            <div className="flex items-center gap-2">
              ...
              {saveMut.isSuccess && <span data-testid="tool-save-status" className="text-gray-500">Saved</span>}
              {saveMut.error && <span data-testid="tool-save-error" className="text-red-600">{saveMut.error.message}</span>}
            </div>
          </div>
        </section>
        )}
      ...
      <section className="mt-4 border-t border-gray-200 pt-3" data-testid="tool-implementation">
        <h2 className="text-sm font-semibold text-gray-900">Implementation</h2>

        {sourceQuery.data?.signature && (
          <div className="mt-2">
            <div className="text-xs font-semibold text-gray-500">Signature — what to pass when you run it</div>
            <CodeBlock
        ...
        {(tool.input_schema || tool.output_schema) && (
          <div className="mt-3 grid gap-3 lg:grid-cols-2">
            <div>
              <div className="text-xs font-semibold text-gray-500">Input schema</div>
              <CodeBlock
            ...
            <div>
              <div className="text-xs font-semibold text-gray-500">Output schema</div>
              <CodeBlock
        ...
        {sourceQuery.data?.doc && (
          <p className="mt-3 whitespace-pre-wrap text-xs text-gray-600">{sourceQuery.data.doc}</p>
        )}
        ...
        <div className="mt-3">
          ...
          {sourceQuery.data?.available ? (
            ...
            sourceQuery.data && (
              <div className="mt-1 text-xs text-gray-400" data-testid="tool-source-unavailable">
                Source unavailable{sourceQuery.data.error ? `: ${sourceQuery.data.error}` : "."}
              </div>
            )
          )}
        </div>
      ...
      <section className="mt-4 border-t border-gray-200 pt-3">
        ...
        <div className="mt-2 grid gap-3 lg:grid-cols-2">
          <div>
            <label className="text-xs font-semibold text-gray-500" htmlFor="tool-run-input">
              Input JSON
            </label>
            <textarea
              id="tool-run-input"
            ...
            <div className="mt-2 flex items-center gap-2">
              ...
              {runInputError && (
                <span data-testid="tool-run-input-error" className="text-xs text-red-600">
                  {runInputError}
                </span>
              )}
            </div>
          ...
          <div>
            ...
            {testRunMut.error && (
              <div data-testid="tool-run-error" className="mt-1 text-xs text-red-600">
                {testRunMut.error.message}
              </div>
            )}
            {testRunMut.data ? (
              <pre data-testid="tool-run-result" className="mt-1 max-h-64 overflow-auto rounded bg-gray-50 p-2 text-xs">
                {pretty(testRunMut.data)}
              </pre>
            ) : (
              <div data-testid="tool-run-empty" className="mt-1 rounded bg-gray-50 p-2 text-xs text-gray-400">
                No run yet.
              </div>
            )}
          </div>
      ...
      <div className="mt-3 text-xs">
        ...
        <pre data-testid="tool-agent-binding" className="mt-1 max-h-48 overflow-auto rounded bg-gray-50 p-2">
