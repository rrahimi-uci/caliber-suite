...
function renderDetail(): void {
  ...
  render(
    <QueryClientProvider client={qc}>
      <MemoryRouter future={{ v7_startTransition: true, v7_relativeSplatPath: true }} initialEntries={["/skills/SK-1"]}>
        <Routes>
...
describe("SkillDetail", () => {
  ...
  it("shows the skill content and version", async () => {
    ...
    renderDetail();
    expect(await screen.findByRole("heading", { name: "tool-grounding" })).toBeInTheDocument();
    expect(screen.getByText("v3")).toBeInTheDocument();
    // Overview is the default tab — the always-loaded summary (level 1) is shown there.
    expect(screen.getByText(/call the tool before asserting a fact/i)).toBeInTheDocument();
    // The full content (level 2) lives behind the Content tab.
    await userEvent.click(screen.getByRole("button", { name: "Content" }));
    expect(
      screen.getByText(/call the tool first/i),
    ).toBeInTheDocument();
  });
  ...
  it("switches between the Overview and Content tabs", async () => {
    ...
    expect(await screen.findByTestId("skill-package-panel")).toBeInTheDocument();
    expect(screen.queryByTestId("skill-content")).not.toBeInTheDocument();
  });
  ...
  it("edits content and reports the version bump", async () => {
    ...
    server.use(
      http.get(`${API_BASE}/skills/SK-1`, () =>
        HttpResponse.json(envelope(makeSkill({ version: captured.body ? 4 : 3 }))),
      ),
      http.get(`${API_BASE}/skills/SK-1/package`, () =>
        HttpResponse.json(envelope(makeSkillPackage())),
      ),
      http.patch(`${API_BASE}/skills/SK-1`, async ({ request }) => {
        ...
        captured.body = body;
        return HttpResponse.json(envelope(makeSkill({ version: 4, content: String(body.content) })));
      }),
    );
    renderDetail();
  ...
  it("edits owner and metadata JSON", async () => {
    ...
    server.use(
      http.get(`${API_BASE}/skills/SK-1`, () =>
        HttpResponse.json(
          envelope(makeSkill({ owner: "@caliber", skill_metadata: { openai_package: { resources: [] } } })),
        ),
      ),
      http.get(`${API_BASE}/skills/SK-1/package`, () =>
        HttpResponse.json(envelope(makeSkillPackage())),
    ...
    fireEvent.change(metadata, {
      target: { value: '{"openai_package":{"resources":[]},"reviewed_by":"@qa"}' },
    });
    await userEvent.click(screen.getByTestId("skill-save"));
  ...
  it("edits summary, description, category, lists, allowed tools, and status", async () => {
    ...
    await userEvent.selectOptions(screen.getByLabelText("Category"), "research");
  ...
  it("archives and restores a skill from the detail toolbar", async () => {
    ...
    server.use(
      http.get(`${API_BASE}/skills/SK-1`, () => HttpResponse.json(envelope(current))),
      http.get(`${API_BASE}/skills/SK-1/package`, () =>
        HttpResponse.json(envelope(makeSkillPackage())),
      ),
      http.patch(`${API_BASE}/skills/SK-1`, async ({ request }) => {
  ...
  it("blocks saving invalid metadata JSON", async () => {
    ...
    expect(await screen.findByText(/Metadata JSON is invalid/i)).toBeInTheDocument();
  ...
  it("blocks metadata values that are valid JSON but not an object", async () => {
    ...
    await userEvent.click(await screen.findByTestId("skill-edit-btn"));
    fireEvent.change(await screen.findByTestId("skill-metadata"), { target: { value: "[]" } });
    await userEvent.click(screen.getByTestId("skill-save"));
  ...
  it("shows save failures and lets cancel clear local form errors", async () => {
    ...
    fireEvent.change(screen.getByTestId("skill-metadata"), { target: { value: "{}" } });
    await userEvent.click(screen.getByTestId("skill-save"));
    expect(await screen.findByText(/Save failed: write denied/i)).toBeInTheDocument();
  });
  ...
  it("shows the OpenAI package preview and download link", async () => {
    ...
    expect(screen.getByText("tool-grounding/agents/openai.yaml")).toBeInTheDocument();
    expect(screen.getByText("tool-grounding/references/checklist.md")).toBeInTheDocument();
  ...
  it("shows package warnings and preview errors without hiding the download link", async () => {
    server.use(
      http.get(`${API_BASE}/skills/SK-1`, () =>
        HttpResponse.json(envelope(makeSkill({ tags: [], depends_on: [], allowed_tools: null }))),
      ),
      http.get(`${API_BASE}/skills/SK-1/package`, () =>
        HttpResponse.json({ detail: "package builder unavailable" }, { status: 500 }),
      ),
    );
    renderDetail();
    ...
    expect(await screen.findByText(/Package preview failed: package builder unavailable/i)).toBeInTheDocument();
    expect(screen.getByTestId("skill-package-download")).toHaveAttribute(
      "href",
      `${API_BASE}/skills/SK-1/package.zip`,
    );
  ...
  it("renders package warnings when the generated package is incomplete", async () => {
    ...
    expect(await screen.findByText(/Missing SKILL.md. Missing agent metadata./i)).toBeInTheDocument();
  ...
  it("imports a skill package and navigates to the new skill", async () => {
    ...
    server.use(
      ...
      http.get(`${API_BASE}/skills/SK-2`, () =>
        HttpResponse.json(envelope(makeSkill({ skill_id: "SK-2", name: "imported-skill" }))),
      ),
      http.get(`${API_BASE}/skills/SK-2/package`, () =>
        HttpResponse.json(envelope(makeSkillPackage({ root: "imported-skill" }))),
      ),
    );
    renderDetail();
    await screen.findByRole("heading", { name: "tool-grounding" });
    ...
    const input = screen.getByLabelText("Import skill package folder") as HTMLInputElement;
    ...
    const body = imported as { owner: string; files: Array<{ path: string; content: string }> };
