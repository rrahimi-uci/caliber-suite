...
\item \textbf{Calibration} --- no lease exists and no automatic replay occurs, so
      the row remains \code{running} until an operator records a reasoned
      \code{abandon} or \code{retry}. Assert that the original becomes terminal,
      a retry is a distinct lineage-linked row with the same immutable inputs, and
      a late worker result cannot overwrite the operator disposition.
\end{itemize}
