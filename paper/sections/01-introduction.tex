...
documents those obligations per family but does not yet enforce every declaration
mechanically (\secref{sec:arch:claim}).
\end{quote}

We defend the per-family capability factoring while treating its current
documentation-only enforcement as technical debt (\secref{sec:arch}). A prompt
...
\item \textbf{An execution model without a worker tier.} Claim-and-lease columns
on relational rows give durable queue arbitration with one deployable, no broker,
and no split-brain between broker and database --- together with the honest
converse, that in-memory rate limits become per-process and must be sized for the
worker count (\secref{sec:exec}).
...
\item \textbf{A reproducible measurement protocol and an honest results state.}
A protocol specified in enough detail to be re-executed independently
(\appref{app:protocol}), with every unpopulated cell marked as unmeasured rather
than estimated (\secref{sec:eval}).
\end{enumerate}
...
reimplementations~\cite{zaharia2018mlflow}. It ships a single environment with no
