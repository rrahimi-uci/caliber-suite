...
\para{The quantitative evaluation has not been run.} This is the paper's primary
...
\tier{gated} are correctly withheld from \aria{}'s synchronous tool projection. Two
existing hand-written tools that approve and publish drafts are classified
\tier{mutate}, so a permissive build-mode session advertises and can execute them.
The mechanism is sound; the classification is incomplete
