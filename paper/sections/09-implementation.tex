...
Peer packages at the repository root carry the remote Python SDK, the
zero-runtime-dependency plugin SDK, and the non-interactive CLI; that separation is
deliberate, so developer automation and third-party extensions do not need to
install the server's full MLflow/SQLAlchemy/Starlette runtime to make an HTTP call
or declare a plugin contract.
...
a separate paused draft, review-queue creation, and, added this session, the seeded
two-week-alpha journey (\code{two-week-alpha-journey.spec.ts}, trace through
diagnosis, candidate, evaluation, apply, and release/rollback) and the Workspace
import-to-promotion journey (\code{workspace-journey.spec.ts}).  The local CI mirror terminates the detached
