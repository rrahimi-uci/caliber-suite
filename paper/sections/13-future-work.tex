...
latter composes better with per-family release idioms. A separate, later,
multi-tenant ``Workspace'' initiative has since built environment-scoped
promotion with approval routing as its own distinct subsystem; it targets a
different, multi-tenant product rather than the single-tenant path described
here, so this item remains open work for \emph{this} path, not resolved by it.
...
family cannot violate. A first machine-readable registry now emits the nine-family
contract from \code{/capabilities} and tests every row's shape; it prevents UI and
documentation from inventing missing fields, but it does not yet prove each route
implements its declaration. The remaining work is a retrofit across
\statFamilies{} families and their routes, and it subsumes the registry projection
described next.

\para{Complete the capability registry.} Approval and publication are now gated in
the synchronous projection. Converging the remaining \statAriaTools{} hand-written
...
opposite of what this system is for. The shipped assistant-draft
\code{agent\_review}/\code{full\_autonomy} mechanism supplies the identity,
hash-binding, expiry, and fail-closed plumbing needed to run that experiment, but it
does not satisfy either empirical condition and therefore does not automate the
refinement-job \mode{Apply} path.
