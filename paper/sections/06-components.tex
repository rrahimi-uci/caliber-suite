...
\figref{fig:trust} gives the admission and authority path. Two entry classes have
...
First, \aria{} capabilities are risk-tiered, and capabilities declared \tier{gated}
are withheld from the synchronous tool projection and require a plan interaction.
The mechanism is sound; the protection is only as strong as the classification. Two
existing hand-written tools that approve and publish drafts are classified
\tier{mutate}, so a build-mode session with permissive settings advertises and can
execute them. This is a classification gap, not a mechanism gap, and we prefer to
name it than to describe the mechanism as though the classification were complete.
