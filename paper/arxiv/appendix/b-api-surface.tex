...
The shared \code{GET /capabilities} response makes that absence machine-readable:
its \code{artifact\_families} object declares history, live-target, promotion,
rollback, evidence, gate, and calibration facets for all nine families. A contract
test requires every row and field. This is stronger than a UI convention but weaker
than a type system that proves every route implements the declaration.

That machine-readable surface is now published in four aligned forms. The
management API serves its own OpenAPI document; the repository ships a lightweight
remote Python client SDK (\code{caliber-sdk}); a thin operator CLI
(\code{caliberctl}) over the same HTTP contract; and a separate plugin SDK
(\code{caliber-plugin-sdk}) for in-process extension points that are intentionally
not exposed through the remote management surface. The important property for this
paper is alignment rather than packaging: browser, OpenAPI, SDK, CLI, and external
automation all consume the same governed-asset management surface instead of a
private side channel, while extension authors get a versioned contract that does
not drag in the server runtime. The caveat is the same as above: typed access is a
stronger discovery surface than a UI convention, not a proof that every route
family has an equally mature wrapper.
...
Operators act on incomplete crossings through explicit recovery surfaces:
\code{GET /releases/operations}, global and background reconciliation for
\code{applying}/\code{reconcile\_required}, and exact retry or reasoned abandonment
for a provably pre-effect \code{prepared} row. Tool calibration follows a different
rule: a reasoned retry creates a new lineage-linked job because repeating authored
tool code automatically would be unsafe.
