...
\item \textbf{Remediation latency decouples from deployment latency.} The bound on
      ...
      not measured it.
\item \textbf{Rollback is a pointer move, not a redeploy.} Restoring the recorded
      target is a bounded database and registry operation rather than a code
      revert, and the target restored is the one the release \emph{recorded}
      rather than one inferred as ``the previous number.'' Rollback is itself a
      release operation, so it takes the same intent record and the same audit
      row as a promotion (\algref{alg:promote}).
\item \textbf{Authoring cannot move the pointer.} Registering a prompt version is
      a non-live operation: the shared registration helper rejects a request that
      would also set an alias, and directs the caller to the release service.
      That is what makes the two properties above hold for every in-repository
      call site rather than only for the path this paper describes.
...
\subsection{R4 in practice: one enforced gate, one human in refinement}
...
Within the refinement path, \sys automates diagnosis, candidate generation, evaluation, gating, evidence
...
Assistant drafts are outside that measured refinement chain. Their optional
\code{agent\_review} and \code{full\_autonomy} policies replace the draft-review
human with a separately scoped agent identity and, for release, a third service
identity. They retain attributable records but have no measured reviewer-quality
result; \secref{sec:limitations} therefore treats them as a scoped experimental
exception rather than evidence against D4.
...
lets a workflow be governed here and executed elsewhere.  The visual path includes
closed-vocabulary deterministic transforms and direct review-queue enqueueing, so
policy tables, schemas, confidence rules, fixtures, and review handoff do not require
custom Python.  A runtime-owned catalog exposes sixteen Cookbook examples; installing
one creates a paused workflow and one editable draft in a single audited transaction,
with prerequisites acknowledged before activation can be considered.
...
\para{Gateway, signoff, and operator automation.} The LLM Gateway is governed as a
first-class integration surface rather than a side panel: endpoint discovery,
guardrail attachment, per-model pricing, and trace-derived usage live beside the
same evidence and audit substrate the release path uses. The release-signoff path
adds typed candidates, weighted criteria, waivers, immutable go/no-go snapshots,
and durable report jobs whose output is Allure-compatible JSON bound to a
candidate hash. Those operational surfaces are exposed consistently across the SPA,
the served management API, the remote SDK, and the CLI, so scripted operator
automation does not depend on a browser-only flow.
