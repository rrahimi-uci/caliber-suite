...
*Read top-down: the stack, the canonical chain, the unit of governance, then the topologies.*
...
| **The canonical chain** | `Signal → Evidence → Candidate → Measurement → Decision → Release → Trace` — a reference shape for refinement- and release-capable families. Prompt and skill use the standard queued refinement path; workflow-manifest calibration enters at a workflow-specific stage but reuses durable job and apply machinery. Other families implement only the applicable facets. |
...
flowchart LR
  ...
  subgraph GA["◆ GOVERNED ASSET"]
    direction TB
    p1["<b>FULL LOOP</b><br/>―――――<br/><b>Prompt</b><br/><br/>Immutable MLflow registry<br/>versions behind <code>@prod</code><br/>Advisory gate verdict<br/>Audited promote records the<br/>outgoing alias target —<br/>rollback restores it<br/>Optimizer-backed calibration"]:::t1
    ...
    p3["<b>VERSIONED / MANAGED</b><br/>―――――<br/><b>Skill</b> · <b>Tool</b> · <b>Test set</b> · <b>MCP</b><br/><br/>Skill, tool, and test-set<br/>version forms; mutable MCP<br/>definitions with audit history<br/>No shared live-alias contract<br/>Skill rollback creates a new version"]:::t3
    p1 --- p2 --- p3
  end
...
flowchart LR
    ...
    c5["<b>DECISION</b><br/>a human accepts or rejects"]:::chain
...
Implemented paths leave durable state and evidence across the chain. A stage is
not necessarily a distinct table or row:
...
| Measurement | Job, regression, or evaluation records with scores and a gate decision; a per-version gate-verdict row only where that path writes one |
| Decision | Approval request recording who accepted and on what evidence |
...
flowchart TB
  ...
  subgraph R2[" "]
    ...
    n7["<b>Gate verdict</b><br/><br/>Path-specific pass/fail<br/>evidence — advisory"]:::cell
...
> or release paths.
>
> And the honest limits: a **gate verdict is advisory evidence, not an
> unbypassable cross-artifact policy**; guarantees are **asset-specific**, not
> uniform; and v1 ships a **single environment** with no `dev → staging → prod`
> ladder.
...
| Family | History & liveness | Gate | Release / rollback | Calibration idiom |
| --- | --- | --- | --- | --- |
| **Prompt** | Immutable MLflow registry versions behind an alias such as `@prod` | Persisted advisory verdict | Audited promote records the outgoing alias target; rollback restores it | Provider optimizer + EvalProvider |
| **Workflow** | Editable drafts → published version rows; deployment aliases select one | Deploy-gate policy, optimistic alias check | Rollback pops the deployment's checkpoint stack | Manifest replay |
...
| **Skill** | Mutable current skill + immutable version snapshots | None | Rollback restores the prior snapshot as a new current version | Agent-free optimizer path |
...
flowchart TB
    ...
    subgraph DOM["RELATIONAL DOMAINS"]
      ...
      subgraph DR1[" "]
        ...
        m3["<b>Tools &amp; skills</b><br/>registry rows · test runs<br/>skill versions"]:::dom
